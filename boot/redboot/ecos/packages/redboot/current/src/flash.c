...
//
void
do_bdmove(int argc, char *argv[])
{
    int flash_data_found;
    volatile char *bd_cfg;
    unsigned char cfg_data[2296];
    unsigned char *bd_data;
    unsigned short *eeprom;
    volatile char *cal_cfg;
    int i;

#ifndef CYGNUM_USE_CONSOLIDATED_BOARD_DATA
    return;
#endif

    if (*(int *) 0xbfdf0000 == 0x35333131) {
	diag_printf("Board Data is already relocated!\n");
	return;
    }

    bd_data = &cfg_data[0];
    eeprom = (unsigned short *) &cfg_data[248];

    memset((void *) &cfg_data[0], 0, sizeof(cfg_data));
     ...
     */
    flash_data_found = 0;
    for (bd_cfg = (char *)0xbfde0000;
	 bd_cfg > (char *)0xbfd80000;
	 bd_cfg -= 0x1000) {
	if ( *(int *) bd_cfg == 0x35333131) {
	    flash_data_found = 1;
	    break;
	}
    }
    ...
    if (((int) bd_cfg & ~0xbfd00000) == 0x1f0000) {
	diag_printf("Board data has already been relocated!\n");
	return;
    }
    
    memcpy((void *) bd_data, (void *) bd_cfg, 248);
    
    if (*(int *) bd_data != 0x35333131) {
	diag_printf("Failed to cache boarddata\n");
	return;
    }
    
    flash_data_found = 0;
    for (cal_cfg = bd_cfg + 0x1000; cal_cfg < (char *) 0xbffff000;
	 cal_cfg += 0x1000) {
	if (*(int *) cal_cfg != 0xffffffff) {
	    flash_data_found = 1;
	    break;
	}
    }
    ...
    memcpy((void *) eeprom, (void *) cal_cfg, 2048);
    
    if (eeprom[1] != 0x168c) {
	diag_printf("Failed to cache eeprom\n");
	return;
    }

    diag_printf("Starting sector program for 0x%08x! Please wait!\n", 
		(int) cal_cfg);
    (void) flash_erase_block((void *) cal_cfg, (unsigned int) 0x10000);
    (void) flash_program_buf((void *) cal_cfg, 
			     (void *) cfg_data,  2296, 0, 0);

    if ((*(volatile int *) cal_cfg) == 0x35333131) {
	(void) flash_erase_block((void *) bd_cfg, (unsigned int) 0x10000);
	diag_printf("Relocation completed successfully!!\n");
    }
    else
	diag_printf("Relocation FAILED!!\n");
...
//
void
do_bdrestore(int argc, char *argv[])
{
    int flash_data_found;
    volatile char *bd_cfg;
    unsigned char cfg_data[2296];
    ...
    /*
     * Find start of Board Data data, using heuristics:
     * Search back from the (aliased) end of flash by 0x1000 bytes
     * at a time until we find the string "5311", which marks the
     * start of Board Configuration.  Give up if we've searched
     * more than 500KB.
    ...
    flash_data_found = 0;
    for (cal_cfg = bd_cfg + 0xf8; cal_cfg < (char *) 0xbffdf000;
	 cal_cfg += 0x10f8) {
	if (*(int *) cal_cfg != 0xffffffff) {
	    flash_data_found = 1;
	    break;
    ...
    diag_printf("Starting sector program for 0x%08x! Please wait!\n",
		(int) 0xbfde0000);
    (void) flash_erase_block((void *) 0xbfde0000, (unsigned int) 0x10000);
    (void) flash_program_buf((void *) 0xbfde0000, 
			     (void *) bd_data,  248, 0, 0);

    if (*(int *) 0xbfde0000 != 0x35333131) {
	diag_printf("Failed to restore Board Data!!\n");
	return;
    }
    ...
    diag_printf("Starting sector program for 0x%08x! Please wait!\n", 
		(int) bd_cfg);
    (void) flash_erase_block((void *) bd_cfg, (unsigned int) 0x10000);
    (void) flash_program_buf((void *) bd_cfg, 
			     (void *) eeprom,  2048, 0, 0);

    if ((*(volatile int *) 0xbfde0000) == 0x35333131) {
	diag_printf("Restore completed successfully!!\n");
    }
    else
	diag_printf("Restore FAILED!!\n");
...
fis_init(int argc, char *argv[])
{
	...
	if ( *(int *) 0xbfde0000 == 0x35333131) {
	    diag_printf("Relocating Board Data to new location.\n");
	    do_bdmove( 0, (char *) NULL);
	}
#endif
