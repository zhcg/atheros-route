...
#define set_next_addr(d, a)	do {				\
	(d)->nextAddr = (struct zsDmaDesc *)PHYSADDR(a);	\
} while(0)
#define get_next_addr(d)	\
	({ (struct zsDmaDesc *)KSEG1ADDR((d)->nextAddr); })

#define set_last_addr(d, a)	do {				\
	(d)->lastAddr = (struct zsDmaDesc *)PHYSADDR(a);	\
} while(0)
#define get_last_addr(d)	\
	({ (struct zsDmaDesc *)KSEG1ADDR((d)->lastAddr); })

#define set_data_addr(d, a)	do {				\
	(d)->dataAddr = (volatile u32_t)PHYSADDR(a);	\
} while(0)
