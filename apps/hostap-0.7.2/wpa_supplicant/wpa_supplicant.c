...
struct wpa_ssid * wpa_supplicant_get_ssid(struct wpa_supplicant *wpa_s)
{
	...
	while (entry) {
		/* When ssid is not set and bssid is set, we use the current SSID
		   retrieved from wpa_drv_get_ssid to recalculate the PMK */
		if (entry->ssid_len == 0 && entry->bssid_set == 1) {
			if (strlen(ssid) != 0) {
				entry->ssid = os_malloc(MAX_SSID_LEN+1);
				os_memcpy(entry->ssid, ssid, MAX_SSID_LEN);
				entry->ssid_len = ssid_len;
				wpa_config_update_psk(entry);
