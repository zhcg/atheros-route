...
{
	extern int handle_reload_iface (struct hostapd_iface *, void *);
...
*/
	/* Dynamically applying the new config wont work well 
	   with our Atheros driver, so restarting the hostapd 
	   when new the new configuration is set through wps 
	*/

	handle_reload_iface(iface,user_ctx);
}
...
static int hostapd_wps_cred_cb(void *ctx, const struct wps_credential *cred)
{
	...
	/* to support saving the config in flash for 2mb flash ap solutions*/
	sprintf(buf,"cfg wpssave %s", hapd->iface->config_fname);
	system(buf);

	/* mark the wps state to configured in flash parameters*/
	sprintf(buf,"cfg -a WPS_ENABLE=%d",2);
	system(buf);

	/* save the param in nvram*/
	sprintf(buf,"cfg -c");
	system(buf);
#endif
	os_free(tmp_fname);
...
//	eloop_register_timeout(0, 100000, wps_reload_config, hapd->iface,
	eloop_register_timeout(15, 0, wps_reload_config, hapd->iface,
			       NULL);
