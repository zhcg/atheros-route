...
static int IsDhcpdAlive(pid_t pid)
{
    FILE  *pFile = NULL;
    char buf[128] = {};
    char proc[128] = {};
    snprintf(proc,128,"/proc/%d/cmdline",pid);
    pFile = fopen(proc,"r");
    if(pFile == NULL) return 0;
    fread(buf,sizeof(char),sizeof(buf),pFile);
    fclose(pFile);

    if(buf[0] == 'd' && buf[1] == 'h' && buf[2] == 'c' && buf[3] == 'p' && buf[4] == 'd') return 1;
    else return 0; 
	...
	if ((i = open (path_dhcpd_pid, O_RDONLY)) >= 0) {
		...
		if (status > 0) {
			...
			/*
                         ...
                         */
			if (!pid || (pid != getpid() && kill(pid, 0) == 0 && IsDhcpdAlive(pid) ))  
