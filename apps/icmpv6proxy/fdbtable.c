...
struct FdbTable *
findFdb(struct in6_addr *group)
{
    struct FdbTable *cfdb;

    for (cfdb = fdb_table; cfdb; cfdb = cfdb->nextfdb) 
    {       
        if ( IN6_ARE_ADDR_EQUAL( &(cfdb->group), group ) ) 
            return cfdb;
    }
...
int insertFdb(struct in6_addr *group, uint32 port)
{
    ...
    {
        /* atlog(LOG_WARNING, 0, "The group address %s is not a valid Multicast group. Table insert failed.",
            inetFmt(group, s1)); */
        return 0;
    }
    ...
    if (port == 0)
    {
        atlog(LOG_WARNING, 0, "port %d is not a valid port. Table insert failed.", port);
        return 0;
    }
    ...
    if (cfdb == NULL) 
    {
        ...
        //IF_DEBUG atlog(LOG_DEBUG, 0, "No existing fdb for %s. Create new.", inetFmt(group, s1));
        ...
        if (fdb_table == NULL) 
        {
            ...
            fdb_table = newfdb;
            IF_DEBUG atlog(LOG_DEBUG, 0, "No fdbs in table. Insert at beginning.");
        } 
        else 
        {
            IF_DEBUG atlog(LOG_DEBUG, 0, "Found existing fdbs. Find insert location.");

            // Check if the route could be inserted at the beginning...
            if ( memcmp( &(fdb_table->group), group, sizeof(*group)) > 0) 
            {
                /* IF_DEBUG atlog(LOG_DEBUG, 0, "Inserting at beginning, before route %s",
            ...
            else 
            {
                ...
                for (cfdb = fdb_table; cfdb->nextfdb != NULL; cfdb = cfdb->nextfdb) 
                {
...
*/
int removeFdb(struct FdbTable *cfdb)
{
    //struct Config *conf = getCommonConfig();
    int result = 1;
...
void clearAllFdbs()
{
    ...
    for (cfdb = fdb_table; cfdb; cfdb = remainfdb) 
    {
        ...
        // Log the cleanup in debugmode...
        //IF_DEBUG atlog(LOG_DEBUG, 0, "Removing route entry for %s", inetFmt(cfdb->group, s1));
...
*/
static int internAgeFdb(struct FdbTable *cfdb)
{
    struct Config *conf = getCommonConfig();
    int result = 0;
    ...
    if (cfdb->ageValue == 0) 
    {
        ...
        if (cfdb->ageActivity > 0)
        {
            ...
            cfdb->swifBits = cfdb->ageSwifBits;
...
void showFdbsTable()
{
    ...
    printf("\n****************showFdbsTable called***********************\n");

    for (cfdb = fdb_table; cfdb; cfdb = cfdb->nextfdb) 
    {
        printf("group = ");
        for( i = 0; i < 16; i++ )
            printf("%02x ", ((char *)&(cfdb->group))[i] & 255);
        printf("\n");
        printf("swifBits = %d\n", cfdb->swifBits);
        printf("ageSwifBits = %d\n", cfdb->ageSwifBits);
        printf("ageSwifResultBits = %d\n", cfdb->ageSwifResultBits);
        printf("ageValue = %d\n", cfdb->ageValue);
        printf("ageActivity = %d\n\n", cfdb->ageActivity);
    }
    printf("****************showFdbsTable done***********************\n\n");    
}

/**
*   Debug function 
*/
void showFdbs(struct FdbTable *cfdb)
{
    int i;
