...
int loadConfig(char *configFile) 
{
    ...
    {
        atlog(LOG_ERR, 0, "Unable to open configfile from %s", configFile);
    }

    // Get first token...
    token = nextConfigToken();
    if(token == NULL) 
        atlog(LOG_ERR, 0, "Config file was empty.");
    ...
    while ( token != NULL ) 
    {
        ...
        if(strcmp("phyint", token)==0) 
        {
            ...
            IF_DEBUG atlog(LOG_DEBUG, 0, "Config: Got a phyint token.");
            tmpPtr = parsePhyintToken();
            if(tmpPtr == NULL) 
            {
                // Unparsable token... Exit...
                closeConfigFile();
                atlog(LOG_WARNING, 0, "Unknown token '%s' in configfile", token);
                return 0;
            } 
            else 
            {

                IF_DEBUG atlog(LOG_DEBUG, 0, "IF name : %s", tmpPtr->name);
                IF_DEBUG atlog(LOG_DEBUG, 0, "Next ptr : %x", tmpPtr->next);
                IF_DEBUG atlog(LOG_DEBUG, 0, "Ratelimit : %d", tmpPtr->ratelimit);
                IF_DEBUG atlog(LOG_DEBUG, 0, "Threshold : %d", tmpPtr->threshold);
                IF_DEBUG atlog(LOG_DEBUG, 0, "State : %d", tmpPtr->state);
                IF_DEBUG atlog(LOG_DEBUG, 0, "GlobalIpInfo ptr : %x", tmpPtr->globalIpInfo);
        ...
        else if(strcmp("quickleave", token)==0) 
        {
            ...
            IF_DEBUG atlog(LOG_DEBUG, 0, "Config: Quick leave mode enabled.");
        ...
        else if(strcmp("mode", token)==0) 
        {
            // Got a mode token...
	        token = nextConfigToken();
            IF_DEBUG atlog(LOG_DEBUG, 0, "Config: set mode to '%s'.", token);
            commonConfig.mode = atoi( token );   
...
void configureVifs() 
{
    ...
    for ( Ix = 0; (Dp = getIfByIx( Ix )); Ix++ ) 
    {
        ...
        for( confPtr = vifconf; confPtr; confPtr = confPtr->next) 
        {
            ...
            {
                IF_DEBUG atlog(LOG_DEBUG, 0, "Found config for %s", Dp->Name);
                ...
                if (confPtr->globalIpInfo == NULL)
                    break;
                
                p = Dp->globalIpInfo;
                if (p == NULL)
                {
                    Dp->globalIpInfo = confPtr->globalIpInfo;
                    break;
                }

                if ( memcmp( p, confPtr->globalIpInfo, sizeof(struct globalIpList) ) == 0 ) 
                    break;
                
                while (p->next)
                {
                    if ( memcmp( p->next, confPtr->globalIpInfo, sizeof(struct globalIpList)) == 0 )
                        break;
                    else
                        p = p->next;
                }

                p->next = confPtr->globalIpInfo;
                break;
            }
        }
    }
...
struct vifconfig *parsePhyintToken() 
{
    ...
    if(token == NULL) 
        return NULL;
    if(strlen(token) >= sizeof( ((struct ifreq *)NULL)->ifr_name) ) 
        return NULL;
    IF_DEBUG atlog(LOG_DEBUG, 0, "Config: IF: Config for interface %s.", token);

    // Allocate memory for configuration...
    tmpPtr = (struct vifconfig*)malloc(sizeof(struct vifconfig));
    if(tmpPtr == NULL)
    {
        atlog(LOG_ERR, 0, "Out of memory.");
    }
    ...
    {        
...
#if 0           
            // Altnet...
            //struct in_addr  networkAddr; /* moved this avoid warning */

            token = nextConfigToken();
            IF_DEBUG atlog(LOG_DEBUG, 0, "Config: IF: Got altnet token %s.",token);

            *anetPtr = parseSubnetAddress(token);
            if(*anetPtr == NULL)
            {
                parseError = 1;
                atlog(LOG_WARNING, 0, "Unable to parse subnet address.");
                break;
            } 
            else
            {
        ...
        {
            // Upstream
            IF_DEBUG atlog(LOG_DEBUG, 0, "Config: IF: Got upstream token.");
            tmpPtr->state = IF_STATE_UPSTREAM;
        }
        else if(strcmp("downstream", token)==0) 
        {
            // Downstream
            IF_DEBUG atlog(LOG_DEBUG, 0, "Config: IF: Got downstream token.");
            tmpPtr->state = IF_STATE_DOWNSTREAM;
        }
        else if(strcmp("disabled", token)==0) 
        {
            // Disabled
            IF_DEBUG atlog(LOG_DEBUG, 0, "Config: IF: Got disabled token.");
        ...
        else if(strcmp("ratelimit", token)==0) 
        {
            // Ratelimit
            token = nextConfigToken();
            IF_DEBUG atlog(LOG_DEBUG, 0, "Config: IF: Got ratelimit token '%s'.", token);
            tmpPtr->ratelimit = atoi( token );
            if(tmpPtr->ratelimit < 0) 
            {
                atlog(LOG_WARNING, 0, "Ratelimit must be 0 or more.");
                parseError = 1;
                break;
            }
        }
        else if(strcmp("threshold", token)==0) 
        {
            // Threshold
            token = nextConfigToken();
            IF_DEBUG atlog(LOG_DEBUG, 0, "Config: IF: Got threshold token '%s'.", token);
            tmpPtr->threshold = atoi( token );
            if(tmpPtr->threshold <= 0 || tmpPtr->threshold > 255) 
            {
                atlog(LOG_WARNING, 0, "Threshold must be between 1 and 255.");
                parseError = 1;
                break;
            }
        }
...
*/
struct SubnetList *parseSubnetAddressparseSubnetAddress(char *addrstr) {
    ...
    if(tmpStr != NULL) {
        ...
        if(bitcnt <= 0 || bitcnt > 32) {
            atlog(LOG_WARNING, 0, "The bits part of the address is invalid : %d.",tmpStr);
            return NULL;
        }
    ...
    if(addr == -1 || addr == 0) {
        atlog(LOG_WARNING, 0, "Unable to parse address token '%s'.", addrstr);
        return NULL;
    }
    ...
    IF_DEBUG atlog(LOG_DEBUG, 0, "Config: IF: Altnet: Parsed altnet to %s.",
