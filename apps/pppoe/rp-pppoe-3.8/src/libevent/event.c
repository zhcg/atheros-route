...
"$Id: //depot/sw/releases/Aquila_9.2.0_U11/apps/pppoe/rp-pppoe-3.8/src/libevent/event.c#1 $";
...
***********************************************************************/
int
Event_DelHandlerByFd(EventSelector *es,
		 int fd)
{
    /* Scan the handlers list */
    EventHandler *cur, *prev;
    EVENT_DEBUG(("Event_DelHandler(es=%p, fd=%d)\n", es, fd));
    for (cur=es->handlers, prev=NULL; cur; prev=cur, cur=cur->next) {
	if (cur->fd == fd) {
	    if (es->nestLevel) {
		cur->flags |= EVENT_FLAG_DELETED;
		es->opsPending = 1;
		return 0;
	    } else {
		if (prev) prev->next = cur->next;
