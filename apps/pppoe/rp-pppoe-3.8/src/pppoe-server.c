...
*
* $Id: //depot/sw/releases/Aquila_9.2.0_U11/apps/pppoe/rp-pppoe-3.8/src/pppoe-server.c#1 $
*
* LIC: GPL
*
***********************************************************************/
...
/*WAN interface name*/
char* wan_inf_name=NULL;
/*auth type*/

char* authType = "require-pap";
...
#define pppoe_proxy
int enableProxy = 0;
unsigned short pppdBindPort;
int proxyListenSock = -1;
char*username = NULL;
char*password = NULL;
...
processPADI(Interface *ethif, PPPoEPacket *packet, int len)
{
	...
	/* PADI asked for unsupported service */
      printf("PADI asked for unsupported service\n");
	return;
    }
...
termHandler(int sig)
{
    ...
    if(enableProxy)
    killAllProxy();
    if(proxyListenSock>=0)close(proxyListenSock);
    control_exit();
    exit(0);
}
...
#else
    fprintf(stderr, "   -I if_name  eg.'eth0,eth1'    -- Specify interface (default %s.)\n",
	    DEFAULT_IF);
#endif
    fprintf(stderr, "   -W wan_if_name     -- set wan/proxy interface name\n");
    fprintf(stderr, "   -A  enableProxy  -- start the proxy function\n");
    fprintf(stderr, "   -U  'username,password'  -- set the proxy account\n");
...
main(int argc, char **argv)
{
    ...
    char *options = "hI:C:L:R:T:m:FN:f:O:o:spa:lrudPc:S:1:W:U:A";
#else
    char *options = "hI:C:L:R:T:m:FN:f:O:o:skpa:lrudPc:S:1:W:U:A";
#endif
    if (getuid() != geteuid() ||
	getgid() != getegid()) {
	...
	case 'k':
       ...
       case 'U':
           if(1)
           {
               char*accoutInfo=NULL;
               char*pact;
               
               SET_STRING(accoutInfo, optarg);
               pact = strtok(accoutInfo,",");
               username=pact;
               pact = strtok(NULL,",");
               password=pact;
           }
	...
	case 'I':
           ...
           else*/
           {
               char*ifgroup=NULL;
               char*pif;
               SET_STRING(ifgroup, optarg);
               pif = strtok(ifgroup,",");
               while(pif)
               {
                   found = 0;
                   for (i=0; i<NumInterfaces; i++) {
                   if (!strncmp(interfaces[i].name, pif, IFNAMSIZ)) {
                       found = 1;
                       break;
                   }
                   }
                   if (!found) {
                   strncpy(interfaces[NumInterfaces].name, pif, IFNAMSIZ);
                   NumInterfaces++;
                   }
                   pif = strtok(NULL,",");
               }
    ...
    if(!wan_inf_name)wan_inf_name="eth0";
#ifdef HAVE_LICENSE
    ...
    if(enableProxy)
    {
        ...
        if(username&&password)
            allocProxySes( username, password);
    }
...
startPPPDUserMode(ClientSession *session)
{
    ...
    if(enableProxy)
    {
        ...
        argv[c++] = "auth";
        /*argv[c++] = "defaultroute";*/
        argv[c++] = "hide-password";
        argv[c++] = "ipcp-accept-local";
        argv[c++] = "ipcp-accept-remote";
        argv[c++] = "user";
        argv[c++] = "proxy";
        argv[c++] = "password";
        argv[c++] = "proxy";
...
proxySessionList gProxySessionList;
void initProxy()
{
     
     initProxySesList();
     openPppdSocket(interfaces[0].name);
     printf("interfaces[0].name [%s],pppdBindPort=%d\n",interfaces[0].name,pppdBindPort);
}
void initProxySesList()
{
     int i;
     proxySession*ses;
     gProxySessionList.FirstProxySes = (proxySession*)calloc(sizeof(proxySession),MAX_PROXY_SESSION);
     gProxySessionList.freeProxySes = gProxySessionList.FirstProxySes;
	 
     ses = gProxySessionList.FirstProxySes;
     for(i=0;i<MAX_PROXY_SESSION-1;i++)
     {
         ses->next = ++ses;
     }	 	
}
...
{
     proxySession*pProxySes;
     clientSession*pCliSes;
     int i;
     if(!username || !pwd)return NULL;
     if(gProxySessionList.freeProxySes)
        {
            pProxySes = gProxySessionList.freeProxySes;
            pProxySes->state = e_state_active;
            gProxySessionList.freeProxySes = gProxySessionList.freeProxySes->next;
            pProxySes->clientSesList = (clientSession*)calloc(sizeof(clientSession),MAX_CLIENT_SESSION);
            pProxySes->freeCliSes = pProxySes->clientSesList;
            pCliSes = pProxySes->freeCliSes;
	     pCliSes->proxySession = pProxySes;
            for(i=0;i<MAX_CLIENT_SESSION-1;i++)
            {
                pCliSes->next = ++pCliSes;
	         pCliSes->proxySession = (void*)pProxySes;
            }	 
            pProxySes->username = (char*)strdup(username);
            pProxySes->pwd = (char*)strdup(pwd);
...
int freeProxySes(proxySession*session)
{
    int i;
    clientSession*pCliSes;
    if(!session)return RV_FALSE;
    if(session->state == e_state_inactive)return RV_FALSE;
    free(session->pwd);
    free(session->username);
    /*if(session->sock)close(session->sock);*/
    pCliSes = session->clientSesList;
    for(i=0;i<MAX_CLIENT_SESSION;i++)
    {
        if(pCliSes[i].state)
        { 
            if (pCliSes[i].childPid) {
            kill(pCliSes[i].childPid, SIGTERM);
            }
            delClientSes(&pCliSes[i]);
         }
    }	 
    free(session->clientSesList);
    memset(session,0,sizeof(proxySession));
    session->state = e_state_inactive;
    session->next = gProxySessionList.freeProxySes;
    gProxySessionList.freeProxySes = session;
}
proxySession* checkProxySes(char*username,char*pwd)
{
     ...
     proxySession*pProxySes;
     if(!username || !pwd)return NULL;
     pProxySes = gProxySessionList.FirstProxySes;
     for(i=0;i<MAX_PROXY_SESSION;i++)
      {
         if(pProxySes->state == e_state_active)
            {
                if(!strcmp(pProxySes->username,username)
                    &&!strcmp(pProxySes->pwd,pwd))
                    {
                        return pProxySes;
...
{
     clientSession*pCliSes;
     if(!proxySes)return NULL;
     if(proxySes->freeCliSes)
        {
           pCliSes = proxySes->freeCliSes;
	    proxySes->actCliNum++;
           proxySes->freeCliSes = proxySes->freeCliSes->next;
           pCliSes->proxySession = (void*)proxySes;
           pCliSes->state = e_alloc;
           return pCliSes;
        }
     return NULL;
}
...
{
    clientSession*pCliSes;
    proxySession*proxySes;
    if( !clientSes)return RV_FALSE;
    proxySes = (proxySession*)clientSes->proxySession;
    if( !proxySes)return RV_FALSE;
    memset(clientSes,0,sizeof(clientSession));
    /*if(clientSes->sock)close(clientSes->sock);*/
    clientSes->next = proxySes->freeCliSes;
    proxySes->freeCliSes = clientSes;
    proxySes->actCliNum--;
    return RV_TRUE;
}
...
{
    int i,j;
    proxySession*pProxySes = gProxySessionList.FirstProxySes;
    for(i=0;i<MAX_PROXY_SESSION;i++)
        for(j=0;j<MAX_CLIENT_SESSION;j++)
            {
                 if(pProxySes[i].state == e_state_active)
                    {
                         if(pProxySes[i].clientSesList[j].state)
                            {
                                if(pProxySes[i].clientSesList[j].childPid == pid)
                                    return &pProxySes[i].clientSesList[j];
                            }
                    }
            }
     return NULL;
...
{
    clientSession*pCliSes = NULL;
    proxySession*pProxySes;
    if(pCliSes = checkClientSes(pid))
    	{
    	    pProxySes = (proxySession*)pCliSes->proxySession;
    	    if(delClientSes(pCliSes))
    	    	{
    	    	     if(pProxySes && pProxySes->actCliNum==0)
    	    	     	{
    	    	     	     pid_t ppid = pProxySes->pid;
			     freeProxySes(pProxySes);
    	    	     	     if(ppid)kill(ppid, SIGTERM);
    	    	     	}
		       return RV_TRUE;
...
void killAllProxy()
{
    ...
    for(i=0;i<MAX_PROXY_SESSION;i++)
        if(pProxySes[i].state == e_state_active)
        {
            kill(pProxySes[i].pid, SIGTERM);
        }
}
static void
proxyChildHandler(pid_t pid, int status, void *s)
{
    proxySession*pProxySes = (proxySession*)s;
    
    freeProxySes(pProxySes);
}
/***************************************************
communication with pppd by socket ;
pppd include:server & proxy
***************************************************/

void pppdCommAccept(struct EventSelector_t *es,
    int fd, unsigned int flags,
    void *data);
void pppdCommHandler(struct EventSelector_t *es,
    int fd, unsigned int flags,
    void *data);
void testsock(char*ifName)
{
    int sock;
    struct sockaddr_in my_addr;
    if(ifName)
    {
        if(memcmp(ifName,"eth",3)==0)
        {
            pppdBindPort = STARTING_COMM_PORT + ifName[3]-'0';
        }
        else return ;
    }
    else return ;
    ...
    my_addr.sin_port = htons(pppdBindPort);
...
int openPppdSocket(char*ifName)
{
     int sock;
     struct sockaddr_in my_addr;
     if(ifName)
     ...
     if((proxyListenSock = socket(AF_INET,SOCK_STREAM,0)) < 0)
        {
             perror("openPppdSocket failed!");
             return RV_FALSE;
        }
     bzero(&(my_addr),sizeof(my_addr));
     my_addr.sin_family=AF_INET;
     ...
     if (bind(proxyListenSock, (struct sockaddr *)&my_addr, sizeof(my_addr)) == -1) {
        perror("bind error!");
        close(proxyListenSock);
        exit(1);
        return RV_FALSE;
        }
     if (listen(proxyListenSock, 10) == -1) {
        perror("listen error");
        close(proxyListenSock);
        return RV_FALSE;
        }
    ...
    void *data)
{
     ...
     if ((client_fd = accept(fd, (struct sockaddr *)&remote_addr, \
        &addrlen)) == -1) {
        perror("accept error");
        return;
        }
     syslog(LOG_INFO,
	   "pppdCommAccept socket1[%d]\n",client_fd);
     Event_AddHandler(event_selector,
        client_fd,
        EVENT_FLAG_READABLE,
        pppdCommHandler,
    ...
    void *data)
{
      pppdCommMsg message;
      int recvbytes;
      syslog(LOG_INFO,"pppdCommHandler socket[%d]\n",fd);
      if ((recvbytes=recv(fd, &message, sizeof(message), 0)) ==-1) {      
      perror("pppdCommHandler recv error");
      return;
      } 
      else if(recvbytes==0)
      {
          Event_DelHandlerByFd(es,fd);
          close(fd);
          return;
      }
      ...
      {
        case e_com_reqAuth:
            processReqAuth(&message,fd);
            break;
            case e_com_rspAuth:
                processRspAuth(&message,fd);
                break;
                default:
                    perror("message.msgtype error\n");
                    break;
       }
}
int g_test_sock = -1;
...
void processReqAuth(pppdCommMsg*message,int sock)
{
     pid_t pid;
     authMsg*pAuthMsg;
     proxySession* pProxySes = NULL;
     pid = message->pid;
     pAuthMsg = (authMsg*)message->data;
     syslog(LOG_INFO,
                "processReqAuth: socket[%d]\n",sock);
     /*for test*/
     if(pProxySes = checkProxySes(pAuthMsg->user,pAuthMsg->pwd) == 0)
        {
           if(pProxySes = allocProxySes(pAuthMsg->user,pAuthMsg->pwd))
               {
                  clientSession*pClientSes;
            if(pClientSes = addClientSes(pProxySes))
                        {
                          pClientSes->childPid = pid;
                            pClientSes->sock = sock;
                            /*g_test_sock = sock;*/
                pClientSes->state = e_waiting_auth;              
                   forkPppdProxy(pProxySes);
                }
                    else ;
            }
            else
     ...
     {
          /*
          if(checkClientSes(pid))
             {
                   ;
               }
             else*/
                syslog(LOG_INFO,
                "the ppp account[%s,%s] has been authenticated successfully\n",pAuthMsg->user,pAuthMsg->pwd);
                responseAuthSucc(message,sock);
                return;
                {
                    clientSession*pClientSes;
                     if(pProxySes->actCliNum <NumSessionSlots)
                     if(pClientSes = addClientSes(pProxySes))
                                 {
                                   pClientSes->childPid = pid;
                                     pClientSes->sock = sock;
                         pClientSes->state = e_auth_suc;
                                  responseAuthSucc(message,sock);
                     }
                     else responseAuthFail(message,sock);
                    }
             }
...
void processRspAuth(pppdCommMsg*message,int sock)
{
     ...
     if(pProxySes = checkProxySes(pAuthMsg->user,pAuthMsg->pwd))
        {
          clientSession*pCliSes;
            pCliSes = pProxySes->clientSesList;
             for(i=0;i<MAX_CLIENT_SESSION;i++)
              {
                  if(pCliSes->state == e_waiting_auth)
                      {
                          /*response auth result to pppd-server*/
                             responseAuthSucc(message,pCliSes->sock);
                             pCliSes->state = e_auth_suc;
                             }
                    pCliSes ++;
            }
      }
...
int forkPppdProxy(proxySession* pProxySes)
{
    pid_t child;
    int i;
    child = fork();
    ...
    startPppdProxy(pProxySes);
    return 1;
    
}
void
startPppdProxy(proxySession* pProxySes)
{
    /* Leave some room */
    char *argv[64];
    ...
    if (UseLinuxKernelModePPPoE)
    {
        argv[c++] = "plugin";
        argv[c++] = PLUGIN_PATH;
        snprintf(buffer, SMALLBUF, "nic-%s", wan_inf_name);
        argv[c++] = strdup(buffer);
        /*argv[c++] = "pppoe-proxy"; */
    ...
    {
        argv[c++] = "pty";
        snprintf(buffer, SMALLBUF, "pppoe  -I %s -T 80 -U  -m 1412", wan_inf_name);
        argv[c++] = strdup(buffer);
    }
    argv[c++] = "pppoe-port";
    sprintf(buffer,"%d",pppdBindPort);
    ...
    /*user and password*/
    argv[c++] = "user";
    argv[c++] = strdup(pProxySes->username);
    argv[c++] = "password";
    argv[c++] = strdup(pProxySes->pwd);
...
{
    if(sock<0)return;
    message->msgtype = e_com_rspAuth;
    message->result = e_auth_suc;
    if(send(sock,message,sizeof(pppdCommMsg),0) == -1)
        {
            perror("send error");
            close(sock);
        }
