...
"$Id: //depot/sw/releases/Aquila_9.2.0_U11/apps/pppoe/rp-pppoe-3.8/src/discovery.c#1 $";
...
discovery(PPPoEConnection *conn)
{
	...
	if (padiAttempts > MAX_PADI_ATTEMPTS) {
		...
		printWarning("Timeout waiting for PADO packets");
	    } else {
	...
	if (padrAttempts > MAX_PADI_ATTEMPTS) {
		...
		padrAttempts = 0;
		timeout = conn->discoveryTimeout;
		printWarning("Timeout waiting for PADS packets");
	    } else {
