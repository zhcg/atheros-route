...
#define IP_ICMP6_TYPE				0x3A

#define IPV6_OFFSET_PAYLOAD_LENGTH	4
#define IPV6_OFFSET_NEXT_HEADER		6
...
typedef struct 
	{
#		define ICMP6_NS	135		// neighbor sollicitation
#		define ICMP6_NA	136		// neighbor advertisement
		UCHAR	m_Type;			// Type
		UCHAR	m_Code;			// Code 
		USHORT	m_Checksum;		// Checksum
		ULONG	m_Reserved;		// Reserved
		IP6ADDR m_Target_Address;	// ipv6 address of target
#		define ICMP6_OPTION_SOURCE_LL_ADDRESS	1
#		define ICMP6_OPTION_TARGET_LL_ADDRESS	2
		UCHAR	m_Options_Type;
		UCHAR	m_Options_Length;
		MACADDR	m_Options_Payload;		// icmp6 options, if any. could be none. see type and length
										// we stick to MACADDResses
	}
ICMP6_PACKET, *PICMP6_PACKET;

typedef struct {
        union {
                struct ip6_hdrctl {
                        ULONG ip6_un1_flow; /* 20 bits of flow-ID */
        ...
        IP6ADDR ip6_src;        /* source address */
        IP6ADDR ip6_dst;        /* destination address */
...
#define IPV6_HEADER_SIZE (sizeof (IP6_HEADER))
