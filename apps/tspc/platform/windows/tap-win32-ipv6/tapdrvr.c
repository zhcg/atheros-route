		 ...
		 IN UINT p_Flags)
{
  ...
  __try
  {
    ...
    // packet is ARP or IPv4 or IPv6 (if none, then drop).
    //=====================================================
	...
	switch (ntohs (e->proto))
	  ...
	  case ETH_P_IPV6:
		  ...
		  // First we need to see if this ICMP, if so is it a NS message 
		  // and if so process it

		  if ( 
			   *(l_PacketBuffer->m_Data + ETHERNET_HEADER_SIZE + IPV6_OFFSET_NEXT_HEADER)
			  == IP_ICMP6_TYPE ) {
				  DEBUGP(("[%s] Received an ICMP6 packet, about to call ProcessNS\n", l_Adapter->m_Name));
				  if ( ProcessNS(l_Adapter, l_PacketBuffer->m_Data) == 1 )
					  goto no_queue;  // and let it stop there if it was NS
			  }

			  goto process_ip_packets; // we are done, wasnt NS; now let the packet flow
...
process_ip_packets:
	    ...
	    // not broadcasts - dont check the proto - we want ipv6 and ipv4
	    if (memcmp (e, &l_Adapter->m_TapToUser, ETHERNET_HEADER_SIZE - sizeof (l_Adapter->m_TapToUser.proto) ))
	      goto no_queue;

	    // Packet looks like IP, queue it.
  ...
  switch (l_IrpSp->MajorFunction)
	...
	switch (l_IrpSp->Parameters.DeviceIoControl.IoControlCode)
	  ...
	  case TAP_IOCTL_CONFIG_POINT_TO_POINT:
		  ...
		  (sizeof (IPADDR) * 2))
		{
		  ...
		  l_Adapter->m_TapToUser.proto = l_Adapter->m_UserToTap.proto = htons (ETH_P_IPV6);
	...
	if (p_IRP->MdlAddress == NULL)
	...
	else if (l_Adapter->m_PointToPoint && ((l_IrpSp->Parameters.Write.Length) >= IP_HEADER_SIZE))
	  {
		...
		/* XXX This breaks ipv4 support. It would need
		 * a check to see what the packet is at the IP level
		 * and stamp the ethernet header accordingly,
		 * thus further breaking the encapsulation
...
//===================================================
// Generate a return NA message for
// a NS message.
// Proxy NDP for point-to-point IPv6 mode.
// return 1 the packet doesnt need further processing,
// 0 otherwise.
//
// this needs to detect a NS message and forge
// an IP6 NA message back.
//
// So it requires an icmp6 header + ipv6 header
// + ethernet header.
//
//===================================================

int ProcessNS (TapAdapterPointer p_Adapter, const UCHAR *src) {

	// is this not a neighbor sollicitation message?
	// we already know its ipv6 (from the eth header)
	// and also that is icmp6 (from the ipv6 header)

	// get references to the source packet in context

	PIP6_HEADER pip6h_source = (PIP6_HEADER) (src + ETHERNET_HEADER_SIZE);
	PICMP6_PACKET picmp6p_source = (PICMP6_PACKET) (src + ETHERNET_HEADER_SIZE	+ IPV6_HEADER_SIZE);
	
	if (src == NULL)
		return 0;		// fail in silence

	// if this is not a NS, fail

	if (picmp6p_source->m_Type != ICMP6_NS)		
		return 0;

	// NOTE:

	/* if Duplicate Address
       Detection is in progress [ADDRCONF] source address is the
       unspecified address.
	   */

	// If this is the case, fail

	if (pip6h_source->ip6_src.__u6_addr.__u6_addr32[0] == 0 &&
		pip6h_source->ip6_src.__u6_addr.__u6_addr32[1] == 0 &&
		pip6h_source->ip6_src.__u6_addr.__u6_addr32[2] == 0 &&
		pip6h_source->ip6_src.__u6_addr.__u6_addr32[3] == 0)
		return 0;	// DAD in progress, we dont respond to that


	// ok. we need to generate a neighbor advertisement
	// message and send it down (or is it up?)

	else {
		UCHAR packet[sizeof(ETH_HEADER) + sizeof(IP6_HEADER) + sizeof(ICMP6_PACKET)];

		PETH_HEADER eh = (PETH_HEADER) packet;
		PIP6_HEADER ip6h = (PIP6_HEADER) (packet + sizeof(ETH_HEADER));
		PICMP6_PACKET icmp6p = (PICMP6_PACKET) (packet + sizeof(ETH_HEADER) + sizeof(IP6_HEADER));

		// some data for the checksum calculation

		ULONG pseudo_hdr[2];
		vec_t cksum_vec[4];

		NdisZeroMemory(&packet, sizeof(packet));
		NdisZeroMemory(&pseudo_hdr, sizeof(pseudo_hdr));
		NdisZeroMemory(&cksum_vec, sizeof(cksum_vec));

		// initialize the ethernet header - from ProcessARP
		// TapToUser.dest is generated from the tap
		// driver's MAC address and a ++
		
		COPY_MAC (eh->src, p_Adapter->m_TapToUser.dest);
		COPY_MAC (eh->dest, p_Adapter->m_TapToUser.src);
		eh->proto = htons(ETH_P_IPV6);
		
		// initialize the IP header - from rfc2460

		// stamp that magic number in

		ip6h->ip6_ctlun.ip6_un2_vfc = 0x60;

		// get the source address (from the icmp6 target field)

		memcpy (ip6h->ip6_src.__u6_addr.__u6_addr32,
			picmp6p_source->m_Target_Address.__u6_addr.__u6_addr32, sizeof(IP6ADDR));

		// copy destination to my header (from the incoming IP packet source address)
		memcpy (ip6h->ip6_dst.__u6_addr.__u6_addr32,
			pip6h_source->ip6_src.__u6_addr.__u6_addr32, sizeof(IP6ADDR));

		// initialize the payload length and hop limit and next header

		ip6h->ip6_ctlun.ip6_un1.ip6_un1_plen = htons (0x20);		// this is constant. we only forge icmp6 NA.
		ip6h->ip6_ctlun.ip6_un1.ip6_un1_nxt = IP_ICMP6_TYPE;
		ip6h->ip6_ctlun.ip6_un1.ip6_un1_hlim = 0xff;
		
		// initialize the ICMP6 NA packet data from rfc2461

		icmp6p->m_Type = ICMP6_NA;	// type is advertisement
		icmp6p->m_Code = 0;			// zero
		...
		*/
		...
		icmp6p->m_Reserved = htonl(64<<24);		// we need to set the S flag in here
		...
		memcpy (icmp6p->m_Target_Address.__u6_addr.__u6_addr32,
			picmp6p_source->m_Target_Address.__u6_addr.__u6_addr32, sizeof (IP6ADDR));

		// options (fake link layer address), use the same as our ethernet proxy adress

		icmp6p->m_Options_Type = ICMP6_OPTION_TARGET_LL_ADDRESS;
		icmp6p->m_Options_Length = 0x01;	// fixed to 1 (inc of 8), macaddr + type + length bytes = 8
		memcpy (icmp6p->m_Options_Payload, p_Adapter->m_TapToUser.dest, sizeof (MACADDR));

		// get the checksum
		// this is not correctly explained in the rfc, or I didnt get it.
		// this comes from BSD 4.4-lite.

		cksum_vec[0].ptr = (UCHAR *) &(ip6h->ip6_src);
	    cksum_vec[0].len = sizeof (ip6h->ip6_src);
	    cksum_vec[1].ptr = (UCHAR *) &(ip6h->ip6_dst);
	    cksum_vec[1].len = sizeof (ip6h->ip6_dst);

		pseudo_hdr[0] = htonl(0x20);
		pseudo_hdr[1] = htonl(IP_ICMP6_TYPE);
		cksum_vec[2].ptr = (UCHAR *) &pseudo_hdr;
		cksum_vec[2].len = sizeof (pseudo_hdr);

	    cksum_vec[3].ptr = (UCHAR *) icmp6p;
		cksum_vec[3].len = sizeof (ICMP6_PACKET);

		icmp6p->m_Checksum = in_cksum(cksum_vec, 4);
	  ...
	  //------------------------------------------------------------
			NdisMEthIndicateReceive(
				...
				ETHERNET_HEADER_SIZE,
				((unsigned char *) &packet) + ETHERNET_HEADER_SIZE,
				sizeof (packet) - ETHERNET_HEADER_SIZE,
				sizeof (packet) - ETHERNET_HEADER_SIZE);
		...
		__except (EXCEPTION_EXECUTE_HANDLER)
		{
			DEBUGP (("[%s] NdisMEthIndicateReceive failed in ProcessNS\n",
				p_Adapter->m_Name));
			NOTE_ERROR (p_Adapter);
		}
	}
