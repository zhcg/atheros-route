...
#include "phone_event_wrapper.h"
#include "phone_sleep.h"
//#include "phone_trace.h"
...
bool ThreadPosix::Start(unsigned int& thread_id)
{
  ...
  if (kEventSignaled != event_->Wait(WEBRTC_EVENT_10_SEC)) {
    //cout << "posix thread event never triggered" << endl;
    PLOG(LOG_ERROR,"posix thread event never triggered");
  ...
  if ((min_prio == EINVAL) || (max_prio == EINVAL)) {
    //cout << "unable to retreive min or max priority for threads" << endl;
    PLOG(LOG_ERROR,"unable to retreive min or max priority for threads");
    return true;
  }
  ...
  if (result == EINVAL) {
    //cout << "unable to set thread priority" << endl;
    PLOG(LOG_ERROR,"unable to set thread priority");
  }
  return true;
}

#define WEBRTC_MIPSEL_FOR_PHONE_

// CPU_ZERO and CPU_SET are not available in NDK r7, so disable
// SetAffinity on Android for now.
#if (defined(WEBRTC_LINUX) && (!defined(WEBRTC_ANDROID)) && (!defined(WEBRTC_MIPSEL_FOR_PHONE_)) )
...
void ThreadPosix::Run() {
  ...
  if (set_thread_name_) {
#if ( defined(WEBRTC_LINUX) && (!defined(WEBRTC_MIPSEL_FOR_PHONE_)) )
    prctl(PR_SET_NAME, (unsigned long)name_, 0, 0, 0);
#endif
    //cout << "Thread with name: "<< name_ << " started " << endl;
    PLOG(LOG_INFO,"Thread with name: %s started ",name_);
  } else {
    //cout << "Thread without name started" << endl;
    PLOG(LOG_INFO,"Thread without name started" );
  }
  bool alive = true;
  bool run = true;
  while (alive) {
  ...
  if (set_thread_name_) {
    ...
    if (strcmp(name_, "Trace")) {
      //cout << "Thread with name: " << name_ << " stopped" << endl;
      PLOG(LOG_INFO,"Thread with name: %s stopped",name_);
    }
  } else {
    //cout << "Thread without name stopped" << endl;
    PLOG(LOG_INFO,"Thread without name stopped");
  }
  {
    CriticalSectionScoped cs(crit_state_);
    dead_ = true;
