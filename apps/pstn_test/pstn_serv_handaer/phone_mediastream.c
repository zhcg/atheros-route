...
int main(int argc, char * argv[])
{
	...
	args = init_default_args();//init and malloc

	if (!parse_args(argc, argv, args))
		return 0;

	//else if (strcmp(argv[i],"--eq")==0){ out->eq=TRUE; }
	...
	for (i=1;i<argc;i++){
		if (strcmp(argv[i],"--local")==0){
			...
			if (isdigit(argv[i][0])){
				...
				out->payload=114;//dynamic
				out->custom_pt=parse_custom_payload(argv[i]);//parse and create new payload type
			}
...
void setup_media_streams(MediastreamDatas* args) {
	...
	//add by chen.chunsheng
	rtp_profile_set_payload(&av_profile,0,&payload_type_pcmu8000);
	rtp_profile_set_payload(&av_profile,3,&payload_type_gsm);
	rtp_profile_set_payload(&av_profile,8,&payload_type_pcma8000);
	...
	rtp_profile_set_payload(&av_profile,116,&payload_type_silk_nb);
...
//   set and add new payload type

	args->q=ortp_ev_queue_new(); //new event queue with mutex

	ms_init();//load plugins
	...
	if (args->pt->type!=PAYLOAD_VIDEO){
		...
		//new duplex rtpsession 
		args->audio=audio_stream_new(args->localport,ms_is_ipv6(args->ip));

	    // rtp_session_set_sockets(args->audio->session, int rtpfd, int rtcpfd);
	 //    session->rtp.socket=rtpfd;
		// session->rtcp.socket=rtcpfd;
	    // fcntl (args->audio->session->rtp.socket, F_SETFL, O_NONBLOCK);
	    // fcntl (args->audio->session->rtcp.socket, F_SETFL, O_NONBLOCK);
...
void run_non_interactive_loop(MediastreamDatas* args) {
	...
	while ( args->stat_cond ) //native global variable
	{
		int n;
		for(n=0;n<100;++n){
#ifdef WIN32
...
static bool_t parse_addr(const char *addr, char *ip, int len, int *port)
{
	...
	*port=0;
	semicolon=strchr(addr,':');// return the first ':' 's addr ;if false , return NULL
...
static PayloadType* create_custom_payload_type(const char *type, const char *subtype, const char *rate, int number){
	PayloadType *pt=payload_type_new(); //allocate new type
	...
	pt->mime_type=ms_strdup(subtype);//malloc  "pcmu" etc.
...
//add by chen.chunsheng
MediastreamDatas* mediastream_init(int argc, char * argv[]){
	MediastreamDatas* args;
	//cond = 1;
...
int mediastream_uninit(MediastreamDatas* args){
	//cond = 0;
	args->audio_running = 0;
...
#if 0
pthread_t run_loop_tid;
static int audio_running = 0; 
#endif

int mediastream_begin(MediastreamDatas* args){
	
	
	if(args->audio_running == 1){
		ms_warning("Audio is running.\n");
		return 0;
	}
	args->audio_running = 1;
	...
	if (args->eq){		
		if((pthread_create(&args->stat_tid, NULL, ( void * (*)(void *))&run_interactive_loop, (void *)args)) == -1){
				perror("pthread_create run_interactive_loop error");
				return -1;
		}
		
	} else {
		#if 1
		ms_message("%s:%d -- %d\n", __FUNCTION__, __LINE__, (int)args->stat_tid);
		if((pthread_create(&args->stat_tid, NULL, ( void * (*)(void *))&run_non_interactive_loop, (void *)args)) == -1){
				perror("pthread_create run_non_interactive_loop error");
				return -1;
		}
		ms_message("%s:%d -- %d\n", __FUNCTION__, __LINE__, (int)args->stat_tid);
		#endif
...
int mediastream_end(MediastreamDatas* args){
	if (args->audio_running == 0){
		ms_warning("Audio is not running.\n");
		return 0;
	}
	args->audio_running = 0;
	...
	if ( args->stat_tid ){
		ms_message("%s:%d -- %d\n", __FUNCTION__, __LINE__, (int)args->stat_tid);
		pthread_join(args->stat_tid, NULL);
		ms_message("%s:%d -- %d\n", __FUNCTION__, __LINE__, (int)args->stat_tid);
	}
	clear_mediastreams(args);
...
#ifdef __TESTMEDIASTREAM
int main(int argc, char * argv[])
{

	MediastreamDatas* args;
	
	args = mediastream_init(argc, argv);
	if (args == NULL) return -1;
	
	mediastream_begin(args);

	sleep(20);

	mediastream_end(args);
	
	mediastream_uninit(args);	
