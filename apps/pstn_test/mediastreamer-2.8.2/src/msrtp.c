...
#ifdef __MY_DEBUG__
	#define MY_LOG(x,...) printf(x,##__VA_ARGS__)
#else
	#define MY_LOG(x)
#endif
...
static void receiver_process(MSFilter * f)
{
	...
	//timestamp = (uint32_t) (9 * (d->rate/1000));
	//MY_LOG("timestamp = %d\n", timestamp);
	while ((m = rtp_session_recvm_with_ts(d->session, timestamp)) != NULL) {
		...
		payload_size = rtp_get_payload(m,&m->b_rptr);
		total_size += payload_size;
		total_receive += payload_size;
		ms_queue_put(f->outputs[0], m); 
#if 0
		MY_LOG("total = %d, payload_size = %d\n", 
					total_size, payload_size);
#endif
	}
}
