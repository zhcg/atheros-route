	...
	MSSndCard *playcard, MSSndCard *captcard, bool_t use_ec)
{
	...
	tmp = PHONE_PATH_CALLING;
	ms_filter_call_method(stream->soundwrite,MS_FILTER_SET_SPEECH_CHANNEL, &tmp);
...
RingStream * ring_start_with_cb(const char *file,int interval,MSSndCard *sndcard, MSFilterNotifyFunc func,void * user_data)
{
	...
	tmp = PHONE_PATH_RINGING;
	ms_filter_call_method(stream->sndwrite,MS_FILTER_SET_SPEECH_CHANNEL,&tmp);
