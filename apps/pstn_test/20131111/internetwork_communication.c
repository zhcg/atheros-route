 ...
 * 初始化结构体
 */
struct class_internetwork_communication internetwork_communication = 
{
...
int unpack_pack_send_data(struct s_data_list *a_data_list, char a_pack_type, void *a_par)
{
    ...
    if (a_pack_type == 1)  // 来电显示 0x80 或者 0x04
    {
        switch (common_tools.deal_attr->deal_type)
        {
            ...
            { 
                ...
                if ((res = communication_stc.cmd_off_hook_respond(off_hook_respond)) < 0)
                {            
    ...
    #endif
    {
        ...
        #else
        communication_stc.cmd = 0x76;
        #endif
        
        if ((res = communication_stc.send_data(NULL, a_data_list, a_data_list->list_len, NULL)) < 0)
        {
