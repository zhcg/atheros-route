 ...
 * pad串口监听  
 */
void * pad_serial_listen(void* para) 
{
    int i = 0;
    int res = 0;
    ...
    {   
        // 接收信号
        if (network_config.pthread_flag == 0)
        {
            sleep(2);
            continue;
        }
            ...
            {                
                if (FD_ISSET(*network_config.serial_pad_fd, &fdset) > 0)
                {
                    ...
                    for (i = 0; i < common_tools.config->repeat; i++)
                    {
                        ...
                        // 数据发送到PAD                            
                        if ((res = network_config.send_msg_to_pad(*network_config.serial_pad_fd, cmd, NULL)) < 0)
                        {
            ...
            default:
            {
                ...
                {                    
                    ...
                    switch (pad_and_6410_msg.cmd)
                    {
                        ...
                        #endif
                        case 0x52:   //0x52: 查看WAN口
                        case 0x53:   //0x53: 获取设备令牌 和 获取sip服务器信息
                        case 0x54:   //0x54: 获取sip服务器信息
                        {
                            cmd = pad_and_6410_msg.cmd;
                            break;
                        }
...
{    
    ...
    int res = 0;
    pthread_t pthread_pad_serial_id, pthread_pad_socket_id, pthread_pad_serial_listen_id; 
    ...
    */
    ...
    pthread_create(&pthread_pad_socket_id, NULL, (void*)pad_socket, NULL); 
    PRINT("pthread_create (pad_socket) success!\n");
    pthread_create(&pthread_pad_serial_id, NULL, (void*)pad_serial, NULL);
    PRINT("pthread_create (pad_serial) success!\n");
    pthread_create(&pthread_pad_serial_listen_id, NULL, (void*)pad_serial_listen, NULL);
    PRINT("pthread_create (pad_serial_listen) success!\n");

    pthread_join(pthread_pad_socket_id, NULL);
    pthread_join(pthread_pad_serial_id, NULL);
    pthread_join(pthread_pad_serial_listen_id, NULL);
