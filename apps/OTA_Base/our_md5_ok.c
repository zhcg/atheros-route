...
int CalcFileMD5(char *filename, char *md5_sum)
{
    ...
    memset(x, 0, 64);
    read_num = fread(&x, 4, 16, fp);


	for(num = 0; num < read_num; num++)
        {
                shift[0] = x[num] & 0xFF;
                shift[1] = ( x[num] >> 8 ) & 0xFF;
                shift[2] = ( x[num] >> 16 )& 0xFF;
                shift[3] = ( x[num] >> 24 )& 0xFF;
                x[num] = (shift[0] << 24) | (shift[1] << 16) | (shift[2] << 8) | shift[3];
        }
	...
	place = ((len%64)/4);
	place_remainder = ((len%64)%4);
	
	printf("place = %d\n", place);
	printf("place_remainder = %d\n", place_remainder);
	//x[place] = (128 >> (8*(3 - place_remainder)));

	x[place] = x[place] | (128 << (8*place_remainder));
...
//	x[len % 64] = x[len % 64] | 0x80000000;
//	x[num] = x[num] | 0x80000000;
