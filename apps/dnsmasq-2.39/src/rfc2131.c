...
#define IPLIST_PATH             "/tmp/ip_forward_list"
#define IP_CONTROL              "/etc/ipcontrol/ip_control"
#define REDIRECT_IPLIST_PATH    "/tmp/redirect_iplist"
#define PORTAL_FILE             "/tmp/portal_file"
#define COMPUTER_NETID          "Computer"
#define NETWORK2                "192.168.2."
#include "flock.h"
extern int arp_query(char *ipaddr, char *iface);

/* Note:
 * the vendorclass id and net id are set the same.
 */
struct dhcp_context *locate_context(
        struct daemon *daemon,
        char *netid) 
{
    struct dhcp_context *pool_context = NULL;

    my_syslog(LOG_INFO, _("Enter %s"),__func__);

    if(netid == NULL)
        return daemon->context;
    for(pool_context = daemon->dhcp; 
            pool_context; 
            pool_context = pool_context->next) {
        if(pool_context->filter && 
                !strcmp(netid, pool_context->filter->net))
            return pool_context;
    }
    return daemon->context;
}

void update_redirect_iplist(char *netid, struct in_addr addr)
{
    int pfd;
    char *nid;
    char portal_info[1024] = {};

    my_syslog(LOG_INFO, _("Enter %s"),__func__);

    if(access(PORTAL_FILE, R_OK) || (pfd = open(PORTAL_FILE, O_RDONLY)) < 0) {
        my_syslog(LOG_INFO, _("open %s failed"), PORTAL_FILE);
        return;
    }

    if(read(pfd, portal_info, sizeof portal_info) <= 0) {
        my_syslog(LOG_INFO, _("No portal information"));
        close(pfd);
        return;
    }
    close(pfd);

    if(!(nid = strstr(portal_info, netid ? netid : COMPUTER_NETID))) {
        my_syslog(LOG_INFO, _("Needn't do portal"));
        return;
    }

        /* Add by bowenguo, for update redirect file dynamically, 2009-06-15 */
#define LINE_LEN 256
    FILE * fp = NULL;
    FILE * fptmp = NULL;
    ...
    char line[LINE_LEN];
    char value[LINE_LEN];
    char tmp[FILENAME_MAX/4];

    if (access(REDIRECT_IPLIST_PATH, F_OK) == -1)
    {
        if((fp = fopen(REDIRECT_IPLIST_PATH, "w+")) == NULL)
            goto out;
        close(fp);
    }
        
    if((fp = fopen(REDIRECT_IPLIST_PATH, "r")) == NULL)
    {
        my_syslog(LOG_INFO, _("Open file %s error: %s"), REDIRECT_IPLIST_PATH,
                  strerror(errno));
        goto out;
    }
    
    sprintf(tmp, "%s.tmp", REDIRECT_IPLIST_PATH);
    if((fptmp = fopen(tmp, "w+")) == NULL)
    {
        my_syslog(LOG_INFO, _("Open file %s error: %s"), tmp,
                  strerror(errno));
        goto out;
    }
    ...
    {
        if(strstr(line, inet_ntoa(addr)) == NULL)
        {
            fprintf(fptmp, "%s", line);
        }
        my_syslog(LOG_INFO, _("line = %s"), line);
    }

    strsep(&nid, ":");
    if(nid == NULL) {
        my_syslog(LOG_INFO, _("strsep failed %s"), nid);
        goto out;
    }

    nid[strlen(nid) - 1] = '\0';
    
    fprintf(fptmp,"%s:%s:%s\n", inet_ntoa(addr), netid, nid);

    fclose(fp);
    fclose(fptmp);
    if(rename(tmp, REDIRECT_IPLIST_PATH) == -1)
    {
        my_syslog(LOG_INFO, _("Rename file error: %s"), strerror(errno));
        return ;
    }
    return ;
  ...
  out:
...
#if 0
    int rfd;
    char redirect_info[1024] = {};
    if((rfd = open(REDIRECT_IPLIST_PATH, O_RDWR | O_CREAT | O_APPEND)) < 0)
        return;

    if(read(rfd, redirect_info, sizeof redirect_info) < 0) {
        my_syslog(LOG_INFO, _("read redirect info failed"));
        goto out;
    }
    if(strstr(redirect_info, inet_ntoa(addr))) {
        my_syslog(LOG_INFO, _("The client has already be redirected"));
        goto out;
    }

    my_syslog(LOG_INFO, _("Vendor information %s"), nid);
    strsep(&nid, ":");
    if(nid == NULL) {
        my_syslog(LOG_INFO, _("strsep failed %s"), nid);
        goto out;
    ...
    *end = '\0';
    ...
    if(write_lock(rfd, 0, SEEK_SET, 0) < 0)
        goto out;

    snprintf(redirect_info, sizeof redirect_info, 
            "%s:%s:%s\n", inet_ntoa(addr), netid, portal_url);
    lseek(rfd, 0L, SEEK_END);
    write(rfd, redirect_info, strlen(redirect_info));
    write_unlock(rfd, 0, SEEK_SET, 0);
out:
    close(rfd);
    return;
#endif
...
void add_ip(struct ip_list **list, struct in_addr addr, int *ip_count)
{
    struct ip_list *last = *list;
    struct ip_list *new = NULL;
...
void remove_ip(struct ip_list **list, struct in_addr addr, int *ip_count)
{
    struct ip_list *prev_ptr = *list;
    struct ip_list *ptr = *list;
    ...
    while(ptr) {
        if(ptr->addr.s_addr == addr.s_addr) {
            ...
            ptr->next = NULL;
            free(ptr);
            *ip_count = ((*ip_count <= 0) ? 0 : (*ip_count)--);  
            return;
        }
...
void iptable_flush(void)
{
    ...
    if((pid = vfork()) == -1)
        ...
        char *argv[] = {IP_CONTROL, "flush", NULL}; 
        execvp(IP_CONTROL, argv);
        exit(0);
    } 
}

void iptable_enable_all(void)
{
    int pid;
    my_syslog(LOG_INFO, _("Enter %s"),__func__);
    if((pid = vfork()) == -1)
        ...
        char *argv[] = {IP_CONTROL, "enableall", NULL}; 
        execvp(IP_CONTROL, argv);
        exit(0);
    } 
}
...
void iptable_update(void)
{
    ...
    if((pid = vfork()) == -1)
        ...
        char *argv[] = {IP_CONTROL, "-c", IPLIST_PATH, "update", NULL}; 
        execvp(IP_CONTROL, argv);
        exit(0);
    } 
}
...
void update_iptable_rules(struct daemon *daemon, char *iface_name)
{
    FILE *fd;
    char pathname[64];
    struct ip_list *lp = NULL;
    struct dhcp_context *pool_context = NULL;

    my_syslog(LOG_INFO, _("Enter %s"),__func__);
    if(daemon == NULL || !daemon->need_iptable || iface_name == NULL)
        return;

    snprintf(pathname, sizeof pathname, IPLIST_PATH);
    if((fd = fopen(pathname, "w")) == NULL)
        return;
        
    fprintf(fd, "#Automatically generated, don't modify it.\n");
    fprintf(fd, "#The following ip list can access internet.\n");
    for(pool_context = daemon->dhcp; pool_context; 
        pool_context = pool_context->next) {
        for(lp = pool_context->ipl; lp; lp = lp->next) {
#if 0
            if(addr.s_addr != lp->addr.s_addr && 
                    arp_query(inet_ntoa(lp->addr), iface_name) != 0) {
                remove_ip(&pool_context->ipl, lp->addr, &pool_context->ip_count);                   
                continue;
            }
#endif
           fprintf(fd, "%s  #%s\n",inet_ntoa(lp->addr), 
                   pool_context->filter ? pool_context->filter->net : "null");
        }
    }
    fclose(fd);
    iptable_flush();
...
int is_new_ip(struct dhcp_context *root, struct in_addr addr)
{
    while(root) {
        struct ip_list *ipl = root->ipl;
        while(ipl) {
            if(ipl->addr.s_addr == addr.s_addr)
                return 1;
            ipl = ipl->next;
        }
        root = root->next;
...
int get_total(struct dhcp_context *root)
{
    int count = 0;
...
int scan_context(struct dhcp_context *dc, char *iface_name)
{
    int ret = 0;
    struct ip_list *ipl  = dc->ipl;

    while(ipl) {
        if(arp_query(inet_ntoa(ipl->addr), iface_name) != 0) {
            my_syslog(LOG_INFO, _("Client %s had declined!"), inet_ntoa(ipl->addr));
            remove_ip(&dc->ipl, ipl->addr, &dc->ip_count);                   
            ret = 1;
        }
        ipl = ipl->next;
    }
...
int scan_all_context(struct dhcp_context *root, char *iface_name)
{
    int ret = 0;
    
    while(root) {
        if(scan_context(root, iface_name))
            ret = 1;
        root = root->next;
    }
    return ret;
...
void init_ip_list(struct daemon *daemon)
{
    my_syslog(LOG_INFO, _("Enter %s"),__func__);
    if(daemon && daemon->need_iptable && !access(IPLIST_PATH, R_OK)) {
        struct in_addr addr;
        char line[256];
        char ip[32];
        char netid[64];
        FILE *fp = fopen(IPLIST_PATH, "r");
        struct dhcp_context *dc;

        if(fp == NULL)
            return;

        while((fgets(line, sizeof line, fp)) != NULL) {
            my_syslog(LOG_INFO, _("Get line %s"), line);

            if(line[0] == '#')
                continue;

            if(sscanf(line, "%s    #%s", ip, netid) != 2)
                continue;
            my_syslog(LOG_INFO, _("%s   #%s"), ip, netid);
            dc = locate_context(daemon, netid);  
            my_syslog(LOG_INFO, _("limit number is %d count is %d"), dc->limit_num, dc->ip_count);
            if(!inet_aton(ip, &addr))
                continue;
            if(dc->limit_num == 255 ||  dc->ip_count < dc->limit_num) {
                add_ip(&dc->ipl, addr, &dc->ip_count);
                update_redirect_iplist(netid, addr);
            }
        }
        fclose(fp);
    }
    update_iptable_rules(daemon, "br0");
}

int is_network2(struct in_addr addr)
{
    const char *network2 = NETWORK2;
    const char *network = inet_ntoa(addr);

    if(strstr(network, network2)) return 1;
    return 0;
}

void __update_redirect_iplist(
        struct dhcp_netid *filter,
        struct in_addr addr)
{
    if(filter)
        update_redirect_iplist(filter->net, addr);
    else
        update_redirect_iplist(NULL, addr);
}

void ip_pool_manage(struct daemon *daemon, struct dhcp_lease *lease, char *iface_name)
{
    struct dhcp_context *pool_context = NULL;

    my_syslog(LOG_INFO, _("Enter %s"),__func__);
    if(daemon == NULL || lease == NULL || 
            daemon->dhcp == NULL || iface_name == NULL)
            goto out;

    if(daemon->limits_mode && daemon->need_iptable && !is_network2(lease->addr)) {
        my_syslog(LOG_INFO, _("The total number of devices is limited."));
        if(!daemon->context->limit_num)
            goto out;

        /* The count of lease which can access internet haven't reach the max.
        * So add this lease to the list and update the iptable rules.
        */
        pool_context = locate_context(daemon, lease->vendorclass);
        my_syslog(LOG_INFO, _("Current pool:%s %s -- %s\n"),
            pool_context->filter ? pool_context->filter->net : "null",
            inet_ntoa(pool_context->start), inet_ntoa(pool_context->end));

        if(is_new_ip(daemon->dhcp, lease->addr)) {
            my_syslog(LOG_INFO, _("The ip %s is already in the list!"),
                    inet_ntoa(lease->addr));
            __update_redirect_iplist(pool_context->filter, lease->addr);
            goto out;   
        }

        if(get_total(daemon->dhcp) >= daemon->context->limit_num) {
           if(!scan_all_context(daemon->dhcp, iface_name))
               goto out;    /* ip pool is full and no client decline, so do nothing */
        }

        add_ip(&pool_context->ipl, lease->addr, &pool_context->ip_count);
        __update_redirect_iplist(pool_context->filter, lease->addr);
    } else if(daemon->need_iptable && !is_network2(lease->addr)) {
        my_syslog(LOG_INFO, _("the number of device xxx is limited"));

        pool_context = locate_context(daemon, lease->vendorclass);
        if(pool_context == daemon->context) {
            pool_context = locate_context(daemon, COMPUTER_NETID);
        }
        ...
        if(pool_context->limit_num != 255) {
            if(pool_context->ip_count >= pool_context->limit_num) 
                if(!scan_context(pool_context, iface_name))
                    goto out;   /* ip pool is full and no client decline */
        }
    ...
    update_iptable_rules(daemon, iface_name);
out:
    return;
}
#endif
      ...
      if (!message)
	{
	  if (have_config(config, CONFIG_ADDR))
	  ...
	  else
	    {
		  ...
		  !address_available(context, lease->addr,NULL))
		{
		   if (lease)
		     {
  ...
  if ((opt = option_find(mess, sz, OPTION_REQUESTED_OPTIONS, 0)))
    {
      ...
      /* Add by bowenguo, for CTC captive portal, 2009-03-10 */
#ifdef CAPTIVEPORTAL
  static unsigned char emac_old[MAC_MAX_LEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
#endif
  ...
  switch (mess_type)
	...
	return 0;
      ...
      if (lease && lease->addr.s_addr == mess->ciaddr.s_addr) {
            if(daemon->need_iptable && !is_network2(lease->addr)) { 
                struct dhcp_context *pool_context;
                if((pool_context = locate_context(daemon, lease->vendorclass)) == NULL)
                    pool_context = daemon->context;
                remove_ip(&pool_context->ipl, lease->addr, &pool_context->ip_count);
                update_iptable_rules(daemon, iface_name);
            }
	      ...
	      else
		{
		  ...
		  if (tmp)
		    my_syslog(LOG_WARNING, _("not using configured address %s because it is in use by the server or relay"), addrs);
			   ...
			   difftime(now, config->decline_time) < DECLINE_BACKOFF)
#else
		  else if (have_config(config, CONFIG_DECLINED) &&
...
#ifdef ATHEROS_EXTEND
      ip_pool_manage(daemon, lease, iface_name);
#endif
      
          /* Add by bowenguo, for CTC captive portal, 2009-03-10 */
#ifdef CAPTIVEPORTAL
      captiveportal(iface_name, inet_ntoa(mess->yiaddr));
#endif
...
#ifdef HAVE_NO_FPU
  else if (lease && lease->expires != 0 && difftime(lease->expires, now) > 0)
#else
