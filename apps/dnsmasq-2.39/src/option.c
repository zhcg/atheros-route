...
{      
  ...
  switch (option)
	...
	char *cp, *a[6] = { NULL, NULL, NULL, NULL, NULL, NULL };
#else
...
#ifdef ATHEROS_EXTEND
    if(k >= leasepos+2) {
        new->limit_num = atoi(a[leasepos+1]);
        if(new->limit_num != 255)
            daemon->need_iptable = 1;

        if(new->filter == NULL) {
            daemon->context = new;
            if(daemon->need_iptable)
                daemon->limits_mode = 1;
        }
        new->ipl = NULL;
    }
#endif
