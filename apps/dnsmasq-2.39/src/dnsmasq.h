...
#ifdef HAVE_NO_FPU
#define difftime(t1,t0) (long)(t1 - t0)
#endif
...
#ifdef ATHEROS_EXTEND
  int limit_num;        /* The number of ip which has no limitation. */
  int ip_count;         /* The count of ip in ipl list. */
  struct ip_list *ipl;  /* The ip in this list are allowed to access the internet. */
#endif
};
...
struct daemon {
  ...
  int need_iptable;
  int limits_mode;
  struct dhcp_context *context;     /* Context repsent the whole range. */
#endif
};
...
struct dhcp_context *address_available(struct dhcp_context *context, struct in_addr addr,struct dhcp_netid *netids);
