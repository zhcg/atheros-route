...
extern void init_ip_list(struct daemon *daemon);
#endif
...
int main (int argc, char **argv)
{
  ...
  if((fp = popen("iptables -t nat -L "CHAIN_NAME, "r")) != NULL)
  {
      ...
      {
          if(strstr(line, CHAIN_NAME) != NULL)
              goto end;
      }
          /* create New chain */
      system("iptables -t nat -N "CHAIN_NAME); /* create new chains */
          /* All 80 tcp packages jump to CHAIN_NAME */
      system("iptables -t nat -A PREROUTING -p tcp --dport 80 -j "CHAIN_NAME);
...
#ifdef HAVE_NO_FPU
      if (last == 0 || difftime(now, last) > 1 || difftime(now, last) < -1)
#else
...
#ifdef HAVE_NO_FPU
			if (difftime(statbuf.st_mtime, last_change) > 0)
#else
