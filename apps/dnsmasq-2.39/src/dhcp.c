...
#if 0
	  my_syslog(LOG_INFO, _("IP RANGE start %s"), inet_ntoa(context->start));
	  my_syslog(LOG_INFO, _("IP RANGE end %s"), inet_ntoa(context->end));
      if(context->filter)
	  my_syslog(LOG_INFO, _("IP RANGE network id %s"), context->filter->net);
#ifdef ATHEROS_EXTEND
	  my_syslog(LOG_INFO, _("IP RANGE access limits %d"), context->limit_num);
#endif
#endif
...
#if 0
    if(ifr.ifr_name)
	  my_syslog(LOG_INFO, _("Interface names %s"), ifr.ifr_name);
    if(daemon->context) {
	  my_syslog(LOG_INFO, _("TOTAL RANGE start %s"), inet_ntoa(daemon->context->start));
	  my_syslog(LOG_INFO, _("TOTAL RANGE end %s"), inet_ntoa(daemon->context->end));
    }
#endif  
  ...
  for (tmp = context; tmp; tmp = tmp->current)
    {
		  ...
		  && addr >= start &&   addr <= end)
	  {
		  if(NULL != netids)
		  {
			  if(match_netid(tmp->filter, netids, 0))
			  {
				  return tmp;
			  }
		  }
...
struct dhcp_context *narrow_context(struct dhcp_context *context, struct in_addr taddr)
{
  ...
  if ((tmp = address_available(context, taddr,NULL)))
    return tmp;
...
#ifdef HAVE_NO_FPU
		  if (difftime(now, r->time) >  PING_CACHE_TIME)
#else
