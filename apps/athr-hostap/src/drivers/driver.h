...
struct wpa_driver_ops {
	 ...
	 * @resp: association response - success or failure
	 ...
	 */
	 int (*sta_assoc)(void *priv, const u8 *own_addr, const u8 *addr,
			  int reassoc, u16 resp, const u8 *ie, size_t len);
	 ...
	 * @reason: authentication response reason code
	 ...
	 */
	 int (*sta_auth)(void *priv, const u8 *own_addr, const u8 *addr,
			 u16 seq, u16 reason, const u8 *ie, size_t len);
	 ...
	 * @buf: Return buffer
	 * @buf_len: Maximum buffer length for return data
	 * Returns: Number of characters written into return buffer on success,
	 *	of -1 on failure
	 *
	 * This optional command is for testing purposes. It can be used to run
	 * driver-specific operations through the control interface.
	 */
	int (*driver_cmd)(void *priv, const char *cmd, char *buf,
			  size_t buf_len);
