...
atheros_set_ieee8021x(void *priv, struct wpa_bss_params *params)
{
	...
	if (!params->enabled) {
		if((params->auth_algs==1  || params->auth_algs==3)
                && params->keys_set==0) {
            set80211param(priv, IEEE80211_PARAM_AUTHMODE,
                    IEEE80211_AUTH_OPEN);
                   return set80211param(priv, IEEE80211_PARAM_PRIVACY, 0);
		}else if(params->auth_algs==1 && params->keys_set){
			set80211param(priv, IEEE80211_PARAM_AUTHMODE,
					IEEE80211_AUTH_OPEN);
			/*Fix for open wep  when run using hostapd*/
			return set80211param(priv, IEEE80211_PARAM_PRIVACY, 1);
		}else if(params->auth_algs==2) {
			set80211param(priv, IEEE80211_PARAM_AUTHMODE,
				 ...
				 size_t len)
{
	...
	wpa_printf(MSG_DEBUG, "%s: subtype 0x%x len %d", __func__, stype, len);
...
atheros_sta_auth(void *priv, const u8 *own_addr, const u8 *addr, u16 seq,
		 u16 reason_code, const u8 *ie, size_t len)
{
	struct atheros_driver_data *drv = priv;
	struct ieee80211req_mlme mlme;
	int ret;
	...
	if (len) {
		if (len < IEEE80211_MAX_OPT_IE) {
			...
			wpa_printf(MSG_DEBUG, "%s: Not enough space to copy "
				   ...
				   "ie_len %d )",
				   __func__, MAC2STR(addr), reason_code, len);
			return -1;
		}
	}
	ret = set80211priv(drv, IEEE80211_IOCTL_SETMLME, &mlme, sizeof(mlme));
...
atheros_sta_assoc(void *priv, const u8 *own_addr, const u8 *addr,
		  int reassoc, u16 reason_code, const u8 *ie, size_t len)
{
	struct atheros_driver_data *drv = priv;
	struct ieee80211req_mlme mlme;
	int ret;

	wpa_printf(MSG_DEBUG, "%s: addr=%s reason_code=%d reassoc %d",
		   __func__, ether_sprintf(addr), reason_code, reassoc);
...
atheros_get_ssid(void *priv, u8 *buf, int len)
{
	...
	os_strlcpy(iwr.ifr_name, drv->iface, IFNAMSIZ);
	iwr.u.essid.pointer = (caddr_t) buf;
	iwr.u.essid.length = (len > IEEE80211_MAX_SSID_LEN) ? IEEE80211_MAX_SSID_LEN : len;
