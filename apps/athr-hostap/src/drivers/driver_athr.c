	...
	while (1) {
		...
		if (frame_type == IEEE80211_EV_RX_MGMT) {
			...
			if (stype == WLAN_FC_STYPE_ACTION) {
				...
				event.rx_action.category =
					mgmt->u.action.category;
				event.rx_action.data =
					&mgmt->u.action.category + 1;
				event.rx_action.len = ((u8 *) mgmt) +
					data_len - event.rx_action.data;
				event.rx_action.freq = freq;
			 ...
			 struct wpa_driver_associate_params *params)
{
	...
	if (params->bssid == NULL) {
	    ...
	    if (driver_atheros_set_ssid(drv, params->ssid,
					 params->ssid_len) < 0)
		ret = -1;
	} else {
	    if (set80211param(drv, IEEE80211_PARAM_ROAMING, 2, 1) < 0)
	...
	*flags = 0;
	...
	for (i = 0; i < chans.ic_nchans; i++) {
		...
		if (chan->ic_flags & IEEE80211_CHAN_PASSIVE)
			nc->flag |= HOSTAPD_CHAN_PASSIVE_SCAN;
				 ...
				 const char *bridge)
{
	...
	wpa_printf(MSG_DEBUG, "atheros: Group interface %s - " MACSTR,
		   ifname, MAC2STR(if_addr));

	switch (type) {
	case WPA_IF_P2P_CLIENT:
		wpa_printf(MSG_DEBUG, " %s:Changing the mode of ifname=%s, "
			   "to IEEE80211_M_P2P_CLIENT",
			   __func__, ifname);
		return set80211param_ifname(drv, ifname,
					    IEEE80211_IOC_P2P_OPMODE,
					    IEEE80211_M_P2P_CLIENT, 1);
	case WPA_IF_P2P_GO:
		wpa_printf(MSG_DEBUG, " %s:Changing the mode of ifname=%s, "
			   "to IEEE80211_M_P2P_GO",
			   __func__, ifname);
		return set80211param_ifname(drv, ifname,
					    IEEE80211_IOC_P2P_OPMODE,
					    IEEE80211_M_P2P_GO, 1);
	case WPA_IF_P2P_GROUP:
		/* TODO: Should not need to do this(?), but some operations
		 * did not work if the interface was left as-is.. */
		wpa_printf(MSG_DEBUG, " %s:Changing the mode of ifname=%s, "
			   "to IEEE80211_M_P2P_CLIENT",
			   __func__, ifname);
		ret = set80211param_ifname(drv, ifname,
					   IEEE80211_IOC_P2P_OPMODE,
					   IEEE80211_M_P2P_CLIENT, 1);
		return ret;
	default:
...
static int driver_atheros_disable_11b_rates(void *priv, int disabled)
{
	/*
	 * The driver takes care of disabling 802.11b rates automatically for
	 * P2P vaps, so no need to do anything here.
	 */
	return 0;
}
...
static int driver_atheros_shared_freq(void *priv)
{
	struct driver_atheros_data *drv = priv;
	int val, freq;
	...
	if (get80211param(drv, drv->shared_ifname,
			  IEEE80211_IOC_CONNECTION_STATE,
			  &val) < 0) {
		wpa_printf(MSG_DEBUG, "athr: Could not fetch connection "
	...
	if (!val) {
		wpa_printf(MSG_DEBUG, "athr: Shared interface %s is "
			   "disconnected", drv->shared_ifname);
		return 0;
	}

	freq = athr_get_freq(drv, drv->shared_ifname);
	if (freq < 0)
		return -1;

	wpa_printf(MSG_DEBUG, "athr: Shared interface %s is connected on "
		   "freq %d MHz", drv->shared_ifname, freq);
...
static int driver_atheros_set_param(void *priv, const char *param)
{
	...
	if (pos) {
		...
		wpa_printf(MSG_DEBUG, "athr: Shared interface: %s",
			   drv->shared_ifname);
		driver_atheros_shared_freq(drv);
	}
				   ...
				   struct wpa_driver_associate_params *params)
{
	...
	wpa_printf(MSG_DEBUG, " %s: Set channel = %d", __func__, params->freq);

	if (driver_atheros_set_freq(drv, params->freq) < 0)
		return -1;
...
static int athr_send_tdls_msg(struct driver_atheros_data *drv, const u8 *dst,
			      u8 action_code, const u8 *buf, size_t len)
{
	u8 *data, *pos;
	int ret;
	u16 val;
			       ...
			       const u8 *buf, size_t len)
{
	...
	if (status_code == 0 &&
		...
		return athr_send_tdls_msg(drv, dst, action_code, buf, len);
...
const struct wpa_driver_ops wpa_driver_athr_ops = {
	...
	.disassociate = driver_atheros_disassociate,
	...
	.disable_11b_rates = driver_atheros_disable_11b_rates,
	...
	.shared_freq = driver_atheros_shared_freq,
	.set_p2p_powersave = athr_set_p2p_powersave,
	.set_intra_bss = athr_set_intra_bss,
	.ampdu = athr_ampdu,
