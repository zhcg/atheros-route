...
int hostapd_reload_config(struct hostapd_iface *iface)
{
	...
	{
		/* ATHR AP workaround - force full restart of hostapd */
		extern int reloading;
		wpa_printf(MSG_DEBUG, "WPS: Force restart of hostapd(2)");
		reloading = 1;
		eloop_terminate();
		return 0;
	}
...
int hostapd_setup_interface_complete(struct hostapd_iface *iface, int err)
{
     ...
     * If "interface=" and "upnp_iface=" are the same (ex. non-bridge mode),
     * the interface is up only after driver_commit, so call it after
     * driver_commit.
     */
	for (j = 0; j < iface->num_bss; j++) {
		hapd = iface->bss[j];
