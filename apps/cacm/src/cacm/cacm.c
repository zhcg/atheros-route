 ...
 * 初始化线程的运行环境
 */
static int init_thread_run_env(struct s_cacm * cacm)
{
    int res = 0;
...
static void * real_time_event(void* para)
{
    ...
    while (1)
    {
        ...
        //***********************************************************************
        // 把异常情况发送到PAD端
        if ((res = communication_network.make_client_link("10.10.10.100", "8787")) < 0)
        {
        ...
        buf[4] = 0x00;
        buf[5] = ((struct s_cacm *)para)->phone_state_type;
        buf[6] = ((struct s_cacm *)para)->phone_state_type;
        
        PRINT_BUF_BY_HEX(buf, NULL, 7, __FILE__, __FUNCTION__, __LINE__);
        
        // 发送
        if ((res = common_tools.send_data(client_fd, buf, NULL, 7, &timeout)) < 0)
        {
            PERROR("send_data faiiled!\n");
            close(client_fd);
            continue;
        }
        PRINT("send event to pad successful!");
        //***********************************************************************
        usleep(100000);
        close(client_fd);
    }
