...
/*wangyu add for ntp timezone*/
#define timezone_interval ( 60 * 60 )
int timezone_ntp=0;
...
//	xmttime.coarse = arrival->coarse;
//	xmttime.fine   = arrival->fine;
#undef Data

	if (set_clock) {   /* you'd better be root, or ntpclient will crash! */
		struct timeval tv_set;
		/* it would be even better to subtract half the slop */
		tv_set.tv_sec  = xmttime.coarse - JAN_1970 + timezone_ntp * timezone_interval;
		/* divide xmttime.fine by 4294.967296 */
		tv_set.tv_usec = USEC(xmttime.fine);
//		tv_set.tv_sec  = arrival->coarse - JAN_1970;
		/* divide xmttime.fine by 4294.967296 */
//		tv_set.tv_usec = USEC(arrival->fine);
...
/*wangyu add for ntp timezone problem*/
			udp_arrival_ntp.coarse += timezone_ntp * timezone_interval;
...
int main(int argc, char *argv[]) {
	...
	for (;;) {
		c = getopt( argc, argv, "c:" DEBUG_OPTION "g:h:t:i:lp:rs");
		if (c == EOF) break;
		switch (c) {
			case 'c':
...
//				probe_count = 1;//wangyu cannel this line
	...
	if (debug) {
		...
		live, udp_local_port, set_clock,timezone_ntp );
	}
