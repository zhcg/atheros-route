...
int send_data(unsigned char *data, struct s_data_list *a_data_list, unsigned int data_len, struct timeval *tv)
{
    ...
    //2.类型
    PRINT("[communication_serial.cmd = %02X]\n", communication_serial.cmd);
	if ((res = common_tools.send_one_byte(fd, &(communication_serial.cmd), tv)) < 0)
	{
	    close(fd);
    ...
    //5.发送校验和
    check_sum ^= communication_serial.cmd;
    check_sum ^= list_len;
...
int send_data(unsigned char *data, struct s_data_list *a_data_list, unsigned int data_len, struct timeval *tv)
{
    ...
    //2.类型
	if ((res = common_tools.list_tail_add_data(&data_list, communication_serial.cmd)) < 0)
	{
	    close(fd);
	    common_tools.list_free(&data_list); 
        OPERATION_LOG(__FILE__, __FUNCTION__, __LINE__, "list_tail_add_data failed!", res);
