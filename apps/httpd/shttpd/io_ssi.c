...
do_exec(struct ssi *ssi, char *buf, int len, int *n)
{
	...
	if(ssi->conn->query)
	{
		setenv("MYQUERYSTRING",ssi->conn->query,1);
	}
	if (inc->cond == SSI_STOP) {
		/* Do nothing - conditional FALSE */
...
read_ssi(struct stream *stream, void *vbuf, size_t len)
{
	...
	char		*buf = vbuf;
	int		ch = EOF, n = 0;
    fprintf(stderr,"Enter:%s inc->nbuf: %d len:%d \n",__func__,inc->nbuf,len);
    
    /*When Return 0 , up layer will think it is finished, 
     *but it is really not finished for n+inc->nbuf is less than len
     *so it set errno number to EAGAIN for this special condition.*/
    if(inc->nbuf >= len)
    {
        ERRNO = EWOULDBLOCK;
        return (-1);
...
again:
    ...
    fprintf(stderr,"read:%d inc->nbuf: %d len:%d \n", n, inc->nbuf, len);
