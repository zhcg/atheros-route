 ...
 * $Id: //depot/sw/releases/Aquila_9.2.0_U11/apps/httpd/shttpd/shttpd.c#1 $
 */

#include "defs.h"
#include "shttpdext.h"         /* add by bowenguo, for user management, 2009-02-20 */
...
shttpd_open_listening_port(int port)
{
    ...
    if(isIpv6)
    {
        ...
        memset(&sa.u.sin6.sin6_addr, 0, sizeof(sa.u.sin6.sin6_addr));

        if ((sock = socket(PF_INET6, SOCK_STREAM, 6)) == -1)
            goto fail;
    }
    else
    {
...
is_allowed(const struct shttpd_ctx *ctx, const struct usa *usa)
{
    ...
    if (isIpv6)
    {
        LL_FOREACH(&ctx->acl, lp) {
            ...
            (void) memcpy(&ipv6Addr, &usa->u.sin6.sin6_addr, sizeof(ipv6Addr));
            if (memcmp( &(acl->ip), &ipv6Addr, acl->mask / 8) == 0)
            	allowed = acl->flag;
        }
    }
    else
...
shttpd_socketpair(int sp[2])
{
    ...
    if (isIpv6)
    {
        socklen_t		len = sizeof(sav6);    
        (void) memset(&sav6, 0, sizeof(sav6));
        sav6.sin6_family 	= AF_INET6;
        sav6.sin6_port		= htons(0);
        inet_pton( AF_INET6, "::1", &sav6.sin6_addr );    

        if ((sock = socket(AF_INET6, SOCK_STREAM, 0)) != -1 &&
            !bind(sock, (struct sockaddr *) &sav6, len) &&
            !listen(sock, 1) &&
            !getsockname(sock, (struct sockaddr *) &sav6, &len) &&
            (sp[0] = socket(AF_INET6, SOCK_STREAM, 6)) != -1 &&
            !connect(sp[0], (struct sockaddr *) &sav6, len) &&
            (sp[1] = accept(sock,(struct sockaddr *) &sav6, &len)) != -1) 
        {
            /* Success */
            ret = 0;
        }
    ...
    {
        socklen_t		len = sizeof(sav4);            
        (void) memset(&sav4, 0, sizeof(sav4));
        sav4.sin_family 		= AF_INET;
        sav4.sin_port		= htons(0);
        sav4.sin_addr.s_addr	= htonl(INADDR_LOOPBACK);

        if ((sock = socket(AF_INET, SOCK_STREAM, 0)) != -1 &&
            !bind(sock, (struct sockaddr *) &sav4, len) &&
            !listen(sock, 1) &&
            !getsockname(sock, (struct sockaddr *) &sav4, &len) &&
            (sp[0] = socket(AF_INET, SOCK_STREAM, 6)) != -1 &&
            !connect(sp[0], (struct sockaddr *) &sav4, len) &&
            (sp[1] = accept(sock,(struct sockaddr *) &sav4, &len)) != -1) 
        {
            /* Success */
            ret = 0;
        } 
...
#ifdef IPV6_ENABLE
     fprintf(stderr, "  -mode         4 for ipv4, 6 for ipv6\n");
#endif //IPV6_ENABLE
