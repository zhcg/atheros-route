 ...
 * $Id: //depot/sw/releases/Aquila_9.2.0_U11/apps/httpd/shttpd.ivi/shttpd.c#1 $
 */

#include "defs.h"
#include "webuserman.h"         /* add by bowenguo, for user management, 2009-02-20 */
#include "captiveportal.h"      /* add by bowenguo, for captive portal, 2009-03-10 */
...
#define CAPTIVEFILENAME "/tmp/redirect_iplist"
    if((c->loc_port ==  REDIRECTPORT) && (access(CAPTIVEFILENAME, F_OK) == 0))
    {
#if 0
        FILE * fp = NULL;
        char line[LINE_LEN];
        char url[LINE_LEN] = "http://210.22.21.1/"; /* defalut example */
        char cmdbuf[CMD_BUF_LEN];
        if_status_t if_status;
        
        if((fp = popen("iptables -t nat -L "CHAIN_NAME, "r")) != NULL)
        {
            get_ifinfo(&if_status, "br0");
            while(fgets(line, LINE_LEN, fp) != NULL)
            {
                    /* rule exist, delete one by one */
                if(strstr(line, inet_ntoa(c->sa.u.sin.sin_addr)) != NULL)
                {
                    sprintf(cmdbuf, "iptables -t nat -D %s -p tcp -s %s -d! %s --dport 80 -i br0 -j REDIRECT --to-ports %d",
                            CHAIN_NAME, inet_ntoa(c->sa.u.sin.sin_addr), if_status.ipaddr, REDIRECTPORT);
                    system(cmdbuf);
            ...
            if((fp = fopen(CAPTIVEFILENAME, "r")) != NULL)
            {
                while(fgets(line, LINE_LEN, fp) != NULL)
                {
                    if(strstr(line, inet_ntoa(c->sa.u.sin.sin_addr)) != NULL)
                    {
                        memset(url, '\0', LINE_LEN);
                        sprintf(url, "http://%s", strrchr(line, ':') + 1);
                        url[strlen(url) - 1] = '\0';
...
#define CAPTIVEREQ "/athCgiMain.cgi?action=captiveportal"
        ...
        get_ifinfo(&if_status, "br0");
        sprintf(url, "http://%s%s", if_status.ipaddr, CAPTIVEREQ);
        
        shttpdext_redirect(c, 200, url);

        fprintf(stderr, "===> what to do captive\n");
        return;
...
#define LOGINREQ "/athCgiMain.cgi?action=login"
#define LOGINPIC1 "/e8logo.gif"
#define LOGINERR    "/login_err.html"
#define ACCOUNTERR "/account_error.html"
#define AREADYERR  "/useralready_error.html"
#define CAPTIVEREQ "/athCgiMain.cgi?action=captiveportal"
    if((strstr(c->uri, LOGINREQ) == NULL) && (strstr(c->uri, LOGINPIC1) == NULL) && 
            (strstr(c->uri, LOGINPAGE) == NULL) && (strstr(c->uri, CAPTIVEREQ) == NULL) && 
            (strstr(c->uri, LOGINERR) == NULL) && (strstr(c->uri, ACCOUNTERR) == NULL) &&
            (strstr(c->uri, AREADYERR) == NULL)) {
        if(shttpdext_userverify(SESSIONFILE, inet_ntoa(c->sa.u.sin.sin_addr)) == -1) {
            shttpdext_redirect(c, 200, LOGINPAGE);
            return;
        }
    }
#undef SESSIONFILE
...
//	assert(stream->io_class != NULL);
	if(stream->io_class == NULL)
	{
		_shttpd_elog(E_LOG,NULL,"There is an unused stream:%i,to be deleted!\n",stream->chan.fd);
...
shttpd_poll(struct shttpd_ctx *ctx, int milliseconds)
{
	...
	{
		_shttpd_elog(E_LOG,NULL,"Unexpected Error:%d",ERRNO);
...
shttpd_socketpair(int sp[2])
{
    ...
    /* Firstly bring up loopback interface */
    shttpd_loopback_enable();
