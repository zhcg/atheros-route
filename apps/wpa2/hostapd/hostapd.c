...
int hostapd_self_configuration_protect; /* nonzero to prevent self-config from WPS */
...
#ifndef CONFIG_NATIVE_WINDOWS
#if 0   /* NO LONGER SUPPORTED AT ALL   #ifndef EAP_WPS */
...
static void hostapd_dump_state(struct hostapd_data *hapd)
{
	...
	if (!hapd->conf->dump_log_name) {
		...
		return;
                #else
                hapd->conf->dump_log_name = strdup("/tmp/hostapd.dump");
                #endif
	}
...
static struct hostapd_bss_config *hostapd_get_conf_for_wps(void *ctx)
{
	struct hostapd_data *hapd = (struct hostapd_data *)ctx;
	return hapd?hapd->conf:NULL;
}
...
static int hostapd_init_wps_opt_upnp(struct hostapd_data *hapd)
{
	...
	ctx->get_conf = hostapd_get_conf_for_wps;
	...
	if (hapd->wps_opt_upnp == NULL) {
		...
		return -1;
                #else
                wpa_printf(MSG_INFO, "Continuing with WPS UPnP disabled.");
                #endif
	}
...
#if 0   /* not needed for Atheros chips? */
/**
 ...
 */
static int hostapd_setup_bss(struct hostapd_data *hapd, int first/*unused*/)
{
	...
	if (!first) {
        ...
        #else
        memcpy(conf->bssid, hapd->own_addr, ETH_ALEN);
        #endif
	...
	if (conf->ssid.ssid_set) {
        ...
        #else
        if (!conf->ssid.ssid_set) {
                /* Not set ... Invent SSID based upon mac address.
                 * This is better than nothing, but really only good
                 * for a service set with only this AP.
                 */
                sprintf(conf->ssid.ssid, "AP-%02x%02x%02x%02x%02x%02x",
                        hapd->own_addr[0], hapd->own_addr[1], 
                        hapd->own_addr[2], hapd->own_addr[3], 
                        hapd->own_addr[4], hapd->own_addr[5]);
                conf->ssid.ssid_len = strlen(conf->ssid.ssid);
...
/**
 * setup_interface2_bss - Setup (initialize) an interface (part 2)
 * @iface: Pointer to interface data.
 * @hapd: Pointer to interface BSS data.
 * Returns: 0 on success; -1 on failure.
 *
 * Flushes old stations, sets the channel, DFS parameters, encryption,
 * beacons, and WDS links based on the configuration.
 */
static int setup_interface2_bss(struct hostapd_iface *iface,
...
#ifdef WPS_OPT_UPNP
        if (hapd->conf->wps->wps_disable == 0 &&
            hapd->conf->wps->wps_upnp_disable == 0) {
...
static int setup_interface2(struct hostapd_iface *iface)
{
         ...
         * to treat all BSSs equally, which works well for Atheros chips
         * but probably not for other chips.
         */
	for (i = 0; i < iface->num_bss; i++) {
                if (setup_interface2_bss(iface, iface->bss[i])) {
                        return 1;
                }
        }
	ap_list_init(iface);
...
/**
 * setup_interface1_bss - Setup (initialize) an interface BSS (part 1)
 * @iface: Pointer to interface data
 * @hapd: Pointer to interface BSS data
 * Returns: 0 on success, -1 on failure
 *
 ...
 */
static int setup_interface1_bss(
	...
	struct hostapd_data *hapd)
{
...
#elif defined(EAP_WPS)
        /* Note: even if hapd->conf->wps->wps_disable is set we still
         * go ahead and set things up, in order to avoid possible
         * issues with later enablement (should we support that?)
         * or odd bugs.
         * We honor wps_disable flag as some key points instead.
         */
	wps = hapd->conf->wps;
	if (wps) {
		...
		u16 num_modes, flags;
                ...
                /* NOTE: wps->mac and wps->uuid[_set] are taken care of
                 * in driver init e.g. madwifi_init()
                 */
		...
		if (modes) {
			...
			(void)eap_wps_set_ie(hapd, wps);
		}
                /* Enable default WPS modes per configuration
                 * (Atheros: eap_wps_disable has been modified 
                 * to do this).
                 * This may call eap_wps_set_ie() again.
                 */
	...
	if (hostapd_init_wps_opt_nfc(hapd) < 0)
		return -1;
	if (hapd->conf->nfcname) {
		wps_opt_nfc_sm_set_ifname(hapd->wps_opt_nfc, hapd->conf->nfcname);
	}
#endif /* WPS_OPT_NFC */
...
static int setup_interface1(struct hostapd_iface *iface)
{
         ...
         */
	for (i = 0; i < iface->num_bss; i++) {
                if (setup_interface1_bss(iface, iface->bss[i])) {
                        return 1;
                }
        }
        return 0;
...
#endif /* MODIFIED_BY_SONY */
	fprintf(stderr,
		"Copyright (c) 2007-2008, Atheros Communications\n");
}
...
static void usage(void)
{
	...
	fprintf(stderr,
		...
		"   -d   show more debug messages (-dd for even more)\n"
                "   -p   protect against WPS self-configuration\n"
...
static int hostapd_radio_setup_finish(
        struct hostapd_iface *hapd_iface
        )
{
	struct hostapd_config *conf = hapd_iface->conf;
        int i;
	...
	for (i = 0; i < conf->num_bss; i++) {
		if ((hapd_iface->bss[i] = hostapd_alloc_bss_data(
                                hapd_iface, conf, &conf->bss[i])) == NULL)
			goto fail;
	}
...
{
        wpa_printf(MSG_INFO, "WILL RESTART TO RELOAD CONFIGURATION IN 15 SECONDS");
	eloop_register_timeout(15, 0, hostapd_reload_configuration_timer, 0, 0);
	return 0;
}
#endif /* EAP_WPS */
#endif /* CONFIG_NATIVE_WINDOWS */
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		case 'K':
			...
			break;
                case 'p':
                        hostapd_self_configuration_protect++;
                        break;
		case 'P':
			pid_file = optarg;
			break;
	...
	/* Initialize eloop callback system */
	...
	/* Remaining arguments are topology files */
        if (hostapd_config_read_topology_files(&interfaces, argv+optind)) {
                wpa_printf(MSG_ERROR, "Failed reading config files");
                return 1;
        }
	...
	for (i = 0; i < interfaces.count; i++) {
		if (!interfaces.iface[i])
			goto out;
                if (hostapd_radio_config_finish(interfaces.iface[i]->conf)) {
                        goto out;
                }
        }
	...
	for (i = 0; i < interfaces.count; i++) {
		if (!interfaces.iface[i])
			goto out;
                if (hostapd_radio_setup_finish(interfaces.iface[i])) {
                        goto out;
                }
		for (j = 0; j < interfaces.iface[i]->num_bss; j++) {
		        for (k = 0; k < debug; k++) {
			        if (interfaces.iface[i]->bss[j]->conf->
			            logger_stdout_level > 0)
				        interfaces.iface[i]->bss[j]->conf->
					        logger_stdout_level--;
			        interfaces.iface[i]->bss[j]->conf->debug++;
		        }
                }
...
#ifdef EAP_WPS
        #if 1   /* optional */
        /* Cleaner: just re-exec */
        if (reloading) {
        #ifdef HOSTAPD_OFFLOAD
                system("touch /tmp/reload");
        #endif
                execvp(argv[0], argv);
        }
        #endif
...
void hostapd_msg(struct hostapd_data *hapd, int level, char *fmt, ...)
{
	va_list ap;
	char *buf;
	const int buflen = 2048/*enough?*/ + strlen(fmt);
	int len;
