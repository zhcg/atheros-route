...
#ifndef CONFIG_RADIUS_MAC_AUTHENTICATION 
int hostapd_allowed_address(struct hostapd_data *hapd, const u8 *addr,
			    const u8 *msg, size_t len, u32 *session_timeout,
			    u32 *acct_interim_interval, int *vlan_id)
   
	/* changed the interace as below with the paramets
	 * needed and omitted the unnecessary parameters 
	 * for radius based MAC authentication  */
...
#ifndef CONFIG_RADIUS_MAC_AUTHENTICATION
		/* auth frames is not needed as of current design 
		 * for radius MAC authentication */
...
#ifdef CONFIG_RADIUS_MAC_AUTHENTICATION
		 /* set station authorized if radius accept
		  *  is received */	
		 hostapd_sta_set_flags(hapd, query->addr,
...
#else
	} else if (msg->hdr->code == RADIUS_CODE_ACCESS_REJECT) {
		cache->accepted = HOSTAPD_ACL_REJECT;
		 /* send deauth if station is not radius mac authenticated */
		hostapd_sta_deauth(hapd, query->addr, IEEE80211_REASON_UNSPECIFIED);
	} 
#endif
...
#ifndef CONFIG_RADIUS_MAC_AUTHENTICATION
	/* sending of auth frames is not needed as by design for
