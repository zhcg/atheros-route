/*
 * hostapd / Driver interaction with Atheros UMAC Mainline driver
 * Copyright (c) 2004, Sam Leffler <sam@errno.com>
 * Copyright (c) 2004, Video54 Technologies
 * Copyright (c) 2005-2006, Jouni Malinen <j@w1.fi>
 * Copyright (c) 2009, Cliff Holden <cholden@atheros.com>
 *
...
/*
** Includes required for UMAC build.
*/
...
#ifdef CONFIG_RADIUS_MAC_AUTHENTICATION
...
static void atheros_deinit(void *priv);
static int atheros_sta_deauth(void *priv, const u8 *addr, int reason_code);

#if EAP_WPS
static int atheros_set_wps_beacon_ie(void *priv, u8 *iebuf, int iebuflen);
static int atheros_set_wps_probe_resp_ie(void *priv, u8 *iebuf, int iebuflen);
static int atheros_set_wps_assoc_resp_ie(void *priv, u8 *iebuf, int iebuflen);
static int atheros_start_receive_prob_req(void *priv);
static void atheros_handle_mgmt_frames(void *ctx, const unsigned char *src_addr,
...
set80211priv(struct atheros_driver_data *drv, int op, void *data, int len)
{
	...
	if (ioctl(drv->ioctl_sock, op, &iwr) < 0) {
        ...
        wpa_printf(MSG_ERROR, "ioctl 0x%x failed errno=%d", op, errno);
		return -1;
	}
	return 0;
}
...
static int
atheros_set_ieee8021x(const char *ifname, void *priv, int enabled)
{
	struct atheros_driver_data *drv = priv;
	struct hostapd_data *hapd = drv->hapd;
	struct hostapd_bss_config *conf = hapd->conf;
	...
	if (!enabled) {
    	...
    	*/
		if(conf->auth_algs==1 && wep.keys_set==0)
            ...
            /*Fix for shared wep  when run using hostapd*/
            return set80211param(priv, IEEE80211_PARAM_PRIVACY, 1);
        }
...
static int
atheros_set_privacy(const char *ifname, void *priv, int enabled)
{
	struct atheros_driver_data *drv = priv;
	struct hostapd_data *hapd = drv->hapd;
...
static int
atheros_sta_set_flags(void *priv, const u8 *addr, int flags_or, int flags_and)
{
...
static int
atheros_set_key(const char *ifname, void *priv, const char *alg,
		const u8 *addr, int key_idx,
...
static int
atheros_set_opt_ie(const char *ifname, void *priv, const u8 *ie, size_t ie_len)
{
	/*
...
static int
atheros_is_ifup(struct atheros_driver_data *drv)
{
	struct ifreq ifr;
...
static int
atheros_sta_deauth(void *priv, const u8 *addr, int reason_code)
{
	struct atheros_driver_data *drv = priv;
	struct hostapd_data *hapd = drv->hapd;
	struct ieee80211req_mlme mlme;
...
static int
atheros_sta_disassoc(void *priv, const u8 *addr, int reason_code)
{
	struct atheros_driver_data *drv = priv;
	struct hostapd_data *hapd = drv->hapd;
	struct ieee80211req_mlme mlme;
...
atheros_process_wpa_ie(struct atheros_driver_data *drv, struct sta_info *sta)
{
	...
	if (!ielen) {
		...
		printf("driver_atheros: Prepared EAP-WPS\n");
                #endif
	} else {
		ielen += 2;
#endif /* EAP_WPS */
...
#ifdef CONFIG_RADIUS_MAC_AUTHENTICATION
static int
atheros_radius_acl_check(struct atheros_driver_data *drv, u8 addr[IEEE80211_ADDR_LEN])
{
	struct hostapd_data *hapd = drv->hapd;
	...
	{
		/* set unauthorized until the station gets 
		   radius mac authenticated successfully */
		 	
		hostapd_sta_set_flags(hapd, addr,
                                           0, ~WLAN_STA_AUTHORIZED);

		res = hostapd_allowed_address(hapd, addr, &session_timeout,
			&acct_interim_interval);
	
		if((res ==  HOSTAPD_ACL_REJECT ) || (res == HOSTAPD_ACL_ACCEPT_TIMEOUT))
			 atheros_sta_deauth(drv, addr, IEEE80211_REASON_UNSPECIFIED);

		else if(res ==  HOSTAPD_ACL_ACCEPT)
			hostapd_sta_set_flags(hapd, addr,
...
#ifdef CONFIG_RADIUS_MAC_AUTHENTICATION
	/* check for radius mac authentication */
	atheros_radius_acl_check(drv,addr);
#endif

	if (hapd->conf->wpa) {
		if (atheros_process_wpa_ie(drv, sta))
			return -1;
	}
		...
		while ((key = strchr(key, '\n')) != NULL) {
			...
			*value++ = '\0';
			...
			key = value;
                ...
                char *durp=NULL;
                u32 duration=0;
                u32 wps_push_ignore_secs=0;
                durp = strstr(custom, "dur=");
                if (durp != NULL) {
                    durp+=4;
                    duration = atoi(durp);
                }
                ...
                if (hapd && hapd->conf && hapd->conf->wps) {
                    wps_push_ignore_secs = hapd->conf->wps->push_duration_ignore_secs;
                }
                if (wps_push_ignore_secs == 0) {
                    wps_push_ignore_secs = WPS_DEF_PUSH_BUTTON_DUR_IGNORE_SECS;
                }
                if (duration <= wps_push_ignore_secs) {
                    if (eap_wps_enable(hapd, hapd->conf->wps, &params)) {
                            wpa_printf(MSG_ERROR, "Push button failed to enable WPS");
                    }
                } else {
                    wpa_printf(MSG_INFO, "But WPS push button Ignored because duration=%d >= %d secs Ignore",
                               duration, wps_push_ignore_secs);
                }
#else
                if (eap_wps_enable(hapd, hapd->conf->wps, &params)) {
                        wpa_printf(MSG_ERROR, "Push button failed to enable WPS");
                ...
                atheros_handle_mgmt_frames(drv, NULL, 
					    ...
					    u8 *data, int len)
{
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case IWEVASSOCREQIE:    /* per HACK in driver, same as IWEVCUSTOM */
			...
			atheros_wireless_event_wireless_custom(drv, (char *)buf);
			free(buf);
			break;
		case IWEVGENIE: {
...
atheros_wireless_event_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	while (left >= (int) sizeof(*h)) {
		...
		plen = len - sizeof(*h);
		if (len > left || plen < 0) {
			printf("%s:Malformed netlink message: "
			       "len=%d left=%d plen=%d\n",__func__, 
			       len, left, plen);
			break;
		}
...
atheros_init(struct hostapd_data *hapd)
{
	...
	drv = wpa_zalloc(sizeof(struct atheros_driver_data));
	if (drv == NULL) {
	...
	atheros_set_privacy(drv->iface, drv, 0); /* default to no privacy */
	...
	do {
		...
		wps->mac_set = 1;
                ...
                {
					/*
					**  Autogeneration of UUID
					**  For many applications, we need to use the same UUID for
					**  all wireless interfaces.  To simplify this, we will use
					**  the same UUID generated from the MAC address of the
					**  wifi0 interface (gauranteed to be up and available at
					**  this point).
					**
					**  Since there is no gaurantee we can talk to wifi0 through
					**  this interface instance to the driver, we simply use popen
					**  to get the hardware address using ifconfig | grep
					*/
    				...
    				/*
    				** Provide the command string to popen
    				*/
#ifndef ATH_SINGLE_CFG
    				FILE            *f;
    				f = popen("ifconfig wifi0 | grep HWaddr", "r");
    				if(f)
    				{
    				    /*
    				    ** Read each line.
					...
					*/
                                ...
                                if (ret == 0) {
                                 ...
                                 /* display result */
                                    sprintf(&buffer[7],"%.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
                                ...
                                {
                                    /*
                                    ** Set each value based on Point to the beginning of the string
                                    */
                                    sscanf(&buffer[7 +  0],"%hhx",&(wps->uuid[SIZE_UUID-6]));
                                    sscanf(&buffer[7 +  3],"%hhx",&(wps->uuid[SIZE_UUID-5]));
                                    sscanf(&buffer[7 +  6],"%hhx",&(wps->uuid[SIZE_UUID-4]));
                                    sscanf(&buffer[7 +  9],"%hhx",&(wps->uuid[SIZE_UUID-3]));
                                    sscanf(&buffer[7 + 12],"%hhx",&(wps->uuid[SIZE_UUID-2]));
                                    sscanf(&buffer[7 + 15],"%hhx",&(wps->uuid[SIZE_UUID-1]));
                ...
                if (wps->wps_disable) {
		        if (atheros_set_wps_beacon_ie(drv, (u8 *)"", 0)) {
		        ...
		        /* Set WPS Beacon IE */
		        if (atheros_set_wps_beacon_ie(drv, iebuf, iebuflen)) {
                ...
                if (wps->wps_disable) {
		        if (atheros_set_wps_probe_resp_ie(drv, (u8 *)"", 0)) {
		        ...
		        if (atheros_set_wps_probe_resp_ie(drv, iebuf, iebuflen)) {
			        perror("Set WPS ProbeResp IE error");
			        break;
		        }
		        os_free(iebuf);
                ...
                if (wps->wps_disable) {
                #endif
		        if (atheros_set_wps_assoc_resp_ie(drv, (u8 *)"", 0)) {
		        ...
		        if (atheros_set_wps_assoc_resp_ie(drv, iebuf, iebuflen)) {
                 ...
                 * to be mandatory.
                 */
                atheros_start_receive_prob_req(drv);
	...
	return 0;
bad:
        wpa_printf(MSG_ERROR, "atheros_init failed!");
#if EAP_WPS
#ifndef USE_INTEL_SDK
#endif
#endif
...
static int
atheros_set_ssid(const char *ifname, void *priv, const u8 *buf, int len)
{
	struct atheros_driver_data *drv = priv;
	struct iwreq iwr;
...
static int
atheros_get_ssid(const char *ifname, void *priv, u8 *buf, int len)
{
	struct atheros_driver_data *drv = priv;
	struct iwreq iwr;
	int ret = 0;
...
static int
atheros_set_wps_beacon_ie(void *priv, u8 *iebuf, int iebuflen)
{
...
static int
atheros_set_wps_probe_resp_ie(void *priv, u8 *iebuf, int iebuflen)
{
	return atheros_set_wps_ie(priv, iebuf, iebuflen, 
 ...
 */
static int
atheros_start_receive_prob_req(void *priv)
{
 ...
 */
static void
atheros_handle_mgmt_frames(
        void *ctx,                      /* struct atheros_driver_data * */
...
static int
atheros_set_wps_assoc_resp_ie(void *priv, u8 *iebuf, int iebuflen)
{
	return atheros_set_wps_ie(priv, iebuf, iebuflen, 
...
#ifdef EAP_WPS
	.set_wps_beacon_ie		= atheros_set_wps_beacon_ie,
	.set_wps_probe_resp_ie	= atheros_set_wps_probe_resp_ie,
	.set_wps_assoc_resp_ie	= atheros_set_wps_assoc_resp_ie,
#endif /* EAP_WPS */
};
