				      ...
				      char *buf, size_t buflen)
{
	...
	if (hapd->conf) {
		...
		if (bss->eap_user->password_len > 64 ||
			...
			sprintf(tmp, "invalid password_len %d",
...
extern
void wps_opt_upnp_hostipaddress(void *, char *);

int
hostapd_ctrl_iface_hostipaddress(struct hostapd_data *hapd, char *ipaddr)
{
    if(hapd->wps_opt_upnp)
        wps_opt_upnp_hostipaddress(hapd->wps_opt_upnp, ipaddr);
    return 0;
}
#endif
...
#endif /* EAP_WPS */
#ifdef HOSTAPD_OFFLOAD
    } else if (os_strncmp(buf, "HOST_IPADDRESS", 14) == 0) {
        if (hostapd_ctrl_iface_hostipaddress(hapd, buf + 15))
            reply_len = -1;
#endif
    } else {
