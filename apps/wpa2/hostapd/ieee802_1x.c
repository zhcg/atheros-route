			...
			size_t len)
{
	...
	if (!hapd->conf->ieee802_1x ||
		...
		return;
#else /* EAP_WPS */
        #if 1   /* Atheros extension -- ignore assoc. req. for WPA-PSK */
        /* The atheros extension, when enabled, allows some dumb stations
        *       to do an association request asking for WPA-PSK
        *       (and without WPS), but then to turn around in the
        *       eapol protocol and attach WiFi_Protected_Setup to
        *       end of eapol start to indicate they want to do WPS instead.
        */
        if (hapd->conf->wps->atheros_extension &&
            hdr->type == IEEE802_1X_TYPE_EAPOL_START &&
                (strncmp((const char *)buf+ sizeof(struct ieee802_1x_hdr),
                        "WiFi_Protected_Setup",
                        strlen("WiFi_Protected_Setup"))==0)) {
            wpa_wps_fix(sta->wpa_sm);
        }
        #endif  /* Atheros extension */
...
void ieee802_1x_free_station(struct sta_info *sta)
{
	...
	free(sm->last_eap_supp);
	free(sm->last_eap_radius);
        sm->last_eap_radius = NULL;     /* to be sure */
	free(sm->identity);
				    ...
				    struct sta_info *sta)
{
	...
	if (sm == NULL)
        ...
        #if 0   /* original... doesn't work with WPS */
        if (hapd->conf->ieee802_1x == 0) {
                return;
        } 
        else 
        #endif
        ...
        #if 1   /* move check here to work with WPS */
        } else
        if (hapd->conf->ieee802_1x == 0) {
                return;
        #endif
