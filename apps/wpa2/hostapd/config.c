...
#define LOOPBACK_IP "127.0.0.1"
	ret = hostapd_parse_ip_addr(LOOPBACK_IP, &nserv->addr);
#else
				   ...
				   char *val)
{
	...
	if (val[0] == '"') {
		...
		len -= 2;
                if (len > 0) {
		wep->key[keyidx] = (u8*)malloc(len +1);
		if (wep->key[keyidx] == NULL)
			return -1;
		memcpy(wep->key[keyidx], val + 1, len);
        wep->key[keyidx][len] = '\0';
                }
	} else {
		if (len & 1)
			return -1;
        ...
        )
{
...
#ifdef HOSTAPD_OFFLOAD
		bss->radius->auth_server->port = 1812;
#else
	...
	if (bss->auth_algs == 0) {
        ...
        /*
         * WEP is made inaccessible when WPA is configured for WPS security.
         */
        bss->ssid.wep.idx = 0;
        bss->ssid.wep.key[0] = NULL;
        bss->ssid.wep.key[1] = NULL;
        bss->ssid.wep.key[2] = NULL;
        bss->ssid.wep.key[3] = NULL;
        bss->ssid.wep.len[0] = 0;
        bss->ssid.wep.len[1] = 0;
        bss->ssid.wep.len[2] = 0;
        bss->ssid.wep.len[3] = 0;
