...
#ifdef CONFIG_TDLS
int wpa_driver_madwifi_send_tdls_mgmt(void *priv, u8* buf, u16 len)
{
    struct wpa_driver_madwifi_data *drv = priv;
    struct iwreq iwr;
    int ret = 0;
    ...
    if (ioctl(drv->sock, SIOCIWFIRSTPRIV+21, &iwr) < 0) {
        perror("ioctl[SIOCIWFIRSTPRIV+21]");
        ret = -1;
    }
...
#endif /* CONFIG_ETHERSTA */
#ifdef CONFIG_TDLS
    .send_tdls_mgmt         = wpa_driver_madwifi_send_tdls_mgmt,
#endif /* CONFIG_TDLS */
};
