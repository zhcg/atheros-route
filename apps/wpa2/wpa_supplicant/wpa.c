...
#define STK_RETRY_COUNT 	3
#define SMK_TIMEOUT         5000
#define STK_TIMEOUT         5000
...
#ifdef CONFIG_TDLS
#if 0 /* commented for TDLS Draft9.0 */
static const u8 RSN_KEY_DATA_DHK_I[] = { 0x00, 0x0f, 0xac, 9 };
static const u8 RSN_KEY_DATA_DHK_P[] = { 0x00, 0x0f, 0xac, 10 };
#endif /* commented for TDLS Draft9.0 */
#endif /* CONFIG_TDLS */
...
#ifdef CONFIG_TDLS
#define WPA_KEY_INFO_SMK_MESSAGE_M2 BIT(14)
#endif /* CONFIG_TDLS */
...
//TODO::: static void wpa_smk_send_error(struct wpa_authenticator *wpa_auth,
//                               struct wpa_state_machine *sm, const u8 *peer,
//                               u16 mui, u16 error_type);

static void wpa_tdls_smk_retry(struct wpa_sm *sm, 
			struct wpa_peerkey *peerkey, const u8 *dest, 
			int type, u8 *buf, int buf_len);
...
static void wpa_tdls_smkretry_timeout_cancel(struct wpa_sm *sm, 
			struct wpa_peerkey *peerkey, int type);

static void wpa_tdls_stkretry_timeout(void *eloop_ctx, void *timeout_ctx);

static void wpa_tdls_stkretry_timeout_cancel(struct wpa_sm *sm, 
			struct wpa_peerkey *peerkey);
...
#if 0 /* commented for TDLS Draft9.0 */
DH * tdls_get_pubkey(u8* public_key) {

	//u8 public_key[SIZE_PUB_KEY];
	DH *dh = NULL;
	u32 g;
	int length;
	//DH *dh_secret=NULL;
...
int tdls_compute_dhkey(u8 *shared_secret, u8* pub_key, DH *dh_secret) {
	...
	do{
		...
		if(dh_secret)
		if (!BN_bin2bn(pub_key, SIZE_OF_DHK, bn_peer))
			break;
...
int 
wpa_tdls_set_key(struct wpa_sm *sm, struct wpa_peerkey *peerkey) {
	size_t len, key_len;
	const u8 *_key;
	u8 key_buf[32], rsc[6];
	wpa_alg alg;

	_key = (u8 *) peerkey->tsmk.tk1;
	...
	wpa_hexdump(MSG_DEBUG, "RSN: peerkey->tsmk.tk1", peerkey->tsmk.tk1, 16);
	wpa_hexdump(MSG_DEBUG, "RSN: peerkey->tsmk.kck", peerkey->tsmk.kck, 16);

	if (wpa_sm_set_key(sm, alg, peerkey->addr, -1, 1,
...
int wpa_tdls_send_smk_msg(struct wpa_sm *sm, u8 *buf, uint16_t len) {

	return wpa_sm_send_tdls_mgmt(sm, buf, len);
}


static 
int wpa_tdls_smk_send(struct wpa_sm *sm,
				const u8 *kck, int ver, 
				const u8 *dest, enum TDLS_FRAME_TYPE type, 
				u8 *msg, uint16_t msg_len, 
				u8 *key_mic)
{
	u16 len = msg_len + ETH_ALEN + 2 + 2;
	u8 * buf; 
	u8 * cur_buf;

		buf = malloc(len);
		if(!buf) {
			wpa_printf(MSG_INFO, "TDLS: Error in memory allocation");
			return -1;
		}
		cur_buf  = buf;
		memset(buf, 0, len);
		memcpy(cur_buf, dest, ETH_ALEN); 
		cur_buf += ETH_ALEN;

		*(uint16_t *)cur_buf = type ; 
		cur_buf += 2;

		*(uint16_t *)cur_buf = msg_len ; 
		cur_buf += 2;
		
		/* do MIC before giving the data to driver */
		if(key_mic && type == IEEE80211_TDLS_SETUP_RESP) {
			wpa_eapol_key_mic(kck, ver, msg, msg_len, key_mic);
		}
		else if(key_mic && type == IEEE80211_TDLS_SETUP_CONFIRM) {
			wpa_eapol_key_mic(kck, ver, msg, msg_len, key_mic);
		}
		
		if(msg && msg_len)
			memcpy(cur_buf, msg, msg_len);

		wpa_tdls_smk_retry(sm, NULL, dest, type, buf, len);

		if(wpa_tdls_send_smk_msg(sm, buf, len)) {
			wpa_printf(MSG_INFO, "Error: Sending TDLS SMK, Type-%x ", type);
		} 
...
static u8 * wpa_tdls_alloc_eapol(const struct wpa_supplicant *wpa_s, u8 type,
                            const void *data, u16 data_len,
                            u16 *msg_len, void **data_pos)
{
        ...
        *msg_len = sizeof(*hdr) + data_len;
        ...
        hdr->version = DEFAULT_EAPOL_VERSION;//wpa_s->conf->eapol_version;
        hdr->type = type;
...
#if 0 /* commented for TDLS Draft9.0 */
#ifdef CONFIG_TDLS
static void wpa_tdls_generate_peerkey_smk(struct wpa_peerkey *peerkey,
				const u8 *addr, const u8 *shared_sec)
{
    u8 out[SIZE_256_BITS];

	SHA256(shared_sec, SIZE_PUB_KEY, out);

	memcpy((u8*)&peerkey->smk, &out, PMK_LEN);
	
	/* for MIC check */
	wpa_pmk_to_ptk(peerkey->smk, PMK_LEN, "SMK Key Derivation", 
			addr, peerkey->addr,
			peerkey->inonce, peerkey->pnonce,
			(u8*)&peerkey->tsmk, sizeof(peerkey->tsmk));
	peerkey->tsmk_set = 1;
...
#ifdef CONFIG_TDLS
static void wpa_tdls_generate_peerkey_smk_draft6(struct wpa_peerkey *peerkey,
				const u8 *addr)
{
    //u8 out[SIZE_256_BITS];

	//SHA256(peerkey->pnonce, WPA_NONCE_LEN, out);

	//memcpy((u8*)&peerkey->smk, &out, PMK_LEN);
	
	/* for MIC check */
	wpa_pmk_to_ptk(peerkey->smk, PMK_LEN, "TDLS PMK", 
			addr, peerkey->addr,
			peerkey->inonce, peerkey->pnonce,
			(u8*)&peerkey->tsmk, sizeof(peerkey->tsmk));
	peerkey->tsmk_set = 1;
...
#ifdef CONFIG_TDLS
static int wpa_supplicant_verify_tdls_mic_peerkey(struct wpa_sm *sm, 
			struct wpa_peerkey *peerkey, struct wpa_eapol_key *key, 
			u16 ver, const u8 *buf, size_t len)
{
	u8 mic[16];
	...
	if (peerkey->tsmk_set) {
		...
		wpa_eapol_key_mic(peerkey->tsmk.kck, ver, buf, len,
				  key->key_mic);
		if (os_memcmp(mic, key->key_mic, 16) != 0) {
			wpa_printf(MSG_WARNING, "RSN: Invalid EAPOL-Key MIC "
				   "when using SMK - dropping packet");
			return -1;
		}
	} else {
			wpa_printf(MSG_WARNING, "RSN: Could not verify EAPOL-Key MIC, "
				   "SMK not set - dropping packet");
			return -1;
	}
	return 0;
}
...
int wpa_sm_stkstart(struct wpa_sm *sm, const u8 *peer)
{
...
#if 0
	/* 
	 * NOTE: TDLS-SMK 3-way handshake is blocked by the below check 
	 * which reads as check for STK, so its commented. This function 
	 * is used now for 802.11z T-DLS SMK 3-way and not STK.
	 */
	if (sm->ap_rsn_ie &&
	...
	for (peerkey = sm->peerkey; peerkey; peerkey = peerkey->next) {
		if (os_memcmp(peerkey->addr, peer, ETH_ALEN) == 0)
			break;
	}

	if (peerkey == NULL) {
		wpa_printf(MSG_INFO, "RSN: No matching entry found for"
			   "peer, creating one. " MACSTR, MAC2STR(peer));
...
#if 0 /* TKIP is not used to encrypt TDLS link */
...
#ifdef CONFIG_TDLS			
	kde_len +=	2 + RSN_SELECTOR_LEN + sizeof(lifetime);
#if 0 /* commented for TDLS Draft9.0 */
				2 + RSN_SELECTOR_LEN + SIZE_OF_DHK;
#endif /* commented for TDLS Draft9.0 */

	rbuf = wpa_tdls_alloc_eapol(sm->ctx->ctx, IEEE802_1X_TYPE_EAPOL_KEY, NULL,
#else
	rbuf = wpa_sm_alloc_eapol(sm->ctx->ctx, IEEE802_1X_TYPE_EAPOL_KEY, NULL,
#endif /* CONFIG_TDLS */
	...
	key_info = WPA_KEY_INFO_SMK_MESSAGE | /*WPA_KEY_INFO_MIC |*/
		WPA_KEY_INFO_SECURE | WPA_KEY_INFO_ACK | ver;
	WPA_PUT_BE16(req->key_info, key_info);
	WPA_PUT_BE16(req->key_length, 0);
	os_memcpy(req->replay_counter, peerkey->replay_counter,
		  WPA_REPLAY_COUNTER_LEN);
	...
	pos = wpa_add_kde(pos, RSN_KEY_DATA_MAC_ADDR, sm->own_addr, ETH_ALEN);
...
#if 0 /* commented for TDLS Draft9.0 */
	/* DHK Initiator */
	memset(public_key, 0, SIZE_PUB_KEY);
	dh_secret = tdls_get_pubkey(public_key);
	if(!dh_secret) {
		wpa_printf(MSG_DEBUG, "RSN: DHK not Generated");
		return -1;
	}
	peerkey->dhkptr = dh_secret;
	os_memcpy(&peerkey->dhk.i_key, public_key, SIZE_OF_DHK);
	pos = wpa_add_kde(pos, RSN_KEY_DATA_DHK_I, (u8 *) &peerkey->dhk.i_key, SIZE_OF_DHK);
#endif

	wpa_printf(MSG_INFO, "RSN: Sending TDLS_M1 Request (peer "MACSTR")", MAC2STR(peer));

	wpa_tdls_smk_send(sm, NULL, ver, peer, IEEE80211_TDLS_SETUP_REQ, rbuf, rlen, 0);
...
#if 0 /* commented for TDLS Draft9.0 */
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
	    os_memcmp(pos + 2, RSN_KEY_DATA_DHK_I, RSN_SELECTOR_LEN) == 0) {
		ie->dhk = pos + 2 + RSN_SELECTOR_LEN;
		ie->dhk_len = pos[1] - RSN_SELECTOR_LEN;
		return 0;
	}

	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
	    os_memcmp(pos + 2, RSN_KEY_DATA_DHK_P, RSN_SELECTOR_LEN) == 0) {
		ie->dhk = pos + 2 + RSN_SELECTOR_LEN;
		ie->dhk_len = pos[1] - RSN_SELECTOR_LEN;
		return 0;
	}
					  ...
					  u16 ver)
{
	...
	/* Check if either ssid or bssid configured */
	if (wpa_sm_get_ssid(sm) == NULL && (wpa_sm_get_bssid(sm, sm->bssid) < 0)) {
...
#ifdef CONFIG_PEERKEY
	if (wpa_sm_set_key(sm, alg, sm->bssid, -1, 1, key_rsc, rsclen,
#else
...
static void wpa_supplicant_smk_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if(peerkey->initiator)
		wpa_sm_stkstart(sm, peerkey->addr);
	else {
...
#ifdef CONFIG_TDLS
void tdls_clear_peerkey(struct wpa_peerkey *peerkey)
{
	u8 mac[ETH_ALEN];
	struct wpa_peerkey *tmp;
	
		os_memcpy(mac, peerkey->addr, ETH_ALEN);
		tmp 		 = peerkey->next;
		peerkey->initiator = 0;
		...
		/* restore things */
		os_memcpy(peerkey->addr, mac, ETH_ALEN);
		peerkey->next = tmp;
	return;
}
#endif /* CONFIG_TDLS */
...
#ifdef CONFIG_TDLS
	wpa_tdls_stkretry_timeout_cancel(sm, peerkey);
#endif /* CONFIG_TDLS */		
...
#ifdef CONFIG_TDLS
static int wpa_tdls_send_smk_m2(struct wpa_sm *sm,
				      const unsigned char *src_addr,
				      const struct wpa_eapol_key *key,
				      int ver, const struct wpa_peerkey *peerkey)
{
	u16 rlen;
	struct wpa_eapol_key *reply;
	u8 *rbuf, *pos;
	...
	rbuf = wpa_tdls_alloc_eapol(sm->ctx->ctx, IEEE802_1X_TYPE_EAPOL_KEY,
				  NULL, sizeof(*reply) + kde_len, &rlen,
				  (void *) &reply);
	if (rbuf == NULL)
		return -1;
	...
	reply->type = EAPOL_KEY_TYPE_RSN;
	key_info = ver | WPA_KEY_INFO_SMK_MESSAGE | 
	//WPA_KEY_INFO_MIC |
		WPA_KEY_INFO_SECURE | WPA_KEY_INFO_SMK_MESSAGE_M2 ;
	WPA_PUT_BE16(reply->key_info, key_info);
	WPA_PUT_BE16(reply->key_length, 0);
	os_memcpy(reply->replay_counter, key->replay_counter,
		  WPA_REPLAY_COUNTER_LEN);
	...
	/* Initiator Nonce */
	pos = wpa_add_kde(pos, RSN_KEY_DATA_NONCE, peerkey->inonce, WPA_NONCE_LEN);

	/* Lifetime */
	//lifetime = htonl(43200); /* dot11RSNAConfigSMKLifetime */
	lifetime = htonl(peerkey->lifetime); /* to have Initiator's lifetime */
	pos = wpa_add_kde(pos, RSN_KEY_DATA_LIFETIME, (u8 *) &lifetime, sizeof(lifetime));

#if 0 /* commented for TDLS Draft9.0 */
	/* DHK Peer */
	pos = wpa_add_kde(pos, RSN_KEY_DATA_DHK_P, (u8 *) &peerkey->dhk.p_key, SIZE_OF_DHK);
#endif

	wpa_tdls_smk_send(sm, peerkey->tsmk.kck, ver, src_addr, 
					IEEE80211_TDLS_SETUP_RESP, rbuf, rlen, reply->key_mic);

	return 0;
}


static int wpa_tdls_send_smk_m3(struct wpa_sm *sm,
				      const unsigned char *src_addr,
				      const struct wpa_eapol_key *key,
				      int ver, const struct wpa_peerkey *peerkey)
{
	...
	/* KDEs: Initiator MAC Address, Initiator Nonce */
	kde_len = 2 + RSN_SELECTOR_LEN + ETH_ALEN +
		  2 + RSN_SELECTOR_LEN + WPA_NONCE_LEN;
	...
	wpa_tdls_smk_send(sm, peerkey->tsmk.kck, ver, src_addr, 
					IEEE80211_TDLS_SETUP_CONFIRM, rbuf, rlen, reply->key_mic);
...
static 
int wpa_tdls_process_smk_m1(
	struct wpa_sm *sm, const unsigned char *src_addr,
	const struct wpa_eapol_key *key, u16 extra_len, int ver)
{
	struct wpa_ssid *ssid = sm->cur_ssid;
	struct wpa_peerkey *peerkey;
	struct wpa_eapol_ie_parse kde;
	...
	if (kde.mac_addr == NULL || kde.mac_addr_len < ETH_ALEN) {
		wpa_printf(MSG_INFO, "RSN: No MAC address KDE in TDLS_M1"); 
		return -1; }
#if 0 /* commented for TDLS Draft9.0 */
	if (kde.dhk ==NULL || kde.dhk_len < SIZE_OF_DHK) {
		wpa_printf(MSG_INFO, "RSN: No DHK KDE in TDLS_M1"); 
		return -1; }
#endif
	if (kde.rsn_ie == NULL) {
		wpa_printf(MSG_INFO, "RSN: No RSN IE KDE in TDLS_M1"); 
		return -1; }
	if (kde.lifetime == NULL || kde.lifetime_len < 4) {
		wpa_printf(MSG_INFO, "RSN: No Lifetime KDE in "
			   "TDLS_M1");
		return -1;
	}
	...
	if (peerkey == NULL) {
		wpa_printf(MSG_INFO, "RSN: No matching entry found for"
			   "Initiator, creating one. " MACSTR, MAC2STR(kde.mac_addr));
		peerkey = os_malloc(sizeof(*peerkey));
		if (peerkey == NULL)
			return -1;
		os_memset(peerkey, 0, sizeof(*peerkey));
	...
	peerkey->initiator = 0; /* Need to check */
	os_memcpy(peerkey->inonce, key->key_nonce, WPA_NONCE_LEN);
	os_memcpy(peerkey->rsnie_i, kde.rsn_ie, kde.rsn_ie_len);
	peerkey->rsnie_i_len = kde.rsn_ie_len;
	peerkey->cipher = cipher;
...
#if 0 /* commented for TDLS Draft9.0 */
	/* DHK Initiator Pub key */
	os_memcpy(&peerkey->dhk.i_key, (u8*)kde.dhk, kde.dhk_len); /*SIZE_OF_DHK*/

	/* Compute SMK-like DHK shared-secret */
	memset(public_key, 0, SIZE_PUB_KEY);
	...
	/* Copy Peer pub key to Send TDLS_M2 */	
	os_memcpy(&peerkey->dhk.p_key, public_key, SIZE_OF_DHK);

	/* Compute SMK-like DHK shared-secret */
	memset(&shared_sec, 0, SIZE_PUB_KEY);

	if(tdls_compute_dhkey((u8*)&shared_sec, (u8*)&peerkey->dhk.i_key, peerkey->dhkptr)) {
		wpa_printf(MSG_INFO, "RSN: Error in Computing DH Key secret");
		/* TODO: Send Error Frame */
		wpa_supplicant_peerkey_free(sm, peerkey);
		return -1;
	}

	/* generate SMK using DH key secret */
	wpa_tdls_generate_peerkey_smk(peerkey, sm->own_addr, shared_sec);
#endif /* commented for TDLS Draft9.0 */

	/* generate SMK using Nonce */
	wpa_tdls_generate_peerkey_smk_draft6(peerkey, sm->own_addr);

	/* SMK Lifetime Expiry */
	os_get_time(&now);
	peerkey->expiration = now.sec + peerkey->lifetime;
	/*eloop_register_timeout(peerkey->lifetime, 0, wpa_supplicant_smk_timeout,
			       sm, peerkey);*/

	peerkey->smk_complete = 1;
	/* After successful processing derive smkid */
	rsn_smkid(peerkey->smk, peerkey->pnonce, sm->own_addr,
			  peerkey->inonce, peerkey->addr, peerkey->smkid);
	...
	/* SEND: TDLS_M2 */	
	wpa_printf(MSG_DEBUG, "RSN: Sending TDLS_M2");
	wpa_tdls_send_smk_m2(sm, src_addr, key, ver, peerkey);

	return 0;
}


static int wpa_tdls_process_smk_m2(
	struct wpa_sm *sm, const unsigned char *src_addr,
	/*const*/ struct wpa_eapol_key *key, u16 extra_len, int ver)
{
	struct wpa_ssid *ssid = sm->cur_ssid;
	...
	if (kde.mac_addr == NULL || kde.mac_addr_len < ETH_ALEN) {
		wpa_printf(MSG_INFO, "RSN: No MAC address KDE in TDLS_M2"); 
		return -1; }
#if 0 /* commented for TDLS Draft9.0 */
	if (kde.dhk ==NULL || kde.dhk_len < SIZE_OF_DHK) {
		wpa_printf(MSG_INFO, "RSN: No DHK KDE in TDLS_M2"); 
		return -1; }
#endif /* commented for TDLS Draft9.0 */
	if (kde.nonce ==NULL || kde.nonce_len < WPA_NONCE_LEN) {
		wpa_printf(MSG_INFO, "RSN: No P-Nonce KDE in TDLS_M2"); 
		return -1; }
	if (kde.rsn_ie == NULL) {
	...
	for (peerkey = sm->peerkey; peerkey; peerkey = peerkey->next) {
		if (os_memcmp(peerkey->addr, kde.mac_addr, ETH_ALEN) == 0)
		    // && os_memcmp(peerkey->inonce, kde.nonce, WPA_NONCE_LEN) == 0)
			break;
	}
	if (peerkey == NULL) {
		wpa_printf(MSG_INFO, "RSN: No matching SMK handshake found "
			   "for TDLS_M2: peer " MACSTR,
			   MAC2STR(kde.mac_addr));
		return -1;
	} else {
		wpa_tdls_smkretry_timeout_cancel(sm, peerkey, IEEE80211_TDLS_SETUP_REQ);
	}
	
	if (!os_memcmp(peerkey->inonce, kde.nonce, WPA_NONCE_LEN) == 0) {
...
#if 0 /* commented for TDLS Draft9.0 */
	/* DHK Peer */
	memcpy(&peerkey->dhk.p_key, (u8*)kde.dhk, kde.dhk_len/*SIZE_OF_DHK*/);

	/* Compute SMK-like DHK shared-secret */
	memset(shared_sec, 0, SIZE_PUB_KEY);

	if(tdls_compute_dhkey(shared_sec, peerkey->dhk.p_key, peerkey->dhkptr)) {
		wpa_printf(MSG_INFO, "RSN: Error in Computing DH Key secret");
		wpa_supplicant_peerkey_free(sm, peerkey);
		/* TODO: Send Error Frame */
		return -1;
	...
	if( wpa_supplicant_verify_tdls_mic_peerkey(sm, peerkey, key,
					 ver, ((u8*)key) - sizeof(struct ieee802_1x_hdr), 
					extra_len + sizeof(struct wpa_eapol_key) 
					+ sizeof(struct ieee802_1x_hdr)) < 0) {
		wpa_supplicant_peerkey_free(sm, peerkey);
		return -1;
	}

	wpa_printf(MSG_INFO, "RSN: Sending TDLS_M3");
	wpa_tdls_send_smk_m3(sm, src_addr, key, ver, peerkey);
	...
	/* register STK timeout and retry */
	peerkey->stk_tmr.count = STK_RETRY_COUNT;
	peerkey->stk_tmr.timer = STK_TIMEOUT;
	eloop_register_timeout((peerkey->stk_tmr.timer)/1000, 0, 
							wpa_tdls_stkretry_timeout, sm, peerkey);
#endif /* commented for TDLS Draft9.0 */

	return wpa_tdls_set_key(sm, peerkey);
	
	return 0;
}


static int wpa_tdls_process_smk_m3(
	struct wpa_sm *sm, const unsigned char *src_addr,
	/*const*/ struct wpa_eapol_key *key, u16 extra_len, int ver)
{
	struct wpa_peerkey *peerkey;
	...
	if (wpa_supplicant_parse_ies((const u8 *) (key + 1), extra_len, &kde) < 0) {
		wpa_printf(MSG_INFO, "RSN: Failed to parse KDEs in SMK M4/M5 TDLS_M3");
		return -1;
	}

	if (kde.mac_addr == NULL || kde.mac_addr_len < ETH_ALEN) {
		wpa_printf(MSG_INFO, "RSN: No MAC address KDE in TDLS_M3"); 
		return -1; }
	if (kde.nonce == NULL || kde.nonce_len < WPA_NONCE_LEN) {
		wpa_printf(MSG_INFO, "RSN: No P-Nonce KDE in TDLS_M3"); 
		return -1; }
	...
	for (peerkey = sm->peerkey; peerkey; peerkey = peerkey->next) {
		if (os_memcmp(peerkey->addr, src_addr, ETH_ALEN) == 0) 
		    // && os_memcmp(peerkey->pnonce, key->key_nonce, WPA_NONCE_LEN) == 0)
			break;
	}
	if (peerkey == NULL) {
		wpa_printf(MSG_INFO, "RSN: No matching SMK handshake found "
			   "for TDLS_M3: peer " MACSTR,
			   MAC2STR(src_addr));
		return -1;
	} else {
		wpa_tdls_smkretry_timeout_cancel(sm, peerkey, IEEE80211_TDLS_SETUP_RESP);
	}

	if (!os_memcmp(peerkey->pnonce, key->key_nonce, WPA_NONCE_LEN) == 0)
	{
		    ...
		    const u8 *buf, size_t len)
{
	...
	if (!peerkey && sm->rx_replay_counter_set && 
		/* TDLS: To avoid replay_counter during SMK 3-way handshake */
	...
	if (key_info & WPA_KEY_INFO_KEY_TYPE) {
		...
		if (key_info & WPA_KEY_INFO_ERROR) {
			...
			/* SMK TDLS_M1 :*/
			wpa_tdls_process_smk_m1(sm, src_addr, key,
						      extra_len, ver);
		} 
		else if (key_info & WPA_KEY_INFO_SMK_MESSAGE_M2 ) {
			/* SMK TDLS_M2 :*/
			wpa_tdls_process_smk_m2(sm, src_addr, key,
						      extra_len, ver);
		} 
		else {
			/* SMK TDLS_M3 :*/
			wpa_tdls_process_smk_m3(sm, src_addr, key,
						       extra_len, ver);
		}
#endif /* CONFIG_TDLS */
#endif /* CONFIG_PEERKEY */
...
#ifdef CONFIG_TDLS
static void wpa_tdls_smk_retry(struct wpa_sm *sm, 
			struct wpa_peerkey *_peerkey_, const u8 *dest, 
			int type, u8 *buf, int buf_len)
{

struct wpa_peerkey *peerkey = NULL;

	if(type == IEEE80211_TDLS_SETUP_CONFIRM || type == IEEE80211_TDLS_TEARDOWN_REQ 
		|| type == IEEE80211_TDLS_TEARDOWN_RESP)
		return;

	for (peerkey = sm->peerkey; peerkey; peerkey = peerkey->next) {
		if (os_memcmp(peerkey->addr, dest, ETH_ALEN) == 0)
			break;
	}
	...
	peerkey->sm_tmr.count = SMK_RETRY_COUNT;
	peerkey->sm_tmr.timer = SMK_TIMEOUT; /* value in milli seconds */

	/* Copy SMK message to resend on timeout */
	peerkey->sm_tmr.type 	= type;
	peerkey->sm_tmr.buf_len = buf_len;
	memcpy(peerkey->sm_tmr.buf, buf, buf_len);

	wpa_printf(MSG_INFO,"WPA_TDLS: Peerkey Timeout Registered, Type-%x ", type);
	eloop_register_timeout((peerkey->sm_tmr.timer/1000), 0, 
						wpa_tdls_smkretry_timeout, sm, peerkey);
...
static void wpa_tdls_smkretry_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if(peerkey->sm_tmr.count) {
	
		peerkey->sm_tmr.count--;
		peerkey->sm_tmr.timer = SMK_TIMEOUT; /* value in milli seconds */

		wpa_printf(MSG_INFO, "WPA_TDLS: Peerkey Re-trying... Type-%x.", 
					peerkey->sm_tmr.type);

		if(peerkey->sm_tmr.buf == NULL ) {
			wpa_printf(MSG_INFO, "WPA_TDLS: Error SMK Retry"
					" Type-%x.", peerkey->sm_tmr.type);
			eloop_cancel_timeout(wpa_tdls_smkretry_timeout, sm, peerkey);
			return;
		}

		/* resend SMK message to Peer */
		if(wpa_tdls_send_smk_msg(sm, peerkey->sm_tmr.buf, 
					peerkey->sm_tmr.buf_len)) {
			wpa_printf(MSG_INFO, "Error: TDLS SMK Retry.");
		}
		...
		wpa_tdls_smk_send(sm, NULL, 0, peerkey->addr, 
						IEEE80211_TDLS_TEARDOWN_REQ, NULL, 0, NULL);

		wpa_printf(MSG_INFO, "Clearing SM: Peerkey("MACSTR")", 
						MAC2STR(peerkey->addr));
		eloop_cancel_timeout(wpa_tdls_smkretry_timeout, sm, peerkey);

		/* clear all resources meant for retry */	
		os_memset(peerkey->sm_tmr.buf, 0, peerkey->sm_tmr.buf_len);
		peerkey->sm_tmr.count 	= 0;
		peerkey->sm_tmr.timer 	= 0;
		peerkey->sm_tmr.buf_len = 0;
		peerkey->sm_tmr.type 	= 0;
					...
					struct wpa_peerkey *peerkey, int type)
{
	...
	if(type == peerkey->sm_tmr.type) {

		wpa_printf(MSG_INFO, "Timeout Cancelled for Type-%x ", type);
		...
		wpa_printf(MSG_INFO, "Error in Cancelling SMK Timeout"
					"(Reason: Unknown Type-%x).", type);
	}
...
static void wpa_tdls_stkretry_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	/* to send STK_1_of_4 again */
	/*	1. decrease the Re-try count.
		2. send STK 1_of_4 again to Peer.
		3. eloop_cancel_timer()
		4. reregister, eloop_register_timer()
	*/
	if(peerkey->stk_tmr.count) {
		peerkey->stk_tmr.count--;
		peerkey->stk_tmr.timer = STK_TIMEOUT; /* value in milli seconds */
		wpa_printf(MSG_INFO, "WPA_TDLS: Peerkey STK Re-trying...");
		wpa_supplicant_send_stk_1_of_4(sm, peerkey);
	
		eloop_cancel_timeout(wpa_tdls_stkretry_timeout, sm, peerkey);
		eloop_register_timeout((peerkey->stk_tmr.timer)/1000, 0, 
								wpa_tdls_stkretry_timeout, sm, peerkey);
	} else {
		
		wpa_printf(MSG_INFO, "Sending TDLS Tear_Down Request");
		wpa_tdls_smk_send(sm, NULL, 0, peerkey->addr, 
						IEEE80211_TDLS_TEARDOWN_REQ, NULL, 0, NULL);
		eloop_cancel_timeout(wpa_tdls_stkretry_timeout, sm, peerkey);
		peerkey->stk_tmr.count = 0;
		peerkey->stk_tmr.timer = 0; /* value in milli seconds */
	
		/* send Tear_Down Req and clear Peerkey statemachine */
		wpa_supplicant_peerkey_free(sm, peerkey);
	}		
return;
}
...
#ifdef CONFIG_TDLS
int wpa_tdls_recv_teardown(struct wpa_sm *sm, const u8 *src_addr,
...
/*
 * wpa_tdls_process_smk - This function validates the TDLS msg from driver
 * and process suitably.
 */
void wpa_tdls_process_smk(struct wpa_sm *sm, union wpa_event_data *data) {
	size_t len = 0; 
	u8 * buf = NULL;
	
	len = data->stkstart.smk_len;
	...
	if(buf)
		memcpy(buf, data->stkstart.smk_data, len);

	if( data->stkstart.smk_type == IEEE80211_TDLS_SETUP_REQ &&
		len == 0) {
		wpa_sm_stkstart(sm, data->stkstart.peer);
	}
	else if( data->stkstart.smk_type == IEEE80211_TDLS_SETUP_REQ) {
		wpa_sm_rx_eapol(sm, data->stkstart.peer, buf, len);
	}
	else if( data->stkstart.smk_type == IEEE80211_TDLS_SETUP_RESP) {
		wpa_sm_rx_eapol(sm, data->stkstart.peer, buf, len);
	}
	else if( data->stkstart.smk_type == IEEE80211_TDLS_SETUP_CONFIRM) {
		wpa_sm_rx_eapol(sm, data->stkstart.peer, buf, len);
	}
	else if( data->stkstart.smk_type == IEEE80211_TDLS_TEARDOWN_REQ) {
		wpa_tdls_recv_teardown(sm, data->stkstart.peer, buf, len);
	}
	/* TDLS_TEARDOWN_RESP is not handled currently */
#if 0
	else if( data->stkstart.smk_type == IEEE80211_TDLS_TEARDOWN_RESP) {
		wpa_tdls_recv_teardown(sm, data->stkstart.peer, buf, len);
	}
#endif
