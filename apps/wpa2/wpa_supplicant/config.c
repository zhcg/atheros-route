				  ...
				  const char *value)
{
	...
	if (val & ~(WPA_CIPHER_CCMP | WPA_CIPHER_TKIP | WPA_CIPHER_WEP104 |
		    WPA_CIPHER_WEP40 | WPA_CIPHER_NONE)) {
				...
				const char *value)
{
	...
	if (!value[0]) {
        ...
        if (value[0] == '"') {
            wps->upnp_iface = os_strdup(value+1);
            if (strchr(wps->upnp_iface, '"'))
                *strchr(wps->upnp_iface, '"') = 0;
        } else {
...
static const struct parse_data wps_fields[] = {
        { INT_RANGE(wps_disable, 0, 1) },
        { INT_RANGE(wps_upnp_disable, 0, 1) },
	{ HEX8(version) },
	{ FUNC(uuid) },
...
{
	if (wps) {
                if (wps->newsettings_command)
                        os_free(wps->newsettings_command);
		if (wps->manufacturer)
