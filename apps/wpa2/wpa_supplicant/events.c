		...
		for (ssid = group; ssid; ssid = ssid->pnext) {
			    ...
			    os_memcmp(bss->bssid, ssid->bssid, ETH_ALEN) != 0)
			{
				wpa_printf(MSG_DEBUG, "   skip - "
					   "BSSID mismatch");
				continue;
			} else if((ssid->ssid_len == 0) && ssid->passphrase) { 
...
#ifdef EAP_WPS
#ifndef USE_INTEL_SDK
                /* If we are scanning before selecting the actual AP
                 * then use special logic, otherwise use normal logic.
                 */
                if (wps && wps->wps_job_busy && wps->nwid_trying_wps == -1) {
                        /* WPS search */
			selected = wps_select_ssid(wpa_s, results, num, &ssid);
			break;
                } else {
#endif /* USE_INTEL_SDK */
#endif /* EAP_WPS */
			      ...
			      union wpa_event_data *data)
{
	if (data == NULL)
		...
		wpa_tdls_process_smk(wpa_s->wpa, data);
	}
#else	
