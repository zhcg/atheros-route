						     ...
						     const char *name)
{
	...
	blob->data = base64_decode((const char *)encoded, encoded_len, &blob->len);
	os_free(encoded);
...
#define INT_DEF(t, def) write_int(f, #t, ssid->t, def)
	...
	INT(proactive_key_caching);
        #if 0   /* WAS */
	INT(disabled);
        #else   /* Write only permanent disable bit */
        write_int(f, "disabled", ssid->disabled&0x1, 0);
        #endif
	INT(peerkey);
...
static int wpa_config_write_blob(FILE *f, struct wpa_config_blob *blob)
{
	...
	encoded = base64_encode(blob->data, blob->len, NULL, 72);
	if (encoded == NULL)
		return -1;
...
static void wps_write(FILE *f, const char *field, struct wps_config *wps)
{
	...
	/* in general the parser will not accept fields without values,
	*  which anyway are usually something unsupported.
	*  So don't write out fields without values...
	*/
        if (value && value[0]) 
		fprintf(f, "\t%s=%s\n", field, value?value:"");
	os_free(value);
