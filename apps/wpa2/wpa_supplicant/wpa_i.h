...
#ifdef CONFIG_TDLS
/* Temporarily added for Crypto Diffie-Hellman Key information 
 * will be removed once internel crypto supports DH Key 
 */
#include "crypto_dh.h"
#endif /* CONFIG_TDLS */
...
struct wpa_peerkey {
	...
	struct smk_timer {
		...
		int type;	/* TDLS frame type */
		int buf_len;	/* length of SMK message for retransmission */
		char buf[500];	/* buffer for SMK message */
	}sm_tmr;
...
#ifdef CONFIG_TDLS
static inline int wpa_sm_send_tdls_mgmt(struct wpa_sm *sm, u8* buf, u16 len)
{
	return sm->ctx->send_tdls_mgmt(sm->ctx->ctx, buf, len);
}
#endif /* CONFIG_TDLS */
