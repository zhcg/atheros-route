...
static void wpa_supplicant_init_wps(struct wpa_supplicant *wpa_s,
...
/**
 * wpa_supplicant_reload_configuration_all - Reload config for all interfaces
 * @wpa_s: Pointer to a wpa_supplicant data
 ...
 */
int wpa_supplicant_reload_configuration_all(struct wpa_global *global)
{
        int errors = 0;
        struct wpa_supplicant *wpa_s = global->ifaces;
        while (wpa_s != NULL) {
                if (wpa_supplicant_reload_configuration(wpa_s)) errors++;
                wpa_s = wpa_s->next;
        }
        if (errors) return -1;
        return 0;
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	if (conf == NULL) {
        ...
        /* Fix for Atheros bug 62067, wps must be initialized here.
        *       -Ted Merrill, per Jan Willem van Helden
        */
        if (conf->wps) {
                wpa_supplicant_init_wps(wpa_s, conf->wps);
        }
			      ...
			      struct wpa_ssid *ssid)
{
	...
	/* When doing WPS handshaking, turn off drop unencrypted when AP is set to OPEN */
	if (wpa_s->conf->wps != NULL && bss != NULL)
	{
		struct wps_config *wps = wpa_s->conf->wps;

		if (wps->wps_job_busy == 1 && (bss->caps & IEEE80211_CAP_PRIVACY) == 0)
			use_crypt = 0;
	}
...
#ifdef CONFIG_TDLS
static int wpa_supplicant_send_tdls_mgmt(void *wpa_s, u8 *buf,
					     u16 len)
{
	return wpa_drv_send_tdls_mgmt(wpa_s, buf, len);
			     ...
			     const u8 *buf, size_t len)
{
	...
	wpa_printf(MSG_DEBUG, "%s: new: get mac address again, "
				"for ether-dongle mode\n", __func__);
	wps = wpa_s->conf->wps;
...
/* WPS initialization code... shared with reconfigure
*/
static void wpa_supplicant_init_wps(struct wpa_supplicant *wpa_s,
                struct wps_config * wps)
{
				      ...
				      int wait_for_interface)
{
	...
	wps = wpa_s->conf->wps;
	if (wps) {
                wpa_supplicant_init_wps(wpa_s, wps);
                #if 0   /* was; moved to wpa_supplicant_init_wps above */
		struct wpa_hw_modes *modes;
		u16 num_modes, flags;
...
"Defaulted uuid based on mac addr %02x:%02x:%02x:%02x:%02x:%02x",
		...
		if (modes) {
			...
			free(modes);
                ...
                #endif  /* was; moved to wpa_supplicant_init_wps above */
	}
#endif /* USE_INTEL_SDK */
...
struct wpa_global * wpa_supplicant_init(struct wpa_params *params)
{
	...
	if (params == NULL)
        ...
        #if 0   /* was -- but no easy way to modify params, so hardcode instead */
