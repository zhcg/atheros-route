 ...
 *
 * $Id: //depot/sw/releases/Aquila_9.2.0_U11/linux/kernels/mips-linux-2.6.15/net/sched/sch_htb.c#1 $
 */
...
#if defined(CONFIG_IMQ)||defined(CONFIG_IMQ_MODULE)
	if(skb->imq_flags&IMQ_F_ENQUEUE) cl->bstats.bytes += IMQ_ADDITION_BYTES;
#endif
	htb_activate (q,cl);
    }
...
#if defined(CONFIG_IMQ)||defined(CONFIG_IMQ_MODULE)
	if(skb->imq_flags&IMQ_F_ENQUEUE) sch->bstats.bytes += IMQ_ADDITION_BYTES;
#endif
...
next:
	...
	if (likely(skb != NULL)) {
#if defined(CONFIG_IMQ)||defined(CONFIG_IMQ_MODULE)
		int addition = skb->imq_flags&IMQ_F_ENQUEUE?IMQ_ADDITION_BYTES:0;
		if ((cl->un.leaf.deficit[level] -= (skb->len +addition)) < 0) {
#else
		...
		if (!cl->un.leaf.q->q.qlen)
			htb_deactivate (q,cl);
#if defined(CONFIG_IMQ)||defined(CONFIG_IMQ_MODULE)
		htb_charge_class (q,cl,level,skb->len + addition);
#else
