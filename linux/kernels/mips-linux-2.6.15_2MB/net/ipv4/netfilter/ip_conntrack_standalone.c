...
#if defined (CONFIG_IP_NF_CT_SCAN)
static int ct_scan_seq_show(struct seq_file *s, void *v)
{
	const struct ip_conntrack_tuple_hash *hash = v;
	const struct ip_conntrack *conntrack = tuplehash_to_ctrack(hash);
	...
	if ((IPPROTO_TCP != conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum) && 
	    (IPPROTO_UDP != conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum))
		return 0;

	if (test_bit(IPS_WAN_IN_BIT, &conntrack->status) && 
	    (conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip != conntrack->tuplehash[IP_CT_DIR_REPLY].tuple.dst.ip)) {
		if (seq_printf(s, "%s %hu %u.%u.%u.%u\n",
				(conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_TCP) ? "TCP" : "UDP",
				ntohs(conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all),
				NIPQUAD(conntrack->tuplehash[IP_CT_DIR_REPLY].tuple.src.ip)))
			return -ENOSPC;
		
	} else if (test_bit(IPS_LAN_IN_BIT, &conntrack->status) && 
		   (conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip != conntrack->tuplehash[IP_CT_DIR_REPLY].tuple.dst.ip)) {
		if (seq_printf(s, "%s %hu %u.%u.%u.%u\n",
				(conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_TCP) ? "TCP" : "UDP",
				ntohs(conntrack->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.all),
...
static struct seq_operations ct_scan_seq_ops = {
	.start = ct_seq_start,
	.next = ct_seq_next,
	.stop = ct_seq_stop,
	.show = ct_scan_seq_show
};

static int  ct_scan_open(struct inode *inode, struct file *file)
{
	struct seq_file *seq;
	struct ct_iter_state *st;
	int ret;
	...
	if (st == NULL)
		return -ENOMEM;
	ret = seq_open(file, &ct_scan_seq_ops);
	if (ret)
		goto out_free;
	seq          = file->private_data;
	seq->private = st;
...
static struct file_operations ct_scan_file_ops = {
	.owner   = THIS_MODULE,
	.open    = ct_scan_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
...
#if defined (CONFIG_IP_NF_CT_SCAN)
	struct proc_dir_entry *proc_ct_scan;
#endif
	int ret = 0;
...
#ifdef CONFIG_PROC_FS
#if defined (CONFIG_IP_NF_CT_SCAN)
	proc_ct_scan = proc_net_fops_create("ip_conntrack_scan", 0440, 
					&ct_scan_file_ops);
	if (!proc_ct_scan) goto cleanup_proc_ct_scan;
#endif
...
#if defined (CONFIG_IP_NF_CT_SCAN)
 cleanup_proc_ct_scan:
	proc_net_remove("ip_conntrack_scan");
#endif
 cleanup_init:
