...
#if defined (CONFIG_IP_NF_TARGET_TRIGGER) || defined (CONFIG_IP_NF_CT_SCAN)
#define WAN_PPP_IFNAME "ppp0"
#define WAN_IPC_IFNAME "eth1"
#endif
...
#if defined (CONFIG_IP_NF_TARGET_TRIGGER) || defined (CONFIG_IP_NF_CT_SCAN)
	if (skb->dev) {
		if (strcmp(skb->dev->name, WAN_IPC_IFNAME) == 0
		    || strcmp(skb->dev->name, WAN_PPP_IFNAME) == 0)
			conntrack->status |= IPS_WAN_IN;
#endif
#if defined CONFIG_IP_NF_CT_SCAN
		else if (strcmp(skb->dev->name, LAN_IFNAME) == 0)
			conntrack->status |= IPS_LAN_IN;
#endif
#if defined (CONFIG_IP_NF_TARGET_TRIGGER) || defined (CONFIG_IP_NF_CT_SCAN)
	}
#endif 
