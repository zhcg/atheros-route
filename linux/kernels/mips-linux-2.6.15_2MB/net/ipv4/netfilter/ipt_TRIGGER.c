...
{
        struct ipt_trigger *trig = data;
        u_int16_t dport;

        if(!(i->status & IPS_WAN_IN))
                return 0;

        if (i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_TCP)
                dport = i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.tcp.port;
        else if (i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_UDP)
                dport = i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port;
        else
                return 0;

        if (trig->rproto == i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum
            || trig->rproto == 0)
                return (trig->ports.rport[0] <= dport
                        && trig->ports.rport[1] >= dport);
        else
                return 0;
}
...
add_new_trigger(struct ipt_trigger *trig)
{
	...
	new->timeout.expires = jiffies + (new->trigger_timeout * HZ);
		...
		void *userinfo)
{
	...
	if (found) {
		...
		trigger_refresh(found, found->trigger_timeout * HZ);
