...
static int modified_src_occupied(const struct ip_conntrack_tuple *tuple,
...
#if defined(CONFIG_IP_NF_TARGET_SNATP2P)
	if ((conn->status & IPS_SNATP2P_MASK) 
	    && (conn->status & IPS_SNATP2P_DONE_MASK))
		  ...
		  unsigned int hooknum)
{
	...
	if (have_to_hash_modified_src){
		unsigned int srchash
			...
			hash_by_dst(&conntrack->tuplehash[IP_CT_DIR_REPLY].tuple) 
			: hash_by_dst(&conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
