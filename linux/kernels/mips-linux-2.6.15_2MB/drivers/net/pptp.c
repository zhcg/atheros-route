...
int pptp_connect(struct sock *sk, struct sockaddr * uaddr, int addr_len)
{
	...
	if (usin->pa.call_id != 0) {
		...
		if (ip_route_output_key(&rt, &po->pptp.fl))
			goto end;
...
static inline int pptp_output(struct sk_buff *skb)
{
...
int __pptp_xmit(struct sock *sk, struct sk_buff *skb)
{
	...
	err = ip_route_output_key(&rt, &po->pptp.fl);
	...
	skb2->h.raw = (unsigned char *)ph;
	skb2->nh.iph = iph;
	skb2->dst = dst_clone(&rt->u.dst);

	NF_HOOK(PF_INET, NF_IP_LOCAL_OUT, skb2, NULL, rt->u.dst.dev, pptp_output);
