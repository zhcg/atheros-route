...
static int ath_usb_ehci_init(struct usb_hcd *hcd)
{
	 ...
	 */
#if 0
	ath_usb_reg_wr(ATH_USB_MODE, (ath_usb_reg_rd(ATH_USB_MODE) | 3));

	printk("%s Intialize USB CONTROLLER in host mode: %x\n", __func__,
		ath_usb_reg_rd(ATH_USB_MODE));

	writel((readl(&ehci->regs->port_status[0]) | (1 << 28)),
			  ...
			  struct platform_device *pdev)
{
	...
	ath_usb_reg_wr(ATH_USB_USB_MODE, (ath_usb_reg_rd(ATH_USB_USB_MODE) | 3));

	printk("Intialize USB CONTROLLER in host mode: %x\n", ath_usb_reg_rd(ATH_USB_USB_MODE));
#endif
...
#endif
#if 0
static int __devinit ath_usb_ehci_hcd_init(void)
{
	printk("######## %s ###########\n", __func__);
#ifdef CONFIG_USB_ATH_OTG
	return usb_otg_ath_usb_probe(&ath_usb_ehci_driver);
#else
	return driver_register(&ath_usb_ehci_hcd_driver);
#endif
}

static void __devexit ath_usb_ehci_hcd_cleanup(void)
{
#ifdef CONFIG_USB_ATH_OTG
	struct ath_usb_otg *ath_usb_otg = ath_usb_get_otg();
	printk("Host Reg otg \n");
	if (ath_usb_otg) {
		struct usb_hcd *hcd = dev_get_drvdata(ath_usb_otg->dev);
		struct ehci_hcd *ehci = hcd_to_ehci(hcd);
		usb_remove_hcd(hcd);
		otg_set_host(ehci->transceiver, &hcd->self);
		dev_set_drvdata(ath_usb_otg->dev, NULL);
		usb_put_hcd(hcd);
	}
#else
	driver_unregister(&ath_usb_ehci_hcd_driver);
#endif
}

device_initcall(ath_usb_ehci_hcd_init);
module_exit(ath_usb_ehci_hcd_cleanup);
#endif
