...
#define TX_TX_IPG	0x58
	if (is_ar934x_13_or_later()) {
		ath_reg_wr(ATH_USB_PHY_CTRL5, ((ath_reg_rd(ATH_USB_PHY_CTRL5) & ~(0xff)) | TX_TX_IPG));
	}
#undef TX_TX_IPG
#endif
...
#ifdef CONFIG_MACH_AR934x
		if (!is_ar934x_13_or_later()) {
			if((ath_reg_rd(&ehci->regs->status) & STS_PCD) && 
					(hcd->state != HC_STATE_SUSPENDED) && 
					((1<<USB_PORT_FEAT_HIGHSPEED) == ehci_port_speed(ehci, 
					ehci_readl(ehci, &ehci->regs->port_status [0])))) {
				ath_reg_wr(ATH_USB_PHY_CTRL5, (ath_reg_rd(ATH_USB_PHY_CTRL5)|((1<<17) |
						(1<<22) | (1<<23))) & (~((0x3<<18) | (0x1<<20))));
				ath_reg_wr(ATH_USB_PHY_CTRL5, (ath_reg_rd(ATH_USB_PHY_CTRL5)) &
						(~(1<<17)));
			}
		}
#endif
