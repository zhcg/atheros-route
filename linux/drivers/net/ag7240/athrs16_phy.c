...
int athrs16_ioctl(void *args, int cmd)
{
//    uint32_t ar7240_revid;
    ...
    u_int32_t *rxdrops;
    if(cmd  == S26_RD_PHY) {
        etd = (struct eth_cfg_params *)ifr->ifr_data;
        if(etd->portnum != 0xf)
    ...
    else if (cmd == S26_DBG_CONFIG)
    {
        edp = (struct eth_dbg_params *)ifr->ifr_data;
...
#endif        
    } else if (cmd == S26_DBG_RESTART)
    {
        eth_vi_dbg_params.vi_dbg_restart = 1;

    } else if (cmd == S26_DBG_STATS)
    {
        rxdrops = (u_int32_t *)ifr->ifr_data;
       *(rxdrops) = eth_rx_drop;
    }else if (cmd == S26_DBG_ENABLE)
    {
        eth_vi_dbg_params.vi_dbg_cfg = *((int32_t *)ifr->ifr_data);
