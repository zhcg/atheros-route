...
uint32_t python_ioctl_vlan_addports(uint32_t vlan_id, uint32_t port_id);
uint32_t python_ioctl_vlan_delports(uint32_t vlan_id, uint32_t port_id);
uint32_t python_port_egvlanmode_set(uint32_t port_id, port_1q_egmode_t mode);
uint32_t python_port_default_vid_set(uint32_t port_id, uint32_t vlan_id);
uint32_t python_port_igmps_status_set(uint32_t port_id, BOOL enable);
uint32_t python_ioctl_enable_vlan(void);
uint32_t python_ioctl_disable_vlan(void);
uint32_t python_fdb_add(mac_addr_t addr, uint32_t port_bmap, BOOL sa_drop);
uint32_t python_fdb_del(mac_addr_t addr);
void python_clear_multi(void);
void python_set_multi(void);
uint32_t python_ioctl_vlan_qos(uint32_t vlan_id, uint32_t vlan_pri, uint32_t *port_map);
uint32_t python_fdb_add_qos(u_int8_t addr[], uint32_t prio,uint32_t portno);
