...
#define CONFIG_MACH_AR7100_PB47 1

#if defined (CONFIG_MACH_AR7100_PB47) 
#define CONFIG_MAGPIE_PHY 1
#define CONFIG_DUAL_F1E_PHY 1
#endif

#if defined (CONFIG_MAGPIE_PHY) 
#undef CONFIG_CONFIG_VITESSE_8601_7395_PHY
#endif
#include "ar7100.h"
...
static inline int
mac_has_flag(ag7100_mac_t *mac, uint32_t flag)
{
      return ((mac->mac_flags & flag) != 0);
}

static inline void
mac_set_flag(ag7100_mac_t *mac, uint32_t flag)
{
        mac->mac_flags |= flag;
            return;
}
...
#elif defined (CONFIG_AG7100_GE0_RGMII) || defined(CONFIG_AG7100_MAG_PIE_RGMII)
...
#ifdef CONFIG_MAGPIE_PHY
/* hardcoded for rgmii mode, note shift */
#define ag7100_set_mii_ctrl_mode(_mac, _spd)   do {                        \
    ar7100_reg_rmw_clear(mii_reg(_mac), (3));                          \
    ar7100_reg_rmw_set(mii_reg(_mac), ((_spd) << 1));                       \
}while(0);
#endif
/*
...
#define AG7100_RX_BUF_SIZE      \
    (AG7100_RX_RESERVE + ETH_HLEN + ETH_FRAME_LEN + 200 + ETHERNET_FCS_SIZE)
#else
#define AG7100_RX_BUF_SIZE      \
...
#define ATHR_IRQ_ENET_LINK                   AR7100_MISC_IRQ_ENET_LINK
...
{
    int head = (int)ring->ring_head, tail = (int)ring->ring_tail; 

    return ((int)(tail > head ? 0 : ring->ring_nelem) + tail - head);  
}
...
#define ag7100_intr_disable_recv(_mac)                                  \
        ag7100_reg_rmw_clear(mac, AG7100_DMA_INTR_MASK,AG7100_INTR_RX );
...
#define ag7100_intr_enable_recv(_mac)                                   \
        ag7100_reg_rmw_set(mac, AG7100_DMA_INTR_MASK,AG7100_INTR_RX);
