...
#define AG7100_MDIO_BOOT 1
#define MAGPIE_PHY_ADDR 0x1
...
int mac_location = CONFIG_AG7100_MAC_LOCATION;
module_param(mac_location, int, 0);
MODULE_PARM_DESC(mac_location, "Mac Address for Ethernet");
#endif
...
static inline int ag7100_tx_reap_thresh(ag7100_mac_t *mac)
{
    ...
    int tx_reap_thres=  AG7100_TX_REAP_THRESH;
#if defined(CONFIG_AR9100) && defined(CONFIG_AG7100_GE1_RMII)
    if(mac->speed_10t) 
        return (ag7100_ndesc_unused(mac, r) < 2);
    else 
#endif
       return (ag7100_ndesc_unused(mac, r) < tx_reap_thres); 
}
...
extern int athrmagpie_phy_is_up(int);
extern int athrmagpie_phy_is_fdx(int);
extern int athrmagpie_phy_speed(int);
extern int athr_phy_status(int, int *, int *, ag7100_phy_speed_t *);
static inline unsigned int
ag7100_get_link_status(int unit, int *link, int *fdx, ag7100_phy_speed_t *speed)
{
    switch (unit) {
        case 0: 
            *link = athrmagpie_phy_is_up(unit);
            
            if (!(*link))
                break; /* No link */

            *fdx = athrmagpie_phy_is_fdx(unit);
            *speed = athrmagpie_phy_speed(unit);
            break;
        ...
        default:
            panic("Unit number %d is wrong\n",unit);
            return -1;
    }
    /* printk("%s : Link %d fdx %d\n",__FUNCTION__, *link, *fdx); */
    return 0;    
}

extern int athrmagpie_phy_setup(unit);
static void 
ag7100_phy_setup(int unit)
{
    switch (unit) {
        case 0: 
            printk("calling magpie setup\n");
            athrmagpie_phy_setup(unit);
            break;
        
        case 1:
            printk("calling athrf1e setup\n");
            athr_phy_setup (unit);
            break;
...
ag7100_open(struct net_device *dev)
{
    ...
    ag7100_mac_t *mac = (ag7100_mac_t *)ATHR_MAC_PRIV(dev);
    int st;
    ...
    st = request_irq(mac->mac_irq, ag7100_intr, ATHR_MAC_IRQF_DISABLED, dev->name, dev);
    if (st < 0)
    {
    ...
    */
    ...
    printk("Call mac_unit as 0 %d\n",mac->mac_unit);
#ifdef CONFIG_MACH_AR7100_PB47
    if (mac->mac_unit == 0)
        printk("virtual phy setup\n");
    else
    ...
    */
    ...
    printk("CALL ag7100_check_link %llu\n",jiffies);
    ...
    mod_timer(&mac->mac_dbg_timer, jiffies);
#endif
...
ag7100_hw_setup(ag7100_mac_t *mac)
{
    ...
    ag7100_reg_wr(mac, AG7100_DMA_RX_STATUS,
            ag7100_reg_rd(mac, AG7100_DMA_RX_STATUS));
...
#if defined(CONFIG_AG7100_MAG_PIE_RGMII)
    ag7100_reg_rmw_set(mac, AG7100_MAC_CFG2, (AG7100_MAC_CFG2_HUGE_FRAME_EN | AG7100_MAC_CFG2_PAD_CRC_EN |
...
/* Let this come in its own mode, do not choose any mode here */
//#ifdef AG7100_MDIO_BOOT
 //   ar7100_reg_wr(AG7100_MII0_CONTROL,0x12);
//#endif
 //   mdelay(2);
    printk(MODULE_NAME ": cfg1 %#x cfg2 %#x = %x \n", ag7100_reg_rd(mac, AG7100_MAC_CFG1),
           ag7100_reg_rd(mac, AG7100_MAC_CFG2), ar7100_reg_rd(AG7100_MII0_CONTROL));
...
#ifndef CONFIG_MACH_AR7100_PB47
#ifdef CONFIG_MAGPIE_PHY
     /*change RGMII/MII etc mode based on current speed, 
       in case of 1000 mbps, it would be RGMII and in case of
       100 mbps it would be MII 
     */
    if(mac->mac_unit == 0)    
        ag7100_set_mii_ctrl_mode(mac, speed);
#endif
#endif
...
#else
#if defined CONFIG_MAGPIE_PHY && defined CONFIG_DUAL_F1E_PHY
        if (mac->mac_unit == 0){
#if 0       
            ag7100_set_pll(mac, 0x13441111);
#else
			ag7100_set_pll(mac, 0x01441011);
#endif
        }
...
static int check_for_dma_hang(ag7100_mac_t *mac) {
    ...
    mod_timer(&mac->mac_dbg_timer, jiffies + 2 );
    return 0;
}
#endif
...
#define ATHR_RX_POLL        0x00004000
        if (mac_has_flag(mac,ATHR_RX_POLL)) {
            ...
            {
                int status = ag7100_reg_rd(mac, AG7100_DMA_RX_STATUS);
 ...
 */
    IF_RX_STATUS_DONE(ret,AG7100_RX_STATUS_DONE,work_done,budget)
    {
        spin_lock_irqsave(&mac->mac_lock, flags);
...
process_pkts:
    ...
    */
    ...
    /*  For NAPI : Check RX PKT recvd or not */
...
ag7100_rx_replenish(ag7100_mac_t *mac)
{
    ...
    while(tail != head)
    {
        ...
        athr_mac_cache_inv((unsigned long)(bf->buf_pkt->data), AG7100_RX_BUF_SIZE);
...
#ifdef CONFIG_AG7100_MAC_LOCATION
    u8 *eep_mac_addr = (u8 *)( mac_location + (mac->mac_unit)*6);
#else
...
{
    ag7100_mac_t *mac = ATHR_MAC_PRIV(dev);
    /* struct Qdisc *sch; */
    int i;
...
ag7100_init(void)
{
    ...
    */
    ...
    for(i = 0; i < AG7100_NMACS; i++)
    {
        ...
        * watchdog task
        */
		ATHR_MAC_INIT_WORK(mac, ag7100_tx_timeout_task);
        mac_set_flag(mac,ATHR_RX_POLL);
    ...
    if (mac_has_flag(mac,ETH_LINK_INTERRUPT)) {
        ...
        dev = mac->mac_dev;
            ...
            st = request_irq(ATHR_IRQ_ENET_LINK, athr_gmac_link_intr,
                    ATHR_MAC_IRQF_DISABLED, "gmac link intr", dev);
        if (st < 0) {
            printk(MODULE_NAME ": request irq %d failed %d\n"
...
{
        uint16_t rddata=0;
        ag7100_mac_t *mac = ag7100_macs[unit];

        addr  |= (MAGPIE_PHY_ADDR << AG7100_ADDR_SHIFT);
        ag7100_reg_wr(mac, AG7100_MII_MGMT_ADDRESS, addr);
        ag7100_reg_wr(mac, AG7100_MII_MGMT_CTRL, data);
...
{
        uint16_t rddata,val=0;
        ag7100_mac_t *mac = ag7100_macs[unit];
        
        addr  |= (MAGPIE_PHY_ADDR << AG7100_ADDR_SHIFT);
        ag7100_reg_wr(mac,AG7100_MII_MGMT_CMD,0x0);
        ag7100_reg_wr(mac,AG7100_MII_MGMT_ADDRESS,addr);
        ag7100_reg_wr(mac,AG7100_MII_MGMT_CMD,0x1);
...
/* initialize the configuration needed for mdio boot operation. 
   Whin unit that should be used be passed from upper layer, 
   based on board and how mdio lines are routed. 
*/
int
mdio_boot_init(int unit)
{
    ...
    printk(MODULE_NAME "CHH: Mac base \n");
    ...
    */
    ag7100_reg_wr(mac, AG7100_MAC_CFG1, 
            (AG7100_MAC_CFG1_RX_EN | AG7100_MAC_CFG1_TX_EN));
    ag7100_reg_rmw_set(mac, AG7100_MAC_CFG2, 0x7135);
    ag7100_reg_wr(mac, AG7100_MAC_FIFO_CFG_0, 0x1f00);

    #if 0
    ag7100_reg_wr(mac, AG7100_MAC_MII_MGMT_CFG, AG7100_MGMT_CFG_CLK_DIV_20); 
    #else
    #define AG7100_MGMT_CFG_CLK_DIV_28 7
    ag7100_reg_wr(mac, AG7100_MAC_MII_MGMT_CFG, AG7100_MGMT_CFG_CLK_DIV_28);  
    #endif

    printk("Value of ag7100_mac_mii_mgmt_cfg %x\n", 
        ag7100_reg_rd(mac, AG7100_MAC_MII_MGMT_CFG));
