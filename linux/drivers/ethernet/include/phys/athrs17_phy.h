...
int athrs17_phy_is_fdx(int ethUnit, int phyUnit);
int athrs17_phy_speed(int ethUnit, int phyUnit);
BOOL athrs17_phy_is_link_alive(int phyUnit);
int athrs17_phy_setup(void *arg);
unsigned int athrs17_reg_read(unsigned int reg_addr);
void athrs17_reg_write(unsigned int reg_addr, unsigned int reg_val);
int athrs17_ioctl(struct net_device *dev, void *args, int cmd);

int athrs17_register_ops(void *arg);
int athrs17_in_reset(void *arg);
