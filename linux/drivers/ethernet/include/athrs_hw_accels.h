...
typedef int
    (*athr_hw_nat_add) (uint32_t local_ip,  uint16_t local_port,
                        uint32_t wan_ip, uint16_t wan_port,
                        uint8_t proto, int dir);

typedef void
    (*athr_hw_nat_del) (uint32_t local_ip,  uint16_t local_port, uint32_t newip,uint16_t newport,
                        uint8_t proto, int dir);

typedef int
    (*athr_hw_nat_lkp) (uint32_t ipaddr,  uint16_t port,
                        uint8_t proto, uint8_t dir,
                        uint32_t *new_ipaddr, uint16_t *new_port);
