...
static char p_duplex[][MAX_SIZ] = { "Half","Full" };
static char p_speed[][MAX_SIZ] = { "10Mbps","100Mbps" };
...
athrs27_phy_is_link_alive(int phyUnit)
{
    ...
    }else  /* no AZ... yet */ 
    {
        if (phyHwStatus & S27_STATUS_LINK_PASS) 
        {
            ...
            /* turn on LPI only when linked with az partner and in full duplex mode */
            if ((azStatus & 0x2) && (phyHwStatus & S27_STATUS_FULL_DUPLEX))
            {
...
athrs27_phy_setup(void *arg)
{
    ...
    for (phyUnit=0; (phyUnit < S27_PHY_MAX) /*&& (timeout > 0) */; phyUnit++) {
       ...
       /* Force Class A setting phys */
        ...
        /* fine-tune PHYs */
        s27_wr_phy(1, phyUnit, S27_DEBUG_PORT_ADDRESS, 0x3c);
        s27_wr_phy(1, phyUnit, S27_DEBUG_PORT_DATA, 0x1c1); 
        s27_wr_phy(1, phyUnit, S27_DEBUG_PORT_ADDRESS, 0x37);
        s27_wr_phy(1, phyUnit, S27_DEBUG_PORT_DATA, 0xd600);
...
#ifdef S27_8023AZ_FEATURE
    if (!athrPhyInfo[0].azFeature && is_ar934x_13_or_later())
    {
        /* Enable 802.3az feature */
        for (phyUnit = 0; phyUnit < S27_PHY_MAX; phyUnit++) {
            /* if this phyUnit is not in the ethUnit, skip */
            if (!S27_IS_ETHUNIT(phyUnit, ethUnit)) {
             continue;
            }
    ...
    for (phyUnit=0; phyUnit < S27_PHY_MAX; phyUnit++) {
	    ...
	    phyHwStatus = s27_rd_phy(1, phyAddr, S27_PHY_STATUS);
            ...
            if ((!(phyHwControl & S27_CTRL_AUTONEGOTIATION_ENABLE)) 
                ...
                gainedLinks++;
                linkCount++;
                printk("enet%d port%d up %s %s duplex\n" ,ethUnit, phyUnit,
                                          p_speed[(phyHwStatus >> 14) & 1],
                                        p_duplex[(phyHwStatus >> 13) & 1 ]);
                DRV_PRINT(DRV_DEBUG_PHYCHANGE,("\nenet%d port%d up\n",
                                               ethUnit, phyUnit));
