...
int athrs16_in_reset(void *arg)
{
     ...
     for (phyUnit=0; phyUnit < 6 ; phyUnit++) {
         ...
         if ((in_reset & (1 << 12))) {
            ...
            printk(MODULE_NAME ": S16 in reset...wrk queue schld\n");
    	    schedule_work(&mac->mac_reset_wqueue);
...
/* since free_irq() (athr_stop_dev) is called from timer handler(interrupt contex) from the s16 phy driver ,we have seen  call trace. 
 To avoid this( call trace),have implemeted the workqueue to restart the mac from s16 and will be scheduled from athrs16_in_reset()
 */ 
void
athr_gmac_reset_task(ATHR_MAC_RESET_TASK_ARG)
{  
    ATHR_MAC_RESET_TASK_MAC(); 
    struct net_device *dev = mac->mac_dev;

    athr_stop_dev(dev);
    athr_open_dev(dev);
// #define S16_DEBUG 
    printk(" mac is restarted through WQ frm S16...\n");
// #endif 
}
