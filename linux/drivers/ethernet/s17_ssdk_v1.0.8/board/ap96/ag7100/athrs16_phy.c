...
#ifdef FULL_FEATURE
{
    ...
    //athrs16_reg_write(0x98, 0x1aaaa);  // delete this function
    val = 0x1aaaa;
    isis_reg_set(0, 0x98, & val, sizeof(uint32_t));

    /*PORT0 TX_HEADER_MODE_0 all frame with header*/
    //athrs16_reg_write(0x9c, 0x2);  // delete this function
    val = 0x2;
    isis_reg_set(0, 0x9c, & val, sizeof(uint32_t));
    
    /*cpu port en*/    
    //athrs16_reg_write(0x620, (athrs16_reg_read(0x620)|0x400)); // delete this function
    isis_reg_get(0,0x620, & val, sizeof(uint32_t)); 
    val |= 0x400;
    isis_reg_set(0, 0x620, (char *)& val, sizeof(uint32_t));
#endif
