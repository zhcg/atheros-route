...
static struct nf_conn *ct_save[CT_SAVE_MAX];
...
static void
napt_death_add(struct nf_conn *ct, fal_napt_entry_t *napt_entry)
{
	int i= 0;
	for(i = 0; i< CT_SAVE_MAX; i++) {
		if(ct_save[i] == ct) {
...
static void
napt_death_replace(struct nf_conn *ct, fal_napt_entry_t *napt_entry)
{
	del_timer(&ct->timeout);
...
static void
napt_death(struct nf_conn *ct, fal_napt_entry_t *napt_entry)
{
#ifdef AGE_MODE
#ifdef HEADER_CONFIG
            ...
            const struct net_device *out, int (*okfn) (struct sk_buff *))
{
    ...
    struct tftphdr *tftph = ((struct tftphdr *) udp_hdr(skb) + 28);
    fal_napt_entry_t napt_entry;
    ...
    ssaddr = (uint8_t *) (&ct->tuplehash[0].tuple.src.u3.ip);
    tsaddr = (uint8_t *) (&ct->tuplehash[0].tuple.dst.u3.ip);
    sdaddr = (uint8_t *) (&ct->tuplehash[1].tuple.src.u3.ip);
    tdaddr = (uint8_t *) (&ct->tuplehash[1].tuple.dst.u3.ip);
    
    help = nfct_help(ct);
    nat = nfct_nat(ct); 
    
    if ((ct->status & IPS_NAT_MASK) == IPS_SRC_NAT) {   //snat
        usaddr = ct->tuplehash[0].tuple.src.u3.ip;
        usport = ct->tuplehash[0].tuple.src.u.all;
        udaddr = ct->tuplehash[0].tuple.dst.u3.ip;
        udport = ct->tuplehash[0].tuple.dst.u.all;
        tusaddr = ct->tuplehash[1].tuple.dst.u3.ip;
        tusport = ct->tuplehash[1].tuple.dst.u.all;
    } else {                    //dnat
        usaddr = ct->tuplehash[1].tuple.src.u3.ip;
        usport = ct->tuplehash[1].tuple.src.u.all;
        udaddr = ct->tuplehash[1].tuple.dst.u3.ip;
        udport = ct->tuplehash[1].tuple.dst.u.all;
        tusaddr = ct->tuplehash[0].tuple.dst.u3.ip;
            ...
            if (!strcmp(help->helper->name, "pptp")) {
                printk
                    ("PPTP Session state %d, Call state %d, PAC Call ID %d to %d, PNS Call ID %d to %d\n",
                     help->help.ct_pptp_info.sstate,
                     help->help.ct_pptp_info.cstate,
                     ntohs(help->help.ct_pptp_info.pac_call_id),
                     ntohs(nat->help.nat_pptp_info.pac_call_id),
                     ntohs(help->help.ct_pptp_info.pns_call_id),
                     ntohs(nat->help.nat_pptp_info.pns_call_id));
                if (((help->help.ct_pptp_info.sstate == PPTP_SESSION_CONFIRMED)
                     && (help->help.ct_pptp_info.cstate == PPTP_CALL_OUT_REQ))
                    || (help->help.ct_pptp_info.sstate == PPTP_SESSION_STOPREQ)) {
                    memset((void *) &napt_entry, 0, sizeof (fal_napt_entry_t));
                    ...
                    napt_entry.src_addr = usaddr;
                    napt_entry.src_port = help->help.ct_pptp_info.pns_call_id;
                    napt_entry.dst_addr = udaddr;
                    napt_entry.dst_port = 0;
                    napt_entry.trans_addr = 15;
                    napt_entry.trans_port =
                        nat->help.nat_pptp_info.pns_call_id;
                    napt_entry.status = NAPT_AGE;
                }
                if ((help->help.ct_pptp_info.sstate == PPTP_SESSION_CONFIRMED)
                    && (help->help.ct_pptp_info.cstate == PPTP_CALL_OUT_REQ)) {
                    ...
                    isis_napt_add(0, &napt_entry);
					napt_death(ct, &napt_entry);
                } else if (help->help.ct_pptp_info.sstate == PPTP_SESSION_STOPREQ) {
...
arp_in(unsigned int hook,
	...
	memcpy( &athr_header, skb->head, sizeof(aos_header_t));
	...
	arp_entry.intf_id = intf_id;
        ...
        if(rv == 0)
	    printk("##(port:%d)##isis_ip_host_add #%.x##\n", athr_header.sport, *sip);
    }
    return NF_ACCEPT;
}
...
nat_del(unsigned int newnum, unsigned int oldnum) {
  ...
  delete:
    ...
    if (gentry->next_offset == 216) {
        ...
        xport = (struct xt_multiport *)((uint8_t *) gtarget - sizeof (*xport) - 16);
#if 0 /* comment out by jhung */
...
nat_set_ctl(struct sock *sk, int cmd, void __user * user, unsigned int len) {
    ...
    struct xt_entry_target *xtarget = NULL;
    struct nf_nat_multi_range_compat *nfmrange = NULL;
    struct xt_multiport *xport = NULL;
    struct xt_tcp *xtcp = NULL;
    struct xt_udp *xudp = NULL;
    ...
    nfmrange = (struct nf_nat_multi_range_compat *)((uint8_t *) xtarget + sizeof (*xtarget));
    smac = (uint8_t *)(&(ientry->ip.src.s_addr));
    dmac = (uint8_t *)(&(ientry->ip.dst.s_addr));
    if (j == 2) {               //gre need test
        if (ientry->ip.proto == P_TCP)
            xtcp = (struct xt_tcp *)((uint8_t *) xtarget - sizeof (*xtcp));
        else if (ientry->ip.proto == P_UDP)
            xudp = (struct xt_udp *)((uint8_t *) xtarget - sizeof (*xudp) - 2);
    }
    if (j == 3) {
        xport = (struct xt_multiport *)((uint8_t *) xtarget - sizeof (*xport) - 16);
#if 0 /* comment out by jhung */        
        if (!xport->count || (xport->count > IPT_MULTI_PORTS))
            goto normal;
#endif
...
#if 0 /* do not register NAT functions, jhung */
    nat_init();
#endif
#ifdef HEADER_CONFIG	
