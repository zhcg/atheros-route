...
athr_gmac_hw_setup(athr_gmac_t *mac)
{
    ...
    /* Fix for evid 98231 */
    /* Ping from WAN port fails on ap121 when AP scans(RPT mode). 
     * False carrier errors are shown only on wan port when ap scans
     *  0x66bc2 was written in this reg which drops false carrier errors
     * Don't drop packets which are seen with this error */
    else if (is_s26() && mac->mac_unit == 0) {
       athr_gmac_reg_wr(mac, ATHR_GMAC_FIFO_CFG_5, 0x66bc6);
    }   
    else {
       /* 
