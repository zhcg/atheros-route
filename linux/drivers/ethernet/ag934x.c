...
#define ATHR_GMAC_PORT_LED(p)                  (20 + p)
   ...
   athr_gmac_t *mac = (athr_gmac_t *)arg;
    ...
    if ( is_ar934x()) {
        ...
        if (is_s27() && !is_ar934x_13_or_later()) {
            mac_set_flag(mac, ETH_SOFT_LED);
        } else if (is_ar934x_13_or_later()) {
            mac_set_flag(mac, ATHR_AP123_GPIO_SWAP);
        }
        ...
        if (mac_has_flag(mac, ETH_SOFT_LED)) {
            ...
            if (is_s27()) {
                if (is_ar934x_13_or_later() && mac_has_flag(mac, ATHR_AP123_GPIO_SWAP)) {
                    athr_reg_wr(ATHR_GPIO_OUT_FUNCTION4,
                        ((athr_reg_rd(ATHR_GPIO_OUT_FUNCTION4) & 0xffff) | (0x2a2d << 16)));

                    athr_reg_wr(ATHR_GPIO_OUT_FUNCTION5,
                        ((athr_reg_rd(ATHR_GPIO_OUT_FUNCTION5) & (0xff << 24)) | 0x292c2b));
                } else {
