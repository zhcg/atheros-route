	...
	int ret = -1;
   	...
   	spin_lock(&athrs_nat_op_lock);
	     ...
	     ret = athrs_nat_hwtbl_ops_status(cmd,portdir);
            ...
            printk(" INGRESS NAT ADD/DEL :cmd = %d  INFO  DW0 = 0x%x DW1 = 0x%x DW2= 0x%x DW3 = %x \n",cmd,
                          athr_reg_read_lut(IG_INFO_DW0_REG, portdir),
                          athr_reg_read_lut(IG_INFO_DW1_REG, portdir),
                          athr_reg_read_lut(IG_INFO_DW2_REG, portdir),
                          athr_reg_read_lut(IG_INFO_DW3_REG, portdir));
            printk("INGRESS : NAT ADD/DEL  : INFO %X   \n",athr_reg_read_lut(IG_KEY_DW0_REG,portdir));
#endif        
...
#ifdef CONFIG_ATHRS_HW_NAT_DEBUG
              printk(" INFO wan icmp_id = %X \n",egrs->info.wan_icmp_id); 
              printk(" INFO wan ip_idx  = %X \n",egrs->info.wan_ip_idx); 
              printk(" INFO wan l2c st = %X \n",egrs->info.l2_con_st); 
              printk(" KEY  lan ip - %X \n  ",egrs->lan_ip_addr);
              printk(" KEY  PROTO proto  - %X \n  ",egrs->prot_key);
              printk(" KEY  ICMP ID  - %X \n  ",egrs->lan_icmp_id);
#endif 
              athr_reg_write_lut(EG_KEY_DW0_REG, (EG_KEY_DW0_LAN_ADDR(egrs->lan_ip_addr) |
                                 EG_KEY_DW0_PROT(egrs->prot_key) |
...
#ifdef CONFIG_ATHRS_HW_NAT_DEBUG
             printk("EGRESS: NAT ADD : KEYYYYY : CMD = %d  DW0 = 0x%x DW1 = 0x%x \n",cmd,
                  athr_reg_read_lut(EG_KEY_DW0_REG, portdir),
                  athr_reg_read_lut(EG_KEY_DW1_REG, portdir));
             printk("EGRESS: NAT ADD : INFOOOOO  %X \n",athr_reg_read_lut(EG_INFO_INS_DW0_REG,portdir));
#endif 
...
#ifdef CONFIG_ATHRS_HW_NAT_DEBUG
		printk(" 1 IGRESS wan_icmp_id = %X \n ",ingrs->wan_icmp_id);
		printk(" 2 IGRESS protokey    = %X \n ",ingrs->prot_key);
		printk(" 3 IGRESS wan_ip_idx  = %X \n",ingrs->wan_ip_idx);
#endif 
...
#ifdef CONFIG_ATHRS_HW_NAT_DEBUG
		printk(" IGRESS wan_icmp_id = %X \n ",ingrs->wan_icmp_id);
		printk(" IGRESS protokey    = %X \n ",ingrs->prot_key);
		printk(" IGRESS wan_ip_idx  = %X \n",ingrs->wan_ip_idx);
#endif 
                athr_reg_write_lut(IG_KEY_DW0_REG,
...
#ifdef CONFIG_ATHRS_HW_NAT_DEBUG
            printk(" INGRESS NAT ADD/DEL INFOOOOO :cmd = %d  DW0 = 0x%x DW1 = 0x%x DW2= 0x%x DW3 = %x \n",cmd,
                          athr_reg_read_lut(IG_INFO_DW0_REG, portdir),
                          athr_reg_read_lut(IG_INFO_DW1_REG, portdir),
                          athr_reg_read_lut(IG_INFO_DW2_REG, portdir),
                          athr_reg_read_lut(IG_INFO_DW3_REG, portdir));
           printk("INGRESS : NAT ADD/DEL  : KEYYY %X  \n",athr_reg_read_lut(IG_KEY_DW0_REG,portdir));
#endif 
