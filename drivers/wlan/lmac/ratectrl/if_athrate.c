...
ath_rate_tx_complete(struct ath_softc *sc, struct ath_node *an,
                     ...
                     u_int8_t ac, int nframes, int nbad, int nHeadFail, int nTailFail, int rts_retry_limit)
#else
void
ath_rate_tx_complete(struct ath_softc *sc, struct ath_node *an,
                     struct ath_desc *ds, struct ath_rc_series rcs[],
...
u_int32_t
ath_rate_getmaxphyrate(struct ath_softc *sc, struct ath_node *an,uint32_t shortgi)
{
    struct atheros_node *oan = an->an_rc_node;
    struct atheros_softc *asc = oan->asc;
    struct ath_vap         *avp = oan->avp->athdev_vap;
    ...
    u_int8_t sgi_phy_options[2 /* 20 MHz vs. 40 MHz */]
                            [3 /* single, double, triple stream*/] = {
                {
...
ath_rate_max_tx_chains(struct ath_softc *sc, u_int8_t rix, u_int16_t bfrcsflags)
{
        ...
        sc->sc_ah->ah_magic == 0x19741014)
    {
        ...
        pRateTable = (RATE_TABLE_11N*)asc->hwRateTable[sc->sc_curmode];
        if (sc->sc_reg_parm.cddSupport) {
            switch (sc->sc_curchan.channel)
            {
                case 5180 /* chan 36 */:
                case 5200 /* chan 40 */:
                case 5220 /* chan 44 */:
...
EXPORT_SYMBOL(ath_rate_smartant_prepare_rateset);
...
/* 
 * Prepare rate sets on which we need to do training.
 * form rate sets based on number of stream rates
 * right now 3 rate sets, each has max 3 rates
 */
void ath_rate_smartant_prepare_rateset(struct ath_softc *sc, struct ath_node *an, void *prtset)

{
    struct atheros_node *oan = an->an_rc_node;
    struct atheros_softc *asc = oan->asc;
    struct ath_smartant_rateset *pRateSet = (struct ath_smartant_rateset *) prtset;
    TX_RATE_CTRL *pRc  = (TX_RATE_CTRL *)(oan);
    const RATE_TABLE_11N *pRateTable;
    u_int8_t rt,prev_rtcode = 0;
    int i = 0 ;
    ...
    if (sc->sc_ah->ah_magic == 0x19641014 ||
        sc->sc_ah->ah_magic == 0x19741014)
    {
    pRateSet[0].num_of_rates = 0;
    pRateSet[1].num_of_rates = 0;
    pRateSet[2].num_of_rates = 0;

    rt = (pRateTable->info[pRc->maxValidRate-1].rateCode)&0x7f;
    rt = (rt & 070)>>3;

    for(i = pRc->maxValidRate-1; i >= 0; i--) 
    {
        rt = (pRateTable->info[pRc->validRateIndex[i]].rateCode)&0x7f;
        rt = (rt & 070)>>3;
        /* if same rate code exists for two different index */
        if (prev_rtcode == pRateTable->info[pRc->validRateIndex[i]].rateCode)
            continue;

        if (!IS_HT_RATE(pRateTable->info[pRc->validRateIndex[i]].rateCode))
            continue;

/*   Donot restrict the rate to only 3; if we are not able to get enough PER estimates it is worth
 *   go along the rates down untill we found an antenna combination      
 *      if (pRateSet[rt].num_of_rates >= 3)
 *           continue;
 */            

        pRateSet[rt].rates[pRateSet[rt].num_of_rates].ratecode =  pRateTable->info[pRc->validRateIndex[i]].rateCode;
        prev_rtcode = pRateTable->info[pRc->validRateIndex[i]].rateCode;
        pRateSet[rt].rates[pRateSet[rt].num_of_rates].rateindex = pRc->validRateIndex[i];
#if SMARTANTENNA_DEBUG        
        printk("ratset: %d : ratecode:0x%02x rtindex:%d\n", rt,pRateSet[rt].rates[pRateSet[rt].num_of_rates].ratecode
                            ,pRateSet[rt].rates[pRateSet[rt].num_of_rates].rateindex);
#endif        
        pRateSet[rt].num_of_rates++;
    }
