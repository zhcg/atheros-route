...
static int dfs_check_chirping_enhanced (struct ath_softc *sc, struct ath_buf *bf, struct ath_rx_status *rxs, int is_ctl, int is_ext, int *slope, int *is_dc);
	...
	for (n=0;n<dfs->dfs_rinfo.rn_numbin5radars; n++) {
        ...
        for(i = i1 + 1; i < br->br_numelems; i++){
			...
			if( br->br_elems[this].be_ts >= br->br_elems[prev].be_ts ) {
		        ...
		        DFS_DPRINTK(sc, ATH_DEBUG_DFS2," pri=%llu this.ts=%llu prev.ts=%llu\n", (unsigned long long)pri, (unsigned long long)br->br_elems[this].be_ts, (unsigned long long)br->br_elems[prev].be_ts);		        
			if(( (pri >= DFS_BIN5_PRI_LOWER_LIMIT) && (pri <= DFS_BIN5_PRI_HIGHER_LIMIT))) {  //pri: pulse repitition interval in us
				...
				if( width_diff <= DFS_BIN5_WIDTH_MARGIN ) {
					...
					if( rssi_diff <= DFS_BIN5_RSSI_MARGIN ) {
						bursts++;
	                                    ...
	                                    DFS_DPRINTK(sc,ATH_DEBUG_DFS2,"%s %d Bin5 rssi_diff=%d\n", __func__, __LINE__, rssi_diff);
                                            }
		                } else {
	                                    DFS_DPRINTK(sc,ATH_DEBUG_DFS2,"%s %d Bin5 width_diff=%d\n", __func__, __LINE__, width_diff);
                                        }
	            ...
	            DFS_DPRINTK(sc,ATH_DEBUG_DFS2,"%s %d Bin5 PRI check fail pri=%llu\n", __func__, __LINE__,(unsigned long long)pri);
            }
		    prev = this;
		}
		...
		if ( bursts >= br->br_pulse.b5_threshold) {
 				...
 				*
                                ...
                                if( average_diff > DFS_BIN5_WIDTH_MARGIN ) {
                                    ...
                                    DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "bursts=%u numevents=%u total_width=%d average_width=%d total_diff=%d average_diff=%d\n", bursts, numevents, total_width, average_width, total_diff, average_diff);

                                } 
				*/
                           DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "bursts=%u numevents=%u total_width=%d average_width=%d total_diff=%d average_diff=%d\n", bursts, numevents, total_width, average_width, total_diff, average_diff);
                           printk ("bin 5 radar detected, bursts=%d", bursts);
                           return 1;
                        }
		}
	}
...
*/
int dfs_check_chirping (struct ath_softc *sc, struct ath_buf *bf, struct ath_rx_status *rxs, int is_ctl, int is_ext, int *slope, int *is_dc)
{
#define FFT_LEN 70
...
#define MIN_CHIRPING_SLOPE 4
    ...
    if (sc->sc_dfs->sc_dfs_use_enhancement) {
        return (dfs_check_chirping_enhanced(sc, bf, rxs, is_ctl, is_ext, slope, is_dc));
    }
    ...
    *is_dc = 0;
    ...
    fft_data_ptr = ((u_int8_t*)bf->bf_vdata);
    ...
    if (is_ctl) {
        ...
        fft_data_ptr = ((u_int8_t*)bf->bf_vdata) + (FFT_LEN*(num_fft_packets - 1));
        ctl_high1 = *(fft_data_ptr+FFT_LOWER_BIN_MAX_INDEX_BYTE) >> 2;

        // second last packet with 0th packet
        fft_data_ptr = ((u_int8_t*)bf->bf_vdata) + (FFT_LEN*(num_fft_packets - 2));
...
#define UPPER_MAG_BYTE_HT20     29
    ...
    int datalen;            /* number of bytes in FFT data */
    int i;
    int fft_start;
    int chirp_found;
    ...
    if (IS_CHAN_HT40(&sc->sc_curchan)) {
        ...
        if (IS_CHAN_HT40_MINUS(&sc->sc_curchan)) {
            temp   = is_ctl;
            is_ctl = is_ext;
            is_ext = temp;
        }
    ...
    if ((ptr == NULL) || (datalen == 0)) {
        DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "%s: FFT buffer pointer is null or size is 0\n", __func__);
        return 0;
    }

    num_fft_packets = (datalen - 3) / num_fft_bytes;
    if (num_fft_packets < (NUM_DIFFS + DELTA_STEP)) {
        DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "datalen = %d, num_fft_packets = %d, too few packets... (exiting)\n", datalen, num_fft_packets);
        return 0;
    }
    ...
    for (i = 0; i < (NUM_DIFFS + DELTA_STEP); i++) {
        ...
        DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "i=%d, max_index[i]=%d, max_index_lower[i]=%d, max_index_upper[i]=%d\n", i, max_index[i], max_index_lower[i], max_index_upper[i]);
    }
    ...
    */
    for (i = 0; i < NUM_DIFFS; i++) {
        ...
        DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "i=%d, delta_peak[i]=%d, delta_diff=%d\n", i, delta_peak[i], delta_diff);
    }


    if (chirp_found)
    {
        DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "%s: CHIRPING_BEFORE_STRONGBIN_YES\n", __func__);
    } else {
        DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "%s: CHIRPING_BEFORE_STRONGBIN_NO\n", __func__);
    }
    ...
    */
    ...
    for (i = 0; (i < (NUM_DIFFS + DELTA_STEP)) && (chirp_found == 1); i++) {
        ...
        DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "i=%d, computed bin_count=%d\n", i, bin_count);
    }

    if (chirp_found)
    {
        DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "%s: CHIRPING_YES\n", __func__);
    } else {
        DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "%s: CHIRPING_NO\n", __func__);
    }
    return chirp_found;
...
u_int8_t dfs_retain_bin5_burst_pattern(struct ath_softc *sc, u_int32_t diff_ts, u_int8_t old_dur)
{
    ...
    if (diff_ts < 50) {
	DFS_DPRINTK(sc,ATH_DEBUG_DFS1,"%s SPLIT pulse diffTs=%u dur=%d (old_dur=%d)\n", __func__, diff_ts, dfs->dfs_rinfo.dfs_last_bin5_dur, old_dur);
    }
	...
	DFS_DPRINTK(sc,ATH_DEBUG_DFS1,"%s this pulse belongs to the same burst as before, give it same dur=%d (old_dur=%d)\n", __func__, dfs->dfs_rinfo.dfs_last_bin5_dur, old_dur);
...
/* Chirping pulses may get cut off at DC and report lower durations. This function
 will compute a suitable random duration for each pulse. Duration must be between 50 and 100 us, but remember that in ath_process_phyerr() which calls this function,
we are dealing with the HW reported duration (unconverted). dfs_process_radarevent() 
will actually convert the duration into the correct value */
