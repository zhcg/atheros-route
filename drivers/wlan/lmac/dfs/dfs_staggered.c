                             ...
                             u_int32_t deltaT, u_int32_t width)
{
	...
	int numpulseshigh=0, numpulsesmid=0, numpulsestemp=0;
	    ...
	    if (midscore == 0) {
            ...
            numpulsestemp = dfs_bin_pri_check(sc, rf, dl, highestscore, refpri, refdur,0, highestpri);
	    numpulseshigh = numpulsestemp;
            numpulsestemp = dfs_bin_pri_check(sc, rf, dl, highestscore, refpri, refdur,0, highestpri + midpri);
	    if (numpulsestemp > numpulseshigh) {
		    numpulseshigh = numpulsestemp;
	    }
            numpulsestemp = dfs_bin_pri_check(sc, rf, dl, highestscore, refpri, refdur,0, highestpri + midpri + lowestpri);
	    if (numpulsestemp > numpulseshigh) {
		    numpulseshigh = numpulsestemp;
	    }
            ...
            //numpulsesmid = dfs_bin_pri_check(sc, rf, dl, midscore, refpri, refdur,0, 1);
            numpulsestemp = dfs_bin_pri_check(sc, rf, dl, midscore, refpri, refdur,0, midpri);
	    numpulsesmid = numpulsestemp;
            numpulsestemp = dfs_bin_pri_check(sc, rf, dl, midscore, refpri, refdur,0, highestpri + midpri);
	    if (numpulsestemp > numpulsesmid) {
		    numpulsesmid = numpulsestemp;
	    }
            numpulsestemp = dfs_bin_pri_check(sc, rf, dl, midscore, refpri, refdur,0, highestpri + midpri + lowestpri);
	    if (numpulsestemp > numpulsesmid) {
		    numpulsesmid = numpulsestemp;
	    }
            ...
            numpulseslow = dfs_bin_pri_check(sc, rf, dl, lowestscore, refpri, refdur,0, 1);
	    */
            DFS_DPRINTK(sc, ATH_DEBUG_DFS2, "numpulseshigh=%d, numpulsesmid=%d\n",numpulseshigh, numpulsesmid);

	if ( (numpulseshigh >= rf->rf_threshold) && (numpulsesmid >= rf->rf_threshold) ) {
        /*if (numpulses >= rf->rf_threshold) {*/
            found = 1;
            DFS_DPRINTK(sc, ATH_DEBUG_DFS, "MATCH filter=%u numpulseshigh=%u numpulsesmid= %u thresh=%u\n", rf->rf_pulseid, numpulseshigh, numpulsesmid, rf->rf_threshold);     
        }
        return found;
    }
