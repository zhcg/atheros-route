...
int dfs_bin_fixedpattern_check(struct ath_softc *sc, struct dfs_filter *rf, u_int32_t dur, int ext_chan_flag)
{
        ...
        if (numpulses >= dfs_get_filter_threshold (sc, rf, ext_chan_flag)) {
           DFS_DPRINTK(sc, ATH_DEBUG_DFS1, "%s FOUND filterID=%u numpulses=%d unadj thresh=%d\n", __func__, rf->rf_pulseid, numpulses, rf->rf_threshold);
           return 1;
        }
        else 
                return 0;
                             ...
                             u_int32_t deltaT, u_int32_t width, int ext_chan_flag)
{
        ...
        for (n=0;n<dl->dl_numelems; n++) {
                ...
                if( refpri == 0)
                        continue;
                if (refpri < rf->rf_maxpri) { // use only the valid PRI range for finding high-scorer.
        ...
        numpulses = dfs_bin_pri_check(sc, rf, dl, score[scoreindex], refpri, refdur, ext_chan_flag, refpri);
        if (numpulses >= dfs_get_filter_threshold (sc, rf, ext_chan_flag)) {
            found = 1;
            DFS_DPRINTK(sc, ATH_DEBUG_DFS1, "ext_flag=%d MATCH filter=%u numpulses=%u thresh=%u refpri=%d primargin=%d\n", ext_chan_flag, rf->rf_pulseid, numpulses,rf->rf_threshold, refpri, primargin);     
            printk("ext_flag=%d MATCH filter=%u numpulses=%u thresh=%u refpri=%d primargin=%d\n", ext_chan_flag, rf->rf_pulseid, numpulses,rf->rf_threshold, refpri, primargin);     
            dfs_print_delayline(sc, &rf->rf_dl);
            dfs_print_filter(sc, rf);
        }
        return found;
}
...
{
        u_int32_t searchpri, searchdur, searchrssi, deltapri=0, deltapri1, deltapri2, deltadur, averagerefpri=0;
