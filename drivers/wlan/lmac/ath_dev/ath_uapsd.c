...
#define ATH_FRAG_PER_QOSNULL_MSDU                   1
    ...
    TAILQ_FOREACH(bf, &sc->sc_uapsdqnulbf, bf_list)
    {
        ...
        wbuf = sc->sc_ieee_ops->uapsd_allocqosnullframe(sc->sc_ieee);
        if (wbuf == NULL) {
            error = -ENOMEM;
...
ath_tx_uapsdqnulbf_complete(struct ath_softc *sc, struct ath_buf *bf, bool held_uapsd_lock)
{
     ...
     */
    if (bf->bf_node == NULL) {
        DPRINTF(sc, ATH_DEBUG_UAPSD, "%s: Error: bf_node is null. bf=%p\n", __func__, bf);
    }
    else {
        struct ath_node *an;
...
ath_uapsd_sendqosnull(struct ath_softc *sc, struct ath_node *an, u_int8_t ac)
{
    ...
    ath_rate_findrate(sc, an, AH_TRUE, 0, ATH_11N_TXMAXTRY,
                      ATH_RC_PROBE_ALLOWED, ac, bf->bf_rcs, &prate, AH_FALSE,bf->bf_flags);
 ...
 */
int
ath_process_uapsd_trigger(ath_dev_t dev, ath_node_t node, u_int8_t maxsp, u_int8_t ac, u_int8_t flush, u_int8_t maxqdepth)
{
    struct ath_node *an = ATH_NODE(node);
    ...
    ath_rate_findrate(sc, an, AH_TRUE, 0, ATH_11N_TXMAXTRY,
                      ATH_RC_PROBE_ALLOWED, ac, rcs, &prate, AH_FALSE,0);
...
#ifdef ATH_SWRETRY
    an->an_uapsd_num_addbuf = count;
    DPRINTF(sc, ATH_DEBUG_SWR, "%s: %d frames sent to sc_uapsdq\n", __func__, count);
#endif
    /*
