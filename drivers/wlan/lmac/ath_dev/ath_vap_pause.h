...
#define    VAP_PAUSE_SYNCHRONIZATION_TIMEOUT  5000  /* msec */
...
#define ath_vap_pause_txq_use_inc(_sc) do  { \
    ...
    spin_lock(&(_sc)->sc_vap_pause_lock);            \
    while(_sc->sc_vap_pause_in_progress) {       \
        spin_unlock(&(_sc)->sc_vap_pause_lock);  \
        OS_DELAY(10);                            \
        ++ iter_count;                           \
...
#define ath_vap_pause_in_progress(_sc) _sc->sc_vap_pause_in_progress 
