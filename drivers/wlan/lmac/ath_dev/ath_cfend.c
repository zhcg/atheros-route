...
ath_tx_cfend_init(struct ath_softc *sc)
{
    ...
    if (bf) {
        DPRINTF(sc, ATH_DEBUG_XMIT,
                "%s * allocated cfend pointer at %p \n", __func__, bf);
    }
        ...
        ath_bufhead *bf_q)
{
    ...
    if (!bf) {
        ...
        DPRINTF(sc, ATH_DEBUG_XMIT,
                "%s freeing cfend buffer bf %p, wbuf %p n",
...
#define CFEND_OFFSET 500
    ...
    /* reset this to help next aggregate processing */
    sc->sc_ah->ah_rxNumCurAggrGood = 0;
}
