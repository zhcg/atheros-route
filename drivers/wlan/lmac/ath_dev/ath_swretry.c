...
{               
    ...
    do {
        ...
        if (!tid->an->an_tim_set && (tid->an->an_flags & ATH_NODE_PWRSAVE) && ((sc->sc_ieee_ops->get_pwrsaveq_len(tid->an->an_node)==0)))
        {
...
#else
            DPRINTF(sc, ATH_DEBUG_SWR, "%s: SWseqno%d\n",__func__, 
