...
#define DEFAULT_TIMEOUT_VALUE 0xD
...
#if !AP_MULTIPLE_BUFFER_RCV	// if define AP_MULTIPLE_BUFFER_RCV the CRC handler will be ourt side
    ...
    if (rxs->rs_more) {
        ...
        if (sc->sc_rx_wbuf_waiting)
        {
            ...
            ATH_SET_RX_CONTEXT_BUF(sc->sc_rx_wbuf_waiting, bf);
...
rx_next:
   ...
   /*setting Rx interupt uncondionally as when we schedule tasklet 
    ...
    if(!TAILQ_EMPTY(&rxedma->rxqueue)){
        sc->sc_rx_work=1;
        sc->sc_imask &= ~(HAL_INT_RXLP|HAL_INT_RXHP);
