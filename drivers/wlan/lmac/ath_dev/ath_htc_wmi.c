...
static a_uint16_t ath_htc_can_xmit(struct ath_softc *sc, a_uint16_t ac);
static void ath_txep_enqueue(struct ath_txep *txep, struct ath_node *an, 
        a_int32_t tidno,     int switch_tid );
...
static struct ath_buf * ath_host_buf_alloc(struct ath_softc *sc, a_uint8_t ac);
...
static struct ath_buf * ath_buffer_setup(struct ath_softc *sc, 
        ath_bufhead *head , const char *name, a_int32_t nbuf);
...
#ifdef ATH_HTC_SG_SUPPORT
    if(rxhdr->is_seg){
        int length = be32_to_cpu(rxhdr->sg_length)- sizeof(struct rx_frame_header);
    ...
    *sg_recv = 0 ;
    ...
    if((rxhdr->is_seg!=0) && (rxhdr->numseg >1)){
        ...
        if(sc->sc_sgrx.Onprocess ==0){
            if(rxhdr->sgseqno !=0){
                /*adf_os_print("1st packet got lost %d \n ",rxhdr->sgseqno);*/
                wbuf_free(wbuf);
                return -1 ;
            }
            ath_clear_sg_state(sc);
            ...
            if(sc->sc_sgrx.nextseq != rxhdr->sgseqno){
                adf_os_print(" DROP amsduamsdu required seq %d   got seq %d \n",sc->sc_sgrx.nextseq,rxhdr->sgseqno);
                adf_os_print("msdu Pkt 1 len  %d total %d  glblseq %d \n",rxhdr->sg_length,rxhdr->numseg,rxhdr->res);
                wbuf_free(wbuf);
                ath_clear_sg_state(sc);
                return -1 ;
            }
        ...
        if(sc->sc_sgrx.Onprocess == 0){
            ...
            sc->sc_sgrx.nextseq++;
            adf_nbuf_cat(sc->sc_sgrx.Firstseg, wbuf);                            
...
ath_htc_rx(ath_dev_t dev, wbuf_t wbuf)
{
    ...
    if(adf_nbuf_queue_len(&sc->sc_rxwbufhead) >ATH_RXBUF*2){
//        adf_os_print("Freeing Rx packet due to Max Rx buf reached  \n");
        wbuf_free(wbuf);
        return;
    }

    adf_nbuf_queue_add(&sc->sc_rxwbufhead,wbuf);
...
ath_htc_rx_tasklet(ath_dev_t dev, int flush)
{
    ...
    wbuf = adf_nbuf_queue_remove(&sc->sc_rxwbufhead);
...
ath_htc_txq_update(ath_dev_t dev, int qnum, HAL_TXQ_INFO *qi)
{
	...
	DPRINTF(sc, ATH_DEBUG_HTC_WMI, "%s: qnum = %d, aifs = %d, cwmin = %d, cwmax = %d, burstTime = %d, readyTime = %d\n",
		   __func__, qnum, qi->tqi_aifs, qi->tqi_cwmin, qi->tqi_cwmax, qi->tqi_burstTime, qi->tqi_readyTime);
...
ath_htc_uapsd_sendqosnull(struct ath_softc *sc, struct ath_node *an, u_int8_t ac)
{
    ...
    wbuf = wbuf_alloc(sc->sc_osdev, WBUF_TX_DATA, (ATH_HTC_HDRSPACE+sizeof(ath_data_hdr_t)+ATH_HTC_QOSNULL_LEN));
...
int
ath_htc_process_uapsd_trigger(ath_dev_t dev, ath_node_t node, u_int8_t maxsp, u_int8_t ac, u_int8_t flush, u_int8_t maxqdepth)
{
...
#ifndef MAGPIE_HIF_GMAC    
    else if (maxsp > an->an_uapsd_qdepth)  /* If STA's maxSP is not zero. */
        maxsp = an->an_uapsd_qdepth;
#endif
...
//            atomic_dec(&an->an_active_tx_cnt);
        ...
        if ((sc->sc_qosDropIpFrag[dropIndex] == 0)
            ...
            if (adf_os_unlikely((htcstatus != A_OK) || (htcstatus == A_CREDIT_UNAVAILABLE)) || tid->paused){
...
#ifdef  ATH_HTC_TX_SCHED
        if (adf_os_unlikely((htcstatus != A_OK) )){
...
#define ATH_GMAC_FRAG_BUF_SIZE          512
    ...
    if (wbuf1) {
        ...
        adf_os_mem_copy(adf_nbuf_put_tail(wbuf1, frag_len), anbdata + 1400,
                frag_len);
...
send1:            
        ...
        if (adf_os_unlikely((htcstatus1 != A_OK) || (htcstatus1 == A_CREDIT_UNAVAILABLE)) || tid->paused){
            if(epid == sc->sc_uapsd_ep)
                return -1;
...
ath_htc_rx_init(ath_dev_t dev, int nbufs)
{
    ...
    adf_nbuf_queue_init(&sc->sc_rxwbufhead);
...
#ifdef  ATH_HTC_TX_SCHED
        if (adf_os_unlikely((status != A_OK) )){
...
void ath_htc_host_txep_init(struct ath_softc *sc)
{
   ...
   adf_os_print(" osdev->beacon_ep%d \n",sc->sc_osdev->beacon_ep);
   adf_os_print(" osdev->cab_ep %d \n", sc->sc_osdev->cab_ep);
   adf_os_print(" osdev->uapsd_ep %d \n",sc->sc_osdev->uapsd_ep);
   adf_os_print(" osdev->mgmt_ep %d \n",sc->sc_osdev->mgmt_ep);
   adf_os_print(" osdev->data_BE_ep %d \n",sc->sc_osdev->data_BE_ep);
   adf_os_print(" osdev->data_BK_ep %d \n",sc->sc_osdev->data_BK_ep);
   adf_os_print(" osdev->data_VI_ep %d \n",sc->sc_osdev->data_VI_ep);
   adf_os_print(" osdev->data_VO_ep %d \n",sc->sc_osdev->data_VO_ep);
...
ath_buffer_setup(struct ath_softc *sc, ath_bufhead *head,
                    const char *name, a_int32_t nbuf) {
    /* allocate buffers */
    struct ath_buf * bf = NULL;
    ...
    if (bf == NULL) {
        adf_os_print("ath_buffer_setup buf allocation Fail \n");
        return NULL ;
    }
    firstbf =bf;
...
ath_htc_tx_cleanup(ath_dev_t dev) {
    ...
    adf_os_print(" < TGT DETACH CMD Executed > \n");
...
//    __stats(sc, tx_tidpaused);
    ...
    tid->paused = 1 ;
#ifndef MAGPIE_HIF_GMAC 
    adf_os_print("Tid paused pause %d \n",tid->paused);
#endif
}
...
ath_txep_resume_tid(struct ath_softc *sc, ath_atx_tid_t *tid)
{
    ...
    tid->paused = 0 ;
#ifndef MAGPIE_HIF_GMAC    
    adf_os_print("   ath_txep_resume_tid tid pause %d\n",tid->paused);
#endif
 ...
 */
static a_uint16_t
ath_htc_can_xmit(struct ath_softc *sc, a_uint16_t ac)
{
...
static void
ath_txep_enqueue(struct ath_txep *txep, struct ath_node *an, a_int32_t tidno,
        int switch_tid )
{
...
static struct ath_buf *
ath_host_buf_alloc(struct ath_softc *sc, a_uint8_t ac)
{
    struct ath_buf *bf = NULL;
    ...
    if(sc->sc_devstopped == 1){
        ATH_TXBUF_UNLOCK(sc);
        adf_os_print("Error should not come in as dev stopped \n");
        return NULL;
    }
     ...
     */
#ifdef todo_hostschedule    
    if (TAILQ_EMPTY(&sc->sc_txbuf)) {
        sc->sc_stats.ast_tx_qstop++;
        adf_net_stop_queue(sc->sc_net);
        sc->sc_devstopped = 1;
        sc->sc_stats.ast_tx_qstop++;
    }
#endif
...
/*
    * Deferred processing of transmit interrupt.
     */
a_int32_t priority_ac[4] = {3,2,0,1};
void
    ...
    for (i = 0; i < WME_NUM_AC ; i++) {
	    ...
	    TAILQ_FOREACH(ac,&txep->ep_acq,ac_qelem) {
		    ...
		    TAILQ_FOREACH_SAFE(tid, &ac->tid_q, tid_qelem, next_tid) {
			    ...
			    tid->sched = AH_FALSE;
#ifdef VOW_TIDSCHED /*Not teseted */
