...
#define    RX_FILTER_PRESERVE    (HAL_RX_FILTER_PHYERR | HAL_RX_FILTER_PHYRADAR)
    ...
    */
    ...
    {
        if (sc->sc_opmode == HAL_M_IBSS || sc->sc_scanning || sc->sc_nvaps > 1) {
...
ath_startrecv(struct ath_softc *sc)
{
    ...
    if (atomic_read(&sc->sc_rx_hw_en) != 1) {
        printk("%s: Warning: Multiple Rx starts or stops; count=%d. Ignored this one!\n",
...
#ifdef TXBF_TODO
        if (sc->last_rx_type == Rx_ACK) {
            /* Store the CSI_1 of is_h */			
            /* buffer data to sc->pos2_data sc->pos2_data_len; */
            /* rxs->rs_datalen */
            sc->pos2_data_len = wbuf_get_pktlen(wbuf);
            if (sc->pos2_data==NULL) {
                sc->pos2_data =	(u_int8_t *)OS_MALLOC(sc->sc_osdev, sc->pos2_data_len, GFP_KERNEL);
            }
            if(sc->pos2_data) {
                DPRINTF(sc, ATH_DEBUG_ANY,"%s Store the CSI_1 of is_h Datalen(%d)-(%d)\n",
                    __FUNCTION__, rxs->rs_datalen, wbuf_get_pktlen(wbuf));
                OS_MEMZERO(sc->pos2_data, sc->pos2_data_len);
                OS_MEMCPY(sc->pos2_data, wbuf_header(wbuf), sc->pos2_data_len);
                OS_MEMCPY(&(sc->pos2_rx_status), rxs, sizeof(struct ath_rx_status));
            } else {
                DPRINTF(sc, ATH_DEBUG_ANY,"==>%s:Can't alloc mem required for Pos2 \n", 
                    __func__);
                sc->pos2_data_len = 0;
            }
        } else if (sc->last_rx_type == Rx_HTC_QoS_NULL) {
            /* prepare and send the CSI report */
#define MAX_CSI_LENGTH 3072
            u_int16_t	buf_len = 100;
            u_int8_t    *mimo_control;
            struct ieee80211com *ic = (struct ieee80211com *) sc->sc_ieee;
            /* struct ath_hal *ah = sc->sc_ah; */
            u_int8_t	*H_data;
            u_int8_t	*CSI_buf_P=(u_int8_t *)OS_MALLOC(sc->sc_osdev, MAX_CSI_LENGTH, GFP_KERNEL);

            DPRINTF(sc, ATH_DEBUG_ANY, "%s prepare and send the CSI report Datalen(%d)-(%d) \n",
                __FUNCTION__, rxs->rs_datalen, wbuf_get_pktlen(wbuf));

            if (CSI_buf_P) {
                OS_MEMZERO(CSI_buf_P, MAX_CSI_LENGTH);
            } else {
                DPRINTF(sc, ATH_DEBUG_ANY, "==>%s:Can't Allocate CSI_buf_P \n", __func__);
            }

            if ((sc->pos3_node != NULL) && CSI_buf_P) {
                /*transfer H to CSI report*/
                H_data = (u_int8_t *) wbuf_header(wbuf);	
                buf_len = (u_int16_t) ath_hal_FillCSIFrame(sc->sc_ah, rxs, 0, H_data, CSI_buf_P);

                mimo_control = CSI_buf_P;
                DPRINTF(sc, ATH_DEBUG_ANY,"Send CSI Report FRAME Len(%d)\n", buf_len);
                DPRINTF(sc, ATH_DEBUG_ANY,"mimo_control 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x \n",
                        *mimo_control, *(mimo_control+1), *(mimo_control+2),
                        *(mimo_control+3), *(mimo_control+4), *(mimo_control+5));
                ic->ic_csi_report_send((sc->pos3_node), (CSI_buf_P+mimo_control_len),
                    (buf_len-6), mimo_control);
            } else {
                DPRINTF(sc, ATH_DEBUG_ANY,"==>%s:sc->pos3_node to be NULL do nothing \n",
                    __func__);
            }
            ...
            DPRINTF(sc, ATH_DEBUG_ANY,"==>%s:CSI_buf free!!\n", __func__);
            if (sc->pos3_node) {
                sc->pos3_node = NULL;
            }
...
#ifdef TXBF_TODO        	
            if (wh->i_fc[0] == (IEEE80211_FC0_TYPE_CTL|IEEE80211_FCO_SUBTYPE_Control_Wrapper)) {
                /* To be ACK */
                DPRINTF(sc, ATH_DEBUG_ANY,"%s To be Control Wrapper ACK(%d)\n",
                    __FUNCTION__,   rxs->rs_datalen);
                sc->last_rx_type = Rx_ACK;
            } else if ((wh->i_fc[1]&IEEE80211_FC1_ORDER) && 
                    (wh->i_fc[0]&IEEE80211_FC0_SUBTYPE_QOS_NULL) && 
                    ((wh1->i_htc[2] & IEEE80211_HTC2_CalPos)!=0)) {
                /*QoS NULL with HTC*/					
                DPRINTF(sc, ATH_DEBUG_ANY,"%s QoS NULL with HTC(%d)\n",
                    __FUNCTION__,rxs->rs_datalen);
                sc->last_rx_type = Rx_HTC_QoS_NULL;
                sc->pos3_node = NULL;
                {
                    ...
                    if (ni_T  == NULL) {
                        DPRINTF(sc, ATH_DEBUG_ANY,"%s Can't Find the NODE?\n",__FUNCTION__);
                    } else {
                        DPRINTF(sc, ATH_DEBUG_ANY,"%s save node of pos3 \n ",__FUNCTION__);
                        sc->pos3_node = ni_T;
                    }		
                }		
            } else
...
#define MAP_INTO_ONESTREAM(_rate) (_rate & 0x87)
...
#ifdef TXBF_TODO
    {
        ...
        if (wh1->i_fc[1]&IEEE80211_FC1_ORDER) {
            ...
            rawdata= (u_int8_t *) wbuf_header(wbuf);
            DPRINTF(sc, ATH_DEBUG_ANY,"==>%s:TRQ received!!,frame header:\n",   __func__);

            for (i=0;i<30;i++) {
                DPRINTF(sc, ATH_DEBUG_ANY," %x", rawdata[i]);
            }
        }
    }
#endif
...
#define PA2DESC(_sc, _pa)                                               \
    ...
    do {
        ...
        ds = bf->bf_desc;
        OS_MEMCPY(&rxs, &(ds->ds_rxstat), sizeof(struct ath_rx_status));
        ++rx_processed;
         ...
         */
        retval = ath_hal_rxprocdesc(sc->sc_ah, ds, bf->bf_daddr, PA2DESC(sc, ds->ds_link), 0);
        if (HAL_EINPROGRESS == retval) {
            ...
            if (tbf) {
                ...
                retval = ath_hal_rxprocdesc(sc->sc_ah, tds, tbf->bf_daddr, PA2DESC(sc, tds->ds_link), 0);
                ...
                /*
                 * Discard the corrupted descriptor but
                 * process the next one which is done.
                 */

                TAILQ_REMOVE(&sc->sc_rxbuf, bf, bf_list);
                TAILQ_INSERT_TAIL(&sc->sc_rxbuf, bf, bf_list);
                ath_rx_buf_link(sc, bf, true); /* try to requeue this descriptor */
                bf = tbf;
            ...
            retval = ath_hal_rxprocdesc(sc->sc_ah, ds, bf->bf_daddr, PA2DESC(sc, ds->ds_link), 
                    0);
        }
        /*
         * For non-UAPSD case, the descriptor has been updated, so the local copy of
         * the rx status from the descriptor should be updated as well.
         */
        OS_MEMCPY(&rxs, &(ds->ds_rxstat), sizeof(struct ath_rx_status));
        ...
        if (0 == rxs.rs_status) {
            ...
            if ((frm->i_fc[1] & IEEE80211_FC1_DIR_MASK) ==
                ...
                sc->sc_ieee_ops->get_bssid(sc->sc_ieee, (struct
                            ...
                            sc->sc_ah->ah_rxLastAggrBssid);
            } else {
                if ((frm->i_fc[1] & IEEE80211_FC1_DIR_MASK) ==
                    IEEE80211_FC1_DIR_FROMDS) {
                    OS_MEMCPY(sc->sc_ah->ah_rxLastAggrBssid, frm->i_addr2,
                            IEEE80211_ADDR_LEN);
                } else {
                    OS_MEMCPY(sc->sc_ah->ah_rxLastAggrBssid, frm->i_addr1,
                            IEEE80211_ADDR_LEN);
                }
            }
        }
        ...
        if (rxs.rs_status != 0) {
             ...
             *
             */
            if ((ATH_ENAB_AOW_ES(sc) || ATH_ENAB_AOW_ESS(sc))
                    && ATH_ENAB_AOW(sc)) {
                if (rxs.rs_status & ~HAL_RXERR_CRC) {
                    goto rx_next;
                }
                     ...
                     ((ATH_ENAB_AOW_ER(sc)) && (ATH_ENAB_AOW(sc)))) {
...
rx_next:
    ...
    ath_hal_rxmonitor(sc->sc_ah, &sc->sc_halstats, &sc->sc_curchan);
#endif
    ...
    for (bf = bf_first; bf; bf = TAILQ_NEXT(bf, bf_list)) {
...
#else
                retval = ath_hal_get_rxkeyidx(ah, tds, &(rxs.rs_keyix), &(rxs.rs_status));
#endif
                if (HAL_EINPROGRESS == retval)
                    break;
            } else {
        ...
        if (rxs.rs_status & ~HAL_RXERR_KEYMISS) {
            ...
            if ((wh->i_fc[1] & IEEE80211_FC1_DIR_MASK) ==
                ...
                if ((wh->i_fc[1] & IEEE80211_FC1_DIR_MASK) ==
                    IEEE80211_FC1_DIR_FROMDS) {
                    OS_MEMCPY(sc->sc_ah->ah_rxLastAggrBssid,
                            wh->i_addr2,IEEE80211_ADDR_LEN);
                } else {
                    OS_MEMCPY(sc->sc_ah->ah_rxLastAggrBssid,
                            wh->i_addr1,IEEE80211_ADDR_LEN);
                }
            }
#endif
        }
...
#define MICROSEC_PER_SEC       1000000
    ...
    prev = (index == 0) ? ATH_STATS_VI_LOG_LEN : (index  - 1);
