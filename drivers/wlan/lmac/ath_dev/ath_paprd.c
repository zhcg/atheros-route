...
int ath_apply_paprd_table(struct ath_softc *sc)
{
    ...
    DPRINTF(sc, ATH_DEBUG_ANY, "%s[%d]: paprdDone %d write %d\n", __func__, __LINE__, sc->sc_curchan.paprdDone, sc->sc_curchan.paprdTableWriteDone);
    ...
    //ath_hal_paprd_enable() should set chan->paprdTableWriteDone = 1
...
int ath_paprd_cal(struct ath_softc *sc)
{
    ...
    {
        /* Intentional print */
        printk("%s PAPRD excessive failure disabling PAPRD now\n", __func__);
        sc->sc_paprd_enable = 0;
        return -1;
        
    }
    if (sc->sc_curchan.paprdDone)
    {
...
FAILED: 
    ...
    DPRINTF(sc, ATH_DEBUG_CALIBRATE, "%s[%d]: failed!!\n", __func__, __LINE__);
    ath_hal_hwgreentx_set_pal_spare(sc->sc_ah, 1);
    return retval;
}
...
int ath_tx_paprd_init(struct ath_softc *sc)
{
    ...
    //sc->sc_debug = ATH_DEBUG_CALIBRATE;
    if (sc->sc_reg_parm.paprdEnable == 0) {
        DPRINTF(sc, ATH_DEBUG_CALIBRATE, "%s Registry disabled PAPRD\n",
                __func__);
        return 0;
    }
    ...
    if ( ath_hal_getcapability(sc->sc_ah, HAL_CAP_PAPRD_ENABLED, 0, &result) != HAL_OK) {
        DPRINTF(sc, ATH_DEBUG_CALIBRATE, "%s No support for PAPRD\n",
                __func__);
        return 0;
    }
    sc->sc_paprd_enable =  1;
    sc->sc_paprd_chain_num = 0;
    sc->sc_paprd_rate = 0xc; /* 54Mbps */
    ...
    TAILQ_FOREACH(bf, &sc->sc_paprdbuf, bf_list){
        ...
        if (bf) {
            DPRINTF(sc, ATH_DEBUG_CALIBRATE,
                    "%s * allocated paprd pointer at %p \n", __func__, bf);
        }
...
int ath_sendpaprd_single(struct ath_softc *sc,  int count, int chain_num) /* sc->sc_myaddr */
{
    ...
    if (NULL == bf) {
        DPRINTF(sc, ATH_DEBUG_CALIBRATE,
            "%s paprd buffer seems NULL, last free has some bug bf %p \n",
            __func__, bf);
        ATH_PAPRD_UNLOCK(sc);
        return -1;
    }
    ...
    wbuf_set_node(bf->bf_mpdu, (struct ieee80211_node *)bf->bf_node);
    bf->bf_isaggr  = 0;
    bf->bf_isbar  = bf->bf_isampdu = 0;
    ...
    if (hdr) {
        OS_MEMCPY(hdr->i_addr1, bssid, IEEE80211_ADDR_LEN);
        OS_MEMCPY(hdr->i_addr2, bssid, IEEE80211_ADDR_LEN);
        OS_MEMCPY(hdr->i_addr3, bssid, IEEE80211_ADDR_LEN);
    ...
    DPRINTF(sc,ATH_DEBUG_CALIBRATE,
    "==== %s[%d] bf %p bf->bf_frmlen %d wbuf_get_len(%d), bf->flags %x "
        "que num %d  bf->bf_buf_len %d\n",
...
#if 1
{
    ...
    for(i = 0; i < 4 ; i++) {
        //series[i].Tries = 1;
        series[i].ChSel = sc->sc_tx_chainmask | (1 << chain_num);
        series[i].RateFlags =  0;
        //NEW
        series[i].Tries = 1;
        series[i].RateIndex = 11;
        series[i].Rate = 0x0c; /* 54 Mbps */
        series[i].TxPowerCap = txpower;
        series[i].PktDuration = ath_pkt_duration(
            sc, series[0].RateIndex, bf,
...
#ifdef AR_DEBUG
{
    ...
    if ((retval = ath_hal_paprd_is_done(sc->sc_ah)) == 0) {
        DPRINTF(sc,ATH_DEBUG_ANY, "%s[%d]:  PAPRD CAL failed on chain %d\n", __func__, __LINE__, 
                                    sc->sc_paprd_chain_num);
        sc->sc_paprd_failed_count++;
        goto RE_CAL;
    }
