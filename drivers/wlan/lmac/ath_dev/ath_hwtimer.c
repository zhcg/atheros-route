                  ...
                  u_int32_t timer_next, u_int32_t period)
{
    ...
    /* Temp Hack. Commented for AoW Build
     * Needs fix 
     */
    ASSERT(timer->cached_state.active == false);
    ...
    /* if it's the first generic timer enabled, turn on the interrupt mask */
    if ((sc->sc_imask & HAL_INT_GENTIMER) == 0) {
