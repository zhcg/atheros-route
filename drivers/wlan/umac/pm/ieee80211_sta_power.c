...
static void ieee80211_sta_power_txrx_event_handler (ieee80211_vap_t vap, ieee80211_vap_txrx_event *event, void *arg);
                                              ...
                                              ieee80211_sta_power_event *event)
{
    ...
    for(i = 0;i < nhandlers; ++i) {
        (*handlers[i])(powersta, event, handler_arg[i]);
    }
}
                                          ...
                                          int                   power_mode) 
{
    ...
    case IEEE80211_PWRSAVE_AWAKE: /* put the chip in awake state */
        if (powersta->ips_nw_sleep) {
            ieee80211_power_exit_nwsleep(powersta->ips_vap);
    ...
    case IEEE80211_PWRSAVE_NETWORK_SLEEP: /* put the chip in network sleep state */
        if (!powersta->ips_nw_sleep) {
            ieee80211_power_enter_nwsleep(powersta->ips_vap);
...
int ieee80211_sta_power_unregister_event_handler(ieee80211_sta_power_t             powersta, 
...
int ieee80211_sta_power_register_event_handler(ieee80211_sta_power_t             powersta, 
                                               ieee80211_sta_power_event_handler evhandler, 
                                               void                              *arg)
{
    int    i;
    ...
    ieee80211_sta_power_unregister_event_handler(powersta,evhandler,arg);
                                                     ...
                                                     void                  *event_data) 
{
    ...
    case IEEE80211_POWER_STA_EVENT_UNPAUSE:                    
        if (powersta->ips_pause_request) {
            ...
            /* 
             * if no data frames in the powersave queue (pause queue)
             ...
             * if data frames in the powersave queue (pause queue)
             * and powersave is not enabled then enter INIT. 
             * if data frames in the powersave queue (pause queue)
             * and powersave is enabled then enter ACTIVE. 
             */
            if (qinfo.data_count == 0 && qinfo.mgt_count == 0 ) {
...
static void ieee80211_sta_power_state_init_entry(void *ctx) 
{
    ...
    ieee80211_vap_txrx_unregister_event_handler(powersta->ips_vap,ieee80211_sta_power_txrx_event_handler,powersta);
...
static void ieee80211_sta_power_state_pending_tx_entry(void *ctx) 
{
    ...
    ieee80211_vap_txrx_register_event_handler(powersta->ips_vap,ieee80211_sta_power_txrx_event_handler,powersta,
...
static void ieee80211_sta_power_state_null_sent_entry(void *ctx) 
{
    ...
    ieee80211_vap_txrx_register_event_handler(powersta->ips_vap,
...
*/
static u_int32_t ieee80211_get_next_tbtt_tsf_32(wlan_if_t vap)
{
   struct ieee80211com *ic = vap->iv_ic;
...
static void ieee80211_sta_power_start_tbtt_timer( ieee80211_sta_power_t        powersta) 
{
   ...
   if (tsf_offset_info.offset > MIN_TSF_TIME_DIFF ) {
       ...
       nexttbtt_tsf32 = ieee80211_get_next_tbtt_tsf_32(powersta->ips_vap);
                                                  ...
                                                  u_int32_t           pspoll_enabled)
{
    ...
    OS_INIT_TIMER(os_handle, &handle->ips_timer,                         
                  ieee80211_sta_power_activity_timer, handle);
    OS_INIT_TIMER(os_handle, &handle->ips_pause_timer,                         
                  ieee80211_sta_power_pause_timer, handle);
    OS_INIT_TIMER(os_handle, &handle->ips_async_null_event_timer,                         
                  ieee80211_sta_power_async_null_event_timer, handle);
    OS_INIT_TIMER(os_handle, &handle->ips_bcn_timeout_timer,
                  ieee80211_sta_power_bcn_timeout_event_timer, handle);
...
void ieee80211_sta_power_vdetach(ieee80211_sta_power_t handle)
{
    ...
    ieee80211_vap_txrx_unregister_event_handler(handle->ips_vap, ieee80211_sta_power_txrx_event_handler, handle);
...
int ieee80211_sta_power_send_keepalive(ieee80211_sta_power_t powersta)
{
    if (powersta->ips_connected) {
...
int ieee80211_sta_power_pause(ieee80211_sta_power_t powersta, 
...
int ieee80211_sta_power_unpause(ieee80211_sta_power_t powersta)
{
    struct ieee80211vap    *vap;
                                         ...
                                         u_int32_t             mode)
{
    ...
    if (mode > IEEE80211_PWRSAVE_MAXIMUM) {
        return;
    }
    /*
...
wlan_get_powersave(wlan_if_t vaphandle)
{
    ...
    return ieee80211_sta_power_get_mode(vaphandle->iv_pwrsave_sta);
}

u_int32_t
wlan_get_apps_powersave_state(wlan_if_t vaphandle)
{
    ieee80211_sta_power_t    powersta = vaphandle->iv_pwrsave_sta;
    u_int32_t                pwr_state;
...
wlan_pwrsave_force_sleep(wlan_if_t vaphandle, bool enable)
{
    ...
    return ieee80211_sta_power_force_sleep(vaphandle->iv_pwrsave_sta, enable);
}
...
int ieee80211_pwrsave_uapsd_set_max_sp_length(wlan_if_t vaphandle,u_int8_t max_sp_val  )
{
    ...
    if (max_sp_val !=0 && max_sp_val !=2 && max_sp_val !=4 && max_sp_val != 6) {
        return EINVAL;
    }
    switch(max_sp_val) {
    case 0:
    ...
    default:
        status=EINVAL;
        val=0;  /* init. to a dummy value to make win7 compile happy. */
        break;

    }
    if (status == EOK) {
        vaphandle->iv_uapsd  &= ~(0x3 << WME_CAPINFO_UAPSD_MAXSP_SHIFT);
        vaphandle->iv_uapsd  |= (val << WME_CAPINFO_UAPSD_MAXSP_SHIFT);
    }
