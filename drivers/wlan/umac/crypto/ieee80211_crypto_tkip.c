...
static	int tkip_enmic(struct ieee80211_key *, wbuf_t, int);
 ...
 */
static int
tkip_enmic(struct ieee80211_key *k, wbuf_t wbuf0, int force)
{
    struct tkip_ctx *ctx = k->wk_private;
    wbuf_t wbuf;
    struct ieee80211_frame *wh = (struct ieee80211_frame *) wbuf_header(wbuf0);
...
tkip_demic(struct ieee80211_key *k, wbuf_t wbuf0, int hdrlen, int force, struct ieee80211_rx_status *rs)
{
    ...
    origHdr = (u_int8_t *)wbuf_header(wbuf0);
...
#define PACKET_NUMBER_GAP 500
    ...
    pn  = ctx->rx_rsc;
     ...
     if ( (pn - k->wk_keyrsc[tid]) > PACKET_NUMBER_GAP && k->wk_keyrsc[tid]) {
        /* PN gap detected, check MIC for possible error */
 
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_CRYPTO,
                         "%s ..Now packet jump detected updating ...\n", __func__);
            if(k->wk_keyrscdropped[tid] == 0 )
            {
                    if((pn - k->wk_keyrscglobal) <= PACKET_NUMBER_GAP)
                    {
                            k->wk_keyrscdropped[tid]=pn;
                            k->wk_keyrscdroppedcnt[tid]=1;
                    }
                    else
                    {
                            k->wk_keyrscdropped[tid]=pn;
                            k->wk_keyrscdroppedcnt[tid]=1;
                            printk("====discarding junk frame====\n");
                            return 0; /* discard junk frame */
                    }
            }
            else
            {
                    if(pn <= k->wk_keyrscdropped[tid])
                    {
                            ieee80211_notify_replay_failure(vap, wh, k, pn);
                            mac_stats->ims_rx_tkipmic++;
                            printk("=======returning and notifying========\n");
                            return 0;
                    }
                    if(pn > (k->wk_keyrscdropped[tid] + PACKET_NUMBER_GAP))
                    {
                            if((pn - k->wk_keyrscglobal) <= PACKET_NUMBER_GAP)
                            {
                                    k->wk_keyrscdropped[tid]=pn;
                    ...
                    {
                            k->wk_keyrscdropped[tid]=pn;
                            k->wk_keyrscdroppedcnt[tid]++;
                            if(k->wk_keyrscdroppedcnt[tid] > THRESHOLD)
                            {
                                    k->wk_keyrscupdate[tid]=1;
                                    k->wk_keyrscdropped[tid]=0;
                                    k->wk_keyrscdroppedcnt[tid]=0;
                            }
                    }
            }
       }
        ...
        if (!tkip_mhdrie_check(wbuf0, pktlen, tkip.ic_miclen)) {
            ...
            if ((now - mac_counterm->timestamp) > 60000) {
                ...
                if (++mac_counterm->mic_count_in_60s == 2) {
                     ...
                     */
                    mac_stats->ims_rx_countermeasure++;
                    mac_counterm->timestamp = now;
                }
            }
            return 0;
    ...
    if (IEEE80211_QOS_HAS_SEQ(wh)) 
        tid = ((struct ieee80211_qosframe *)wh)->i_qos[0] & IEEE80211_QOS_TID;
    if(k->wk_keyrscupdate[tid])
    {
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_CRYPTO,
                         "%s ..Resetting current k->wk_keyrsc[tid] with pn %llu \n", __func__, pn);
	    k->wk_keyrsc[tid] = pn;
	    k->wk_keyrscupdate[tid]=0;
    }
    k->wk_keyrscglobal=pn;
