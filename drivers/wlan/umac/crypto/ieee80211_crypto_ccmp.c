...
static	int ccmp_enmic(struct ieee80211_key *, wbuf_t, int);
...
#define     MAX_CCMP_PN_GAP_ERR             33  /* Max. gap in CCMP PN before doing MIC sanity check */
 ...
 */
static int
ccmp_enmic(struct ieee80211_key *k, wbuf_t wbuf, int force)
{
    return 1;
}
...
ccmp_decap(struct ieee80211_key *k, wbuf_t wbuf, int hdrlen, struct ieee80211_rx_status *rs)
{
    ...
    if ((k->wk_keyrsc[tid] > 1) && (pn > (k->wk_keyrsc[tid] + MAX_CCMP_PN_GAP_ERR))) {
        // Big jump
#if ATH_VOW_EXT_STATS
        #define VOW_EXT_STATS_SIZE  24
        uint8_t vow_extstats[VOW_EXT_STATS_SIZE];   /* to hold extstats temporarly */
        uint8_t *bp = NULL;
        uint16_t es_offset = 0; /* offset of ext stats in the packet */
        if(rs->vow_extstats_offset)
        {
            bp = (uint8_t *)wbuf_raw_data(wbuf);
            es_offset = rs->vow_extstats_offset >> 16;

            /* Copy the vow ext stats from data packet onto temp storage */
            OS_MEMCPY(vow_extstats, bp + es_offset, VOW_EXT_STATS_SIZE);

            /* packet paylod got modified in ath layer for ext stats */
            /* so, revert back the payload */
            OS_MEMZERO(bp + es_offset, VOW_EXT_STATS_SIZE);
        }
#endif
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_CRYPTO,
               ...
               tid,
               (u_int32_t)(k->wk_keyrsc[tid] >> 32), (u_int32_t)k->wk_keyrsc[tid],
               (uint32_t)(pn>>32), (uint32_t)(pn),
               *(u_int16_t*)(wh->i_seq)
               );
        // Jump in the PN. Check the MIC of this frame.
        // No monotically increasing.
        if (ccmp_check_mic(k, wbuf, hdrlen, pn) == 0)
        {
            // The MIC is corrupted. Do not use this frame.
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_CRYPTO,
                    "%s: The MIC is corrupted. Drop this frame.\n", __func__);
            return 0;
        }
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_CRYPTO,
                    "%s: The MIC is OK. Still use this frame and update PN.\n", __func__);

#if ATH_VOW_EXT_STATS
        if(rs->vow_extstats_offset)
        {
            /* Copy back the vow ext stats onto data packet */
            OS_MEMCPY(bp + es_offset, vow_extstats, VOW_EXT_STATS_SIZE);
        }
        #undef VOW_EXT_STATS_SIZE
#endif        
        if (pn > (k->wk_keyrsc[tid] + MAX_CCMP_PN_GAP_ERR_CHECK)) {
...
#pragma prefast(suppress:6202, "we know 3 addresses are contiguous")
    ...
    if (IS_4ADDRESS(wh)) {
        ...
        if (IS_QOS_DATA(wh)) {
            ...
            if (mfp)
                aad[24] = 0xff;	/* just priority bits */
            else
