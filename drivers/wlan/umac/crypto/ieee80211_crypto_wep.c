...
static	int wep_enmic(struct ieee80211_key *, wbuf_t, int);
 ...
 */
static int
wep_enmic(struct ieee80211_key *k, wbuf_t wbuf, int force)
{
    return 1;
}
...
#ifdef notyet
static int
wep_encrypt(struct ieee80211_key *key, wbuf_t wbuf, int hdrlen)
{
#define S_SWAP(a,b) do { uint8_t t = S[a]; S[a] = S[b]; S[b] = t; } while(0)
    ...
    /* NB: this assumes the header was pulled up */
    OS_MEMCPY(rc4key, skb->data + hdrlen, IEEE80211_WEP_IVLEN);
...
#ifdef IEEE80211_DEBUG
                const struct ieee80211_frame *wh =
                    (const struct ieee80211_frame *) origHdr;
