...
#else
typedef htclock_t  ieee80211_resmgr_lock_t;
#endif
...
#define IEEE80211_RESMGR_LOCK_INIT(_rm)             OS_HTC_LOCK_INIT(&(_rm)->rm_lock)
#define IEEE80211_RESMGR_LOCK_DESTROY(_rm)          OS_HTC_LOCK_DESTROY(&(_rm)->rm_lock)
#define IEEE80211_RESMGR_LOCK(_rm)                  OS_HTC_LOCK(&(_rm)->rm_lock)     
#define IEEE80211_RESMGR_UNLOCK(_rm)                OS_HTC_UNLOCK(&(_rm)->rm_lock)
#else
...
#define ieee80211_resmgr_sm_dispatch(resmgr,event,event_data_len,event_data)  (EINVAL) 
static INLINE void ieee80211_vap_iter_check_state(void *arg, struct ieee80211vap *vap){}
