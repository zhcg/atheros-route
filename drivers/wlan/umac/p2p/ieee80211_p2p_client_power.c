...
struct ieee80211_p2p_client_power {
    ...
    /* 
     * In split driver architecture, timer jitter is unacceptale for CTWindows case. Using absolute time rather than relative time is better.
     * To further keep timer more accurate, change OS timer to TSF timer for CTWindows case.
     */
...
static void ieee80211_p2p_client_power_force_pause_control (ieee80211_p2p_client_power_t p2p_cli_power, bool val) 
{
    ...
    if (p2p_cli_power->pcp_vap_force_paused != val) {
        if (val) {
            ...
            ieee80211_power_enter_nwsleep(p2p_cli_power->pcp_vap);
        } else {
            if (p2p_cli_power->pcp_node_pause_pend_null) {
                ASSERT(p2p_cli_power->pcp_vap->iv_bss);
                ieee80211node_unpause(p2p_cli_power->pcp_vap->iv_bss);
            ...
            ieee80211_power_exit_nwsleep(p2p_cli_power->pcp_vap);
...
static void ieee80211_p2p_client_power_state_go_present_entry(void *ctx) 
{
    ...
    if (p2p_cli_power->pcp_uapsd_sp_in_progress ) {
        ...
        ni = ieee80211_ref_bss_node(p2p_cli_power->pcp_vap);
        if (ni) {
 ...
 * wrt AP.
 */
static void ieee80211_p2p_client_power_txrx_event_handler (ieee80211_vap_t vap, ieee80211_vap_txrx_event *event, void *arg)
{
        ...
        if (event->u.status == 0 && p2p_cli_power->pcp_vap->iv_uapsd) { 
            ...
            if (type == IEEE80211_FC0_TYPE_DATA &&  (subtype == IEEE80211_FC0_SUBTYPE_QOS_NULL || subtype == IEEE80211_FC0_SUBTYPE_QOS )) {
                ...
                if (wme_capinfo_ac &  p2p_cli_power->pcp_vap->iv_uapsd) { 
                    ...
                    ieee80211_vap_txrx_register_event_handler(p2p_cli_power->pcp_vap,ieee80211_p2p_client_power_txrx_event_handler,p2p_cli_power,
...
void ieee80211_p2p_client_power_vap_event_handler (ieee80211_p2p_client_power_t p2p_cli_power, ieee80211_vap_event *event)
{
    ...
    case IEEE80211_VAP_DOWN:
        ...
        ieee80211_vap_txrx_unregister_event_handler(p2p_cli_power->pcp_vap,ieee80211_p2p_client_power_txrx_event_handler,p2p_cli_power);
...
ieee80211_p2p_client_power_t ieee80211_p2p_client_power_create(osdev_t os_handle, wlan_p2p_client_t p2p_client_handle, wlan_if_t vap, ieee80211_p2p_go_schedule_t go_schedule)
{
                      ...
                      MESGQ_ASYNCHRONOUS_EVENT_DELIVERY) != 0)
    {
       ...
       ieee80211_tsftimer_free(handle->pcp_tbtt_tsf_timer, true);
       ieee80211_sm_delete(handle->pcp_hsm_handle); 
       OS_FREE(handle);
       return NULL;
    }
    handle->pcp_mlme_swbmiss_id = ieee80211_mlme_sta_swbmiss_timer_alloc_id(vap,(int8_t *)"P2P_CLI_NOA_SM");
    OS_INIT_TIMER(os_handle, &handle->pcp_ctwindow_timer, ieee80211_p2p_client_power_ctwindow_timer_handler, handle );
    OS_INIT_TIMER(os_handle, &handle->pcp_async_null_event_timer, 
                  ieee80211_p2p_client_power_async_null_event_timer, handle); 
