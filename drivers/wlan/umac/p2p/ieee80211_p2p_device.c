        ...
        if (p2p_handle->p2p_inprogress_scan_req_type == IEEE80211_P2P_SCAN_REQ_EXTERNAL_LISTEN) {
            ...
            if (ieee80211_p2p_parse_ies(p2p_handle->p2p_oshandle, vap, ie_data, ie_len, &pie) == 0) {
                ...
                do {
                    ...
                    if (ieee80211_p2p_send_proberesp(ni, wh->i_addr2) != 0) {
                        printk("%s: sending of PROBE_RESP failed\n", __func__);
                    }
                    else {
...
ieee80211_p2p_scan_evhandler(wlan_if_t vaphandle,ieee80211_scan_event *event, void *arg)
{
    ...
    if ((event->type == IEEE80211_SCAN_STARTED) ||
        ...
        (event->type == IEEE80211_SCAN_FOREIGN_CHANNEL)) {
        ...
        if ((scan_event->type == IEEE80211_SCAN_COMPLETED) || (scan_event->type == IEEE80211_SCAN_DEQUEUED)) {
            ...
            if (p2p_handle->p2p_inprogress_scan_req_type == IEEE80211_P2P_SCAN_REQ_NONE) {
                ...
                if (p2p_handle->p2p_pending_scan_req_type != IEEE80211_P2P_SCAN_REQ_NONE) {
                    ...
                    wlan_scan_unregister_event_handler(vap,ieee80211_p2p_scan_evhandler,(os_if_t)p2p_handle);
                    P2P_DEV_LOCK(p2p_handle);
                }
...
ieee80211_p2p_start_scan(wlan_p2p_t p2p_handle, ieee80211_p2p_scan_request type)
{
    ...
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY, "%s num_channels %d chan_1 %d scan_in_progress type=%d id=%d scan_pending type=%d id=%d scan_in_progress %d\n", 
        __func__,
                           ...
                           unsigned int duration, ieee80211_p2p_scan_request type)
{
     ...
     */
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY, "LISTEN: %s type=%d\n",
        __func__,
        type);
...
wlan_p2p_create(wlan_dev_t dev_handle, wlan_p2p_params *p2p_params)
{
    ...
    if (wlan_get_device_param(dev_handle, IEEE80211_DEVICE_P2P) == 0) {
        ieee80211com_note(dev_handle, "%s: Error: P2P unsupported.\n",__func__);
        return NULL;
    }
    ...
    if (p2p_handle == NULL) {
        ieee80211com_note(dev_handle, "%s p2p device allocation failed\n",__func__);
        return NULL;
    }
    ...
    p2p_handle->p2p_vap = wlan_vap_create(dev_handle,
                                          ...
                                          (IEEE80211_CLONE_BSSID | IEEE80211_P2PDEV_VAP),
                                          NULL);
    if (p2p_handle->p2p_vap == NULL) {
        ieee80211com_note(dev_handle, "%s p2p vap creation failed\n",__func__);
        OS_FREE(p2p_handle);
        return NULL;
    }
    ...
    if (wlan_set_desired_ssidlist(p2p_handle->p2p_vap, 1, &ssid)) {
        ieee80211com_note(dev_handle, "%s setting SSID failed\n",__func__);
        return NULL;
    }
    ...
    error = ieee80211_create_infra_bss(p2p_handle->p2p_vap,(u_int8_t *)IEEE80211_P2P_WILDCARD_SSID, IEEE80211_P2P_WILDCARD_SSID_LEN);
    ...
    if (p2p_params) {
        strncpy(p2p_handle->p2p_dev_name, p2p_params->dev_name, IEEE80211_MAX_P2P_DEV_NAME_LEN);
    ...
    if (wlan_scan_get_requestor_id(p2p_handle->p2p_vap, (u_int8_t *)"P2P_DEVICE_UMAC",
...
{
    wlan_scan_unregister_event_handler(p2p_handle->p2p_vap,ieee80211_p2p_scan_evhandler,(os_if_t)p2p_handle);
 ...
 */
int
wlan_p2p_set_channel(wlan_p2p_t  p2p_handle, bool in_listen_state, u_int32_t freq, u_int32_t req_id, u_int32_t channel_time)
{
    if (in_listen_state) {
        return ieee80211_p2p_start_listen(p2p_handle, freq, req_id,
                                          channel_time, IEEE80211_P2P_SCAN_REQ_EXTERNAL_LISTEN);
    }
    else {
        return ieee80211_p2p_start_listen(p2p_handle, freq, req_id,
                                          channel_time, IEEE80211_P2P_SCAN_REQ_EXTERNAL_SEARCH);
    }
}
                               ...
                               u_int32_t data_len)
{
    ...
    if (act_frame) {
        ...
        if (dwell_time > P2P_MAX_DWELL_TIME) {
#else
        if ((dwell_time < P2P_MIN_DWELL_TIME) || (dwell_time > P2P_MAX_DWELL_TIME)) {
#endif
    ...
    if (!hasTimeLeft ||
        wlan_vap_send_action_frame(vap, freq, ieee80211_action_frame_complete_handler,
                                   act_frame, dst_addr, src_addr, bssid, data, data_len) != EOK) {

        /* frame could not be sent out succesfully, queue it in to pending pool */
