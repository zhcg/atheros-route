...
struct ieee80211_p2p_go {
    ...
    u_int8_t                p2p_ie[IEEE80211_MAX_P2P_IE];/* probe resp p2p ie */
    u_int16_t               p2p_ie_len;
...
wlan_p2p_GO_create(wlan_p2p_t p2p_handle, u_int8_t *bssid)
{
    ...
    OS_INIT_TIMER(oshandle, &p2p_go_handle->pause_timer, test_vap_pause_timer_handler, p2p_go_handle );
#endif
    ...
    if (!success) {
        ...
        if (h_app_ie != NULL) {
            wlan_mlme_app_ie_delete(h_app_ie);
            h_app_ie = NULL;
        }
...
int wlan_p2p_GO_delete(wlan_p2p_go_t p2p_go_handle)
{
    ...
    wlan_mlme_app_ie_delete(p2p_go_handle->app_ie_handle);
    ...
    case IEEE80211_FC0_SUBTYPE_PROBE_REQ:
        ...
        if (ieee80211_p2p_parse_ies(oshandle, vap, ie_data, ie_len, &pie) == 0) {
            ...
            if ((subtype == IEEE80211_FC0_SUBTYPE_ASSOC_REQ) || 
                ...
                do {
                    if (pie.ssid == NULL) {
                        /* No SSID found. Do not handle this here. */
                        break;
                    }
                    ...
                    if (pie.p2p_attributes) {
                        if (ieee80211_send_proberesp(ni, wh->i_addr2, p2p_go_handle->p2p_ie, p2p_go_handle->p2p_ie_len) != 0) {
                            IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY, "%s: sending of PROBE_RESP failed\n", __func__);
                        }
                    }
                    else {
                        /* No P2P IE. We will still response but without p2p IE */
                        if (ieee80211_send_proberesp(ni, wh->i_addr2, NULL, 0) != 0) {
                            IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY, "%s: sending of PROBE_RESP(no p2p IE) failed\n", __func__);
                        }
                    }
    ...
    wlan_p2p_go_t       p2p_go_handle)
{
    ...
    efrm = ie_buf;
    efrm = p2p_go_add_p2p_ie(p2p_go_handle, efrm);
...
int wlan_p2p_GO_set_appie(wlan_p2p_go_t p2p_go_handle, ieee80211_frame_type ftype, u_int8_t *buf, u_int16_t buflen)
{
    ...
    p2p_go_handle->p2p_ie_len=0;
    p2p_ie_ptr = p2p_go_handle->p2p_ie;
    ie_ptr = ie_buf;
    ...
    rc = wlan_mlme_set_appie(p2p_go_handle->vap, ftype, ie_buf, ie_len);

    IEEE80211_DPRINTF(p2p_go_handle->vap, IEEE80211_MSG_ANY, 
        "%s: p2p ie len %d non p2p ie len %d \n", __func__, p2p_ie_len,ie_len);
    p2p_go_handle->p2p_ie_len=p2p_ie_len;
    OS_FREE(ie_buf);
    return rc;
}
