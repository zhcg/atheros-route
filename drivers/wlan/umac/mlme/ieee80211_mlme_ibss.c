...
mlme_create_adhoc_bss(struct ieee80211vap *vap)    
{
    ...
    if(vap->iv_des_ibss_chan) {
        chan = ieee80211_find_dot11_channel(ic, vap->iv_des_ibss_chan, vap->iv_des_mode);
         ...
         */
        if (chan == NULL) {
            chan = ieee80211_find_dot11_channel(ic, vap->iv_des_ibss_chan, IEEE80211_MODE_AUTO);
        }
    } else {
        /*
...
void ieee80211_mlme_create_join_ibss_continue(struct ieee80211vap *vap)
{
    struct ieee80211_mlme_priv  *mlme_priv = vap->iv_mlme_priv;
    int                         error = 0;
...
mlme_start_adhoc(struct ieee80211vap *vap)    
{
    ...
    /* Put underlying H/W to RUN state */
...
mlme_adhoc_match_node(struct ieee80211_node *ni)
{
    ...
    if (!ieee80211_check_rate(vap, ni->ni_chan, &ni->ni_rates)) {
        return 0;
    }
...
static void mlme_adhoc_watchdog_timer_iter(void *arg, struct ieee80211_node *ni)
{
    ...
    if (ni->ni_assoc_state == IEEE80211_NODE_ADHOC_STATE_ZERO) {
        if (ni->ni_wait0_ticks++ > MLME_DEAUTH_WAITING_THRESHOLD)
            ni->ni_assoc_state = IEEE80211_NODE_ADHOC_STATE_UNAUTH_UNASSOC;
    }
     ...
     */
    if (ni->ni_assoc_state == IEEE80211_NODE_ADHOC_STATE_UNAUTH_UNASSOC) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,
