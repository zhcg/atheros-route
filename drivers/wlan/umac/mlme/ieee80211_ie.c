...
ieee80211_add_htcap_pre_ana(u_int8_t *frm, struct ieee80211_node *ni,u_int8_t subtype)
{
    ...
    IEEE80211_DPRINTF(ni->ni_vap, IEEE80211_MSG_DEBUG, "%s: use HT caps IE pre-ANA\n",
                      __func__);
...
ieee80211_update_htinfo_cmn(struct ieee80211_ie_htinfo_cmn *ie, struct ieee80211_node *ni)
{
    ...
    {
        ie->hi_txchwidth = (ic_cw_width == IEEE80211_CWM_WIDTH40) ?
...
ieee80211_add_htinfo_pre_ana(u_int8_t *frm, struct ieee80211_node *ni)
{
    ...
    IEEE80211_DPRINTF(ni->ni_vap, IEEE80211_MSG_DEBUG, "%s: use HT info IE pre-ANA\n",
                      __func__);
                    ...
                    struct ieee80211_rsnparms *rsn)
{
    ...
    for (; n > 0; n--) {
        ...
        if (w == WPA_CCKM_AKM) { /* CCKM AKM */
            ...
            rsn->rsn_keymgmtset |= WPA_ASE_CCKM;
        }
        else
    ...
    */
    ...
    for (; n > 0; n--) {
        ...
        if (w == RSN_CCKM_AKM) { /* CCKM AKM */
            RSN_SET_AUTHMODE(rsn, IEEE80211_AUTH_CCKM);
            rsn->rsn_keymgmtset |= RSN_ASE_CCKM;
        }
        else
            rsn->rsn_keymgmtset |= w;
        frm += 4, len -= 4;
    ...
    )
{
    ...
    /*Get new announced channel information*/
    if (pcsaIe) {
        chan = ieee80211_doth_findchan(vap, pcsaIe->newchannel);
    }
    else if (pecsaIe) {
        chan = ieee80211_doth_findchan(vap, pecsaIe->newchannel);
    }
    else {
        return EOK;
    }
