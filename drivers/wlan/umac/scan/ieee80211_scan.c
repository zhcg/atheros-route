...
#define IEEE80211_CHECK_EXIT_CRITERIA_INTVAL           50
...
#define IEEE80211_MAX_MAXDWELL                       1000 /* msec */
...
struct ieee80211_scanner {
    struct ieee80211_scanner_info       ss_info; /* public data.  always need to be at the top */
...
static const u_int16_t scan_order[] = {
    ...
    /* 2.4Ghz ch: 14 */
    2484, 
    /* 5 FCC channel: 149, 153, 157, 161, 165 */
...
static void scanner_construct_chan_list(ieee80211_scanner_t ss)
{
#define FREQUENCY_THRESH 1000
    ...
    if (ss->ss_nreqchans == 0)
    {
        ...
        for (i = 0; i < ss->ss_nreqchans; ++i) {
            if (ss->ss_reqchans[i] < FREQUENCY_THRESH) {
                if (ss->ss_reqchans[i] < 20)
                    freqs[i] = ieee80211_ieee2mhz(ss->ss_reqchans[i], IEEE80211_CHAN_2GHZ); 
                else
                    freqs[i] = ieee80211_ieee2mhz(ss->ss_reqchans[i], IEEE80211_CHAN_5GHZ); 
            } else {
        ...
        for (i = 0; i < ss->ss_nallchans; ++i) {
            ...
            if(!(ss->ss_flags & IEEE80211_SCAN_2GHZ) &&
                IEEE80211_IS_CHAN_2GHZ(ss->ss_all_chans[i]))
                continue; /* only 5G channel, go to next channel */

            if(!(ss->ss_flags & IEEE80211_SCAN_5GHZ) &&
                IEEE80211_IS_CHAN_5GHZ(ss->ss_all_chans[i]))
...
static void ieee80211_scan_handle_sta_power_events(ieee80211_sta_power_t powersta, ieee80211_sta_power_event *event, void *arg)
{
    ieee80211_scanner_t    ss = (ieee80211_scanner_t) arg;
    u_int32_t              now = (u_int32_t) CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP());
                                                ...
                                                void                          *arg)
{
    ...
    IEEE80211_SCAN_PRINTF(ss, IEEE80211_MSG_SCAN, "%d.%03d | %s:%s state=%s event %s (%d) status=%08X\n",
                          now / 1000, now % 1000, __func__,
                          ss->ss_info.si_scan_in_progress ? "" : " IGNORED", 
                          ieee80211_sm_get_current_state_name(ss->ss_hsm_handle),
                          ieee80211_resmgr_get_notification_type_name(notification->type), 
                          notification->type,
                          notification->status);
                       ...
                       scan_request_data_t *request_data)
{
    ...
    for (i = 0; i < params->num_bssid; ++i) {
        OS_MEMCPY(ss->ss_bssid[i], params->bssid_list[i], IEEE80211_ADDR_LEN);
    ...
    if (params->flags & IEEE80211_SCAN_ADD_BCAST_PROBE) {
        ...
        if (!bcast_bssid && (ss->ss_nbssid < IEEE80211_SCAN_MAX_BSSID)) {
            ...
            OS_MEMCPY(ss->ss_bssid[ss->ss_nbssid],broadcast_addr, IEEE80211_ADDR_LEN);
            ss->ss_nbssid++;
        }
    }
    ...
    scanner_construct_chan_list(ss);
    if (ss->ss_nchans == 0) {
    ...
    if (! ieee80211_resmgr_active(ss->ss_ic)) {
        ieee80211_sta_power_register_event_handler(vaphandle->iv_pwrsave_sta,
...
int ieee80211_scan_set_priority(ieee80211_scanner_t      ss, 
                                IEEE80211_SCAN_REQUESTOR requestor,
                                IEEE80211_SCAN_ID        scan_id,
...
int ieee80211_scan_set_forced_flag(ieee80211_scanner_t      ss, 
                                   IEEE80211_SCAN_REQUESTOR requestor,
                                   IEEE80211_SCAN_ID        scan_id, 
                                   bool                     forced_flag)
{
                        ...
                        u_int32_t                flags)
{
    ...
    if (flags & IEEE80211_SCAN_CANCEL_SYNC) {
        ...
        if (flags & IEEE80211_SCAN_CANCEL_WAIT) {
            ...
            while (ss->ss_info.si_scan_in_progress || ss->ss_completion_indications_pending) {
                /*
                ...
                IEEE80211_SCAN_PRINTF(ss, IEEE80211_MSG_SCAN, "%s: Waiting for scan to complete run_counter=%d/%d scan_id=%d/%d flags=%d,%d\n",
                    ...
                    initial_run_counter, ss->ss_run_counter,
                    scan_id, ss->ss_scan_id,
                    ss->ss_info.si_scan_in_progress, ss->ss_completion_indications_pending);
...
int ieee80211_get_last_scan_info(ieee80211_scanner_t ss, ieee80211_scan_info *info)
{
    ...
    if (reason == IEEE80211_REASON_INTERNAL_STOP) {
        ...
        if ((! ss->ss_info.si_in_home_channel) || (ss->ss_suspending_traffic)) {
            ...
            default:     /* Resource Manager not active. */
...
#endif
                /* Fix for evid 98578. Ping fails after scanning.
                 * set_channel drains the txqs which finally disabled them.
                 * no trigger was inititated to re-schedule them
                 * set_channel has done with draining txqs.
                 * Enabling them now - san finishes here.*/
                ...
                {
                    if (ss->ss_is_connected(ss->ss_vap)) {
                        if (ieee80211_sta_power_unpause(ss->ss_vap->iv_pwrsave_sta) != EOK) {
                            /*
                            ...
                            void                *event_data) 
{
    ...
    if (event == SCANNER_EVENT_EXTERNAL_EVENT) {
        struct ieee80211_scan_event_data    *scan_event = event_data;
...
static void scanner_state_idle_entry(void *context)
{
    ...
    if (ss->ss_vap != NULL) {
        ...
        if (! ieee80211_resmgr_active(ss->ss_ic)) {                                          
            (void)ieee80211_sta_power_unregister_event_handler(ss->ss_vap->iv_pwrsave_sta, 
                                                               ieee80211_scan_handle_sta_power_events, 
                                                               ss);
        }
    }
...
static void scanner_state_suspending_traffic_entry(void *context)
{
    ...
    if (! ieee80211_resmgr_active(ss->ss_ic)) {
        ...
        status = ieee80211_sta_power_pause(ss->ss_vap->iv_pwrsave_sta, ss->ss_fakesleep_timeout);
        if (status != EOK) 
        {
...
static void scanner_state_foreign_channel_entry(void *context)
{
     ...
     * a BSS channel or to a Foreign Channel), whiless_offchannel_time is
...
static void scanner_state_bss_channel_entry(void *context)
{
    ...
    if ((ieee80211_sm_get_curstate(ss->ss_hsm_handle) != SCANNER_STATE_BSS_CHANNEL) &&
        ...
        if (! ieee80211_resmgr_active(ss->ss_ic)) {
            ...
            ss->ss_ic->ic_antenna_diversity_resume(ss->ss_ic);
#endif
            /* Let the traffic resume when it comes to home channel(after set_channel) */
            ss->ss_ic->ic_scan_enable_txq(ss->ss_ic);
        }
...
#define IEEE80211_2GHZ_FREQUENCY_THRESHOLD    3000            // in kHz
    ...
    for (i = 0; i < IEEE80211_N(scan_order); ++i) {
        ...
        if (scan_order[i] < IEEE80211_2GHZ_FREQUENCY_THRESHOLD) { /* 2GHZ channel */
            if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11NG_HT20)) {
                c = ieee80211_find_channel(ic, scan_order[i], IEEE80211_CHAN_11NG_HT20); 
            }

            if (c == NULL) {
                c = ieee80211_find_channel(ic, scan_order[i], IEEE80211_CHAN_G);
            }

            if (c == NULL) {
                c = ieee80211_find_channel(ic, scan_order[i], IEEE80211_CHAN_PUREG);
            }

            if (c == NULL) {
                c = ieee80211_find_channel(ic, scan_order[i], IEEE80211_CHAN_B);
            }
        } else {
            if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11NA_HT20)) {
                c = ieee80211_find_channel(ic, scan_order[i], IEEE80211_CHAN_11NA_HT20); 
            }

            if (c == NULL) {
                c = ieee80211_find_channel(ic, scan_order[i], IEEE80211_CHAN_A);
            }
        }
     ...
     */
#if NOT_YET
    /* if a scan is in progress,reconstruct channel list and restart the scan */
    if (ieee80211_sm_get_curstate(ss->ss_hsm_handle) != SCANNER_STATE_IDLE)  {
        scanner_construct_chan_list(ss);
        ss->ss_next = 0; /* restart from 0 */
        ieee80211_sm_dispatch(ss->ss_hsm_handle,
                              SCANNER_EVENT_RESTART_REQUEST,
                              0,
                              NULL); 
    }
#endif
...
int ieee80211_scan_scanentry_received(ieee80211_scanner_t ss, bool bssid_match)
{
                          ...
                          bool (*is_sw_txq_empty)    (wlan_dev_t))
{
    ...
    *ss = (ieee80211_scanner_t) OS_MALLOC(osdev, sizeof(struct ieee80211_scanner), 0);
    if (*ss) {
        ...
        if (! (*ss)->ss_hsm_handle) {
            OS_FREE((*ss));
            adf_os_print("%s: ieee80211_sm_create failed\n", __func__);
            return ENOMEM;
        }
 ...
 */
void ieee80211_set_default_scan_parameters(struct ieee80211vap   *vap,
...
int ieee80211_scan_get_requestor_id(ieee80211_scanner_t ss, u_int8_t *module_name, IEEE80211_SCAN_REQUESTOR *requestor)
{
    int    i, j;
...
void ieee80211_scan_clear_requestor_id(ieee80211_scanner_t ss, IEEE80211_SCAN_REQUESTOR requestor)
{
...
u_int8_t *ieee80211_scan_get_requestor_name(ieee80211_scanner_t ss, IEEE80211_SCAN_REQUESTOR requestor)
{
    int    i;
...
{
    return ieee80211_scan_scheduler_set_priority(ieee80211_scan_scheduler_object(vaphandle->iv_ic->ic_scanner),
                                                 vaphandle,
                                                 requestor,
                                                 scan_id_mask, 
                                                 scanPriority);
...
int wlan_scan_set_forced_flag(wlan_if_t                vaphandle, 
                              ...
                              bool                     forced_flag)
{
    return ieee80211_scan_scheduler_set_forced_flag(ieee80211_scan_scheduler_object(vaphandle->iv_ic->ic_scanner),
                                                    vaphandle,
                                                    requestor,
                                                    scan_id_mask, 
                                                    forced_flag);
...
{
#if ATH_SUPPORT_MULTIPLE_SCANS
    return ieee80211_scan_scheduler_remove(vaphandle->iv_ic->ic_scanner->ss_scheduler,
                                           vaphandle,
                                           requestor,
                                           scan_id_mask,
                                           flags);
     ...
     */
    return ieee80211_scan_stop(vaphandle->iv_ic->ic_scanner,
                               requestor,
...
{
    return ieee80211_scan_scheduler_get_requests(vaphandle->iv_ic->ic_scanner->ss_scheduler,
                                                 scan_request_info,
                                                 total_requests,
                                                 returned_requests);
}
...
int wlan_scan_in_progress(wlan_if_t vaphandle)
{
     ...
     */
    if (vaphandle->iv_ic->ic_scanner->ss_info.si_scan_in_progress) {
        return true;
    }
    else {
        return ieee80211_scan_scheduler_scan_in_progress(vaphandle->iv_ic->ic_scanner->ss_scheduler);
    }
#else
    return (vaphandle->iv_ic->ic_scanner->ss_info.si_scan_in_progress);
#endif
}
...
void wlan_set_default_scan_parameters(wlan_if_t             vaphandle, 
                                      ...
                                      int                   peer_count)
{
     ieee80211_set_default_scan_parameters(vaphandle,
                                           scan_params, 
                                           opmode,
                                           active_scan_flag, 
                                           high_priority_flag,
                             ...
                             IEEE80211_PRIORITY_MAPPING *p_mapping_table)
{
     ...
     * in some platforms.
     */
    return ieee80211_scan_scheduler_set_priority_table(
        ((struct ieee80211com *) devhandle)->ic_scanner->ss_scheduler,
        opmode,
        number_rows,
        p_mapping_table);
}
                             ...
                             IEEE80211_PRIORITY_MAPPING **p_mapping_table)
{
     ...
     * in some platforms.
     */
    return ieee80211_scan_scheduler_get_priority_table(
        ((struct ieee80211com *) devhandle)->ic_scanner->ss_scheduler,
        opmode,
        p_number_rows,
        p_mapping_table);
