...
{
    if (caps & (NAWDS_REPEATER_CAP_TS | NAWDS_REPEATER_CAP_DS)) {
        if (!(caps & (NAWDS_REPEATER_CAP_HT20 | NAWDS_REPEATER_CAP_HT2040)))
            return 0;
    }
    return 1;
}
...
ieee80211_nawds_attach(struct ieee80211vap *vap)
{
    ...
    OS_RWLOCK_INIT(&vap->iv_nawds.lock);
}
...
static int
ieee80211_nawds_config_base(struct ieee80211_node *ni)
{
...
reconfigure:
    ...
    /* base configuration */
    ieee80211_nawds_config_base(ni);

    /* HT capability */
    if (caps & (NAWDS_REPEATER_CAP_HT20 | NAWDS_REPEATER_CAP_HT2040 | NAWDS_REPEATER_CAP_DS)) {
...
int wlan_nawds_config_mac(wlan_if_t vaphandle, char *macaddr, char caps)
{
    ...
    OS_RWLOCK_WRITE_LOCK(&nawds->lock, &lock_state);
