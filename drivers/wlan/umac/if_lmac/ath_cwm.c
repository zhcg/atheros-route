        ...
        enum cwm_opmode opmode)
{
	...
	if((chwidth == CWM_WIDTH40) && (cw_struct->cw_mode == CWM_MODE20))
		return;
    ...
    CWM_RATE_UPDATENODE_CB(cw_struct, pNode);
}
...
cwm_debuginfo(struct ath_cwm *cw_struct)
{
	...
	CWM_DBG_PRINT(cw_struct->wlandev, "%s: ac_hwstate.ht_phymode %d\n",
        __func__, cw_struct->ac_hwstate.ht_phymode);
	CWM_DBG_PRINT(cw_struct->wlandev, "%s: ac_hwstate.ht_extoff %d\n",
        __func__, cw_struct->ac_hwstate.ht_extoff);
#endif
}
