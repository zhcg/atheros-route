...
static int ieee80211_hbr_getstate(wlan_hbr_sm_t sm, void *arg1, void *arg2)
{
    ...
    block = sm->hbr_block;
    printk("%d\t %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\t%s\t%s\t%s\t%d\n", 
        *(int *)arg1, sm->hbr_addr[0], sm->hbr_addr[1], sm->hbr_addr[2], sm->hbr_addr[3],
        ...
        (block==0)?"No":"Yes", sm->hbr_ni->ni_stats.ns_tx_dropblock);
    (*(int *)arg1) ++;
    return 0;
}
...
{
    if (ni) {
        printk("%2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\t%s\n",
...
int ieee80211_hbr_attach(struct ieee80211vap *vap)
{
    ...
    OS_INIT_TIMER(vap->iv_ic->ic_osdev, &vap->iv_hbr_list->hbr_timer, ieee80211_hbr_timer, vap);
    OS_SET_TIMER(&vap->iv_hbr_list->hbr_timer, vap->iv_hbr_list->hbr_timeout);
    return 1;
}
