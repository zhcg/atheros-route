...
#define MAX_TSFSYNC_TIME   6     /* number of beacons */
...
static void ieee80211_assoc_state_disassoc_entry(void *ctx) 
{
    ...
    if (wlan_mlme_disassoc_request(sm->vap_handle, 
                                   cur_bssid, IEEE80211_REASON_ASSOC_LEAVE) !=0 ) {
...
int wlan_assoc_sm_start(wlan_assoc_sm_t smhandle, wlan_scan_entry_t scan_entry, u_int8_t *curbssid)
{
    ...
    if (curbssid) {
           OS_MEMCPY(smhandle->prev_bssid,curbssid,IEEE80211_ADDR_LEN);
    }
