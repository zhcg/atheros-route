...
void
ar5212SetupRegs(struct ath_diag *atd, int what)
{
	size_t space = 0;
	u_int8_t *cp;
	...
	cp = atd->ad_in_data;
	if (what & DUMP_PUBLIC) {
		memcpy(cp, ar5212PublicRegs, sizeof(ar5212PublicRegs));
		cp += sizeof(ar5212PublicRegs);
	}
	if (what & DUMP_INTERRUPT) {
		memcpy(cp, ar5212InterruptRegs, sizeof(ar5212InterruptRegs));
		cp += sizeof(ar5212InterruptRegs);
	}
	if (what & DUMP_XR) {
		memcpy(cp, ar5212XRRegs, sizeof(ar5212XRRegs));
		cp += sizeof(ar5212XRRegs);
	}
	if (what & DUMP_KEYCACHE) {
		memcpy(cp, ar5212KeyCacheRegs, sizeof(ar5212KeyCacheRegs));
		cp += sizeof(ar5212KeyCacheRegs);
	}
	if (what & DUMP_BASEBAND) {
		memcpy(cp, ar5212BasebandRegs, sizeof(ar5212BasebandRegs));
		cp += sizeof(ar5212BasebandRegs);
	}
}

void
ar5212DumpRegs(FILE *fd, int what)
{
