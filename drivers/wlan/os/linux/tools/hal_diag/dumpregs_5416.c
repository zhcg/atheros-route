...
static const HAL_REGRANGE ar5416LogicAnalyzerRegs[] = {
	/* NB: keep these sorted by address */
	{ AR_MAC_PCU_TRACE_REG_START, AR_MAC_PCU_TRACE_REG_END},
};

static const HAL_REGRANGE ar5416DmaDbgRegs[] = {
	/* NB: keep these sorted by address */
	{ AR_DMADBG_0, AR_DMADBG_7},
};

void
ar5416SetupRegs(struct ath_diag *atd, int what)
{
	size_t space = 0;
	u_int8_t *cp;
	...
	if (what & DUMP_LA)
		space += sizeof(ar5416LogicAnalyzerRegs);
	if (what & DUMP_DMADBG)
		space += sizeof(ar5416DmaDbgRegs);
	...
	cp = atd->ad_in_data;
	if (what & DUMP_PUBLIC) {
		memcpy(cp, ar5416PublicRegs, sizeof(ar5416PublicRegs));
		cp += sizeof(ar5416PublicRegs);
	}
	if (what & DUMP_INTERRUPT) {
		memcpy(cp, ar5416InterruptRegs, sizeof(ar5416InterruptRegs));
		cp += sizeof(ar5416InterruptRegs);
	}
	if (what & DUMP_KEYCACHE) {
		memcpy(cp, ar5416KeyCacheRegs, sizeof(ar5416KeyCacheRegs));
		cp += sizeof(ar5416KeyCacheRegs);
	}
	if (what & DUMP_BASEBAND) {
		memcpy(cp, ar5416BasebandRegs, sizeof(ar5416BasebandRegs));
		cp += sizeof(ar5416BasebandRegs);
	}
	if (what & DUMP_LA) {
		memcpy(cp, ar5416LogicAnalyzerRegs, sizeof(ar5416LogicAnalyzerRegs));
		cp += sizeof(ar5416LogicAnalyzerRegs);
	}
	if (what & DUMP_DMADBG) {
		memcpy(cp, ar5416DmaDbgRegs, sizeof(ar5416DmaDbgRegs));
		cp += sizeof(ar5416DmaDbgRegs);
	}
}

void
ar5416DumpRegs(FILE *fd, int what)
{
