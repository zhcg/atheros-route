...
radarGetNol(struct radarhandler *radar, char *fname) 
{
    ...
    fp = fopen(fname, "wb");
    if (!fp) {
	    snprintf(buf, sizeof(buf), "%s: fopen %s error",__func__, fname);
	    perror(buf);
	    return;
	}
    ...
    {
        printf("nol:%d channel=%d ticksfree=%llu\n", i, nolinfo.dfs_nol[i].nolfreq, 
                (unsigned long long)nolinfo.dfs_nol[i].ticksfree);
    }    	
