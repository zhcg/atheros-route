...
host_htc_rdy_handler(osdev_t osdev, u_int32_t TotalCredits)
{
    ...
    //adf_os_mem_zero(&connect, sizeof(HTC_SERVICE_CONNECT_REQ));
...
A_STATUS ath_hif_htc_wmi_init(osdev_t osdev)
{
    ...
    //adf_os_mem_zero(&initInfo, sizeof(HTC_INIT_INFO));
...
#if !defined (MAGPIE_HIF_PCI) && !defined (MAGPIE_HIF_GMAC)
