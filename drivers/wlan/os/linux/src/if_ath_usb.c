...
#define ATH_CONTEXT_SIZE 10400
struct ath_usb_softc {
/*    struct ath_softc_net80211   aps_sc; */
    char aps_sc[ATH_CONTEXT_SIZE];
...
/* attach/detach entry */
extern int __ath_attach(u_int16_t devid, struct net_device *dev, HAL_BUS_CONTEXT *bus_context, struct _NIC_DEV * osdev);
...
/************************************/
extern uint32_t OS_Usb_SubmitMsgInUrb(struct _NIC_DEV *osdev);
extern uint32_t OS_Usb_SubmitCmdOutUrb(struct _NIC_DEV *osdev, uint8_t* buf, uint16_t len, void *context);
extern uint32_t OS_Usb_SubmitRxUrb(struct _NIC_DEV *osdev);
extern uint32_t OS_Usb_SubmitTxUrb(struct _NIC_DEV *osdev, uint8_t* buf, uint16_t len, void *context, HIFUSBTxPipe *pipe);
extern uint16_t OS_Usb_GetFreeTxBufferCnt(struct _NIC_DEV *osdev, HIFUSBTxPipe *pipe);
extern uint16_t OS_Usb_GetMaxTxBufferCnt(struct _NIC_DEV *osdev);
extern uint16_t OS_Usb_GetTxBufferCnt(struct _NIC_DEV *osdev, HIFUSBTxPipe *pipe);
extern uint16_t OS_Usb_GetTxBufferThreshold(struct _NIC_DEV *osdev);
extern uint16_t OS_Usb_GetFreeCmdOutBufferCnt(struct _NIC_DEV *osdev);
extern uint16_t OS_Usb_InitTxRxQ(struct _NIC_DEV *osdev);
extern void OS_Usb_Enable_FwRcv(struct _NIC_DEV *osdev);
extern int ath_net80211_context_size(void);
...
void ath_remove_interface(struct _NIC_DEV * osdev);
void athLnxStartPerSecTimer(struct _NIC_DEV *osdev);
...
int MpHtcAttach(struct _NIC_DEV *osdev);
void MpHtcDetach(struct _NIC_DEV *osdev);
void HIF_USBDeviceDetached(struct _NIC_DEV *);

extern u_int32_t athUsbFirmwareDownload(struct _NIC_DEV * osdev, uint8_t* fw, uint32_t len, uint32_t offset);

extern void init_wlan(void);

extern void ath_register_htc_thread_callback(struct _NIC_DEV * osdev);
/************************************/
 ...
 * 
 */
int ath_host_attach(void *scn, uint16_t devid)
{
#define HAL_BUS_TYPE_USB    0x02
    ...
    if (ath_net80211_context_size() > ATH_CONTEXT_SIZE)
    {
        printk(KERN_ERR "SW context size:%d > predefined size (%d)\n", 
                        ath_net80211_context_size(), ATH_CONTEXT_SIZE);
        BUG();
    }
 ...
 * 
 */
void ath_host_detach(struct _NIC_DEV * osdev)
{
...
void ath_os_hif_cb_assign(struct _NIC_DEV * osdev)
{
                      ...
                      const struct usb_device_id *id)
{
    ...
    printk(KERN_INFO "ath_usb_probe\n");
    ...
    if (i == g_magpie_usb_ifc_max) {
        printk("%s: only %u usb devices are supported.\n", __FUNCTION__, i);
        result = -1;
        goto fail;
    } else {
        printk("%s: %dth device inserted\n",__FUNCTION__,g_magpie_usb_device_count);
    }

    /* allocate net devie */
    net = alloc_netdev(sizeof(struct ath_usb_softc), "wifi%d", ether_setup);
    if (net == NULL) {
        printk(KERN_ERR "ath_usb: no memory for device state\n");
        result = -EIO;
        goto fail;
    }
    ...
    {
        printk(KERN_ERR "%s: athUsbFirmwareDownload fail!", __func__);
        result = -EIO;
        goto fail1;
    }
    ...
    if(!athLnxAllocRegInBuf(&(aps_usb_sc->aps_osdev))) {
        printk(KERN_ERR "%s: athLnxAllocRegInBuf fail!\n", __func__);
        result = -EIO;
        goto fail1;  
    }
#endif    
    ...
    if(MpHtcAttach(&(aps_usb_sc->aps_osdev))) {
        printk(KERN_ERR "%s: MpHtcAttach fail!\n", __func__);
        result = -EIO;
        goto fail2;
    }
    ...
    if(g_magpie_usb_device_count == 1) {
        if (ath_create_htc_tx_thread(net) != 0) { 
            printk(KERN_ERR "%s: create tx thread fail\n", __func__); 
            result = -EINVAL; 
            goto fail1; 
        }
    }
    ...
    /* ath/hal attach */
    if (ath_host_attach((void *)&(aps_usb_sc->aps_sc), devid)) {
        printk(KERN_ERR "%s: ath_host_attach fail!\n", __func__);
        result = -EIO;
        goto fail2;
    }
...
static void ath_usb_disconnect(struct usb_interface *interface)
  ...
  {    
    ...
    printk(KERN_INFO "ath_usb_disconnect\n");
    ...
    if(i == g_magpie_usb_ifc_max) {
        printk(KERN_ERR "%s: interface %p not found\n", __func__, interface);
        return;
    }
    ...
    /* set delete flag */
    printk(KERN_INFO "set deleted flag\n");
...
{  
    ...
    printk(KERN_INFO "%s : Max. WiFi device support to %d\n", __func__, g_magpie_usb_ifc_max);
