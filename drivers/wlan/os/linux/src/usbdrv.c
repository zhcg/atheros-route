...
uint32_t athLnxUsbIn(struct _NIC_DEV *osdev, UsbRxUrbContext *RxUrbCtx);
uint32_t athLnxUsbSubmitTxData(struct _NIC_DEV * osdev, HIFUSBTxPipe *pipe);
void athLnx_rx_streammode_timeout_change(struct _NIC_DEV * osdev, u_int32_t val);
...
void MpHtcSuspend(struct _NIC_DEV* osdev);
void MpHtcResume(struct _NIC_DEV* osdev);
...
static void allocTxUrbs(struct _NIC_DEV * osdev, HIFUSBTxPipe *pipe)
{
    int i;
...
{
    struct _NIC_DEV *             osdev = (struct _NIC_DEV *) param;
...
int athLnxAllocRegInBuf(struct _NIC_DEV * osdev)
{
...
void athLnxFreeRegInBuf(struct _NIC_DEV * osdev)
{
    if(osdev->regUsbReadBuf != NULL) {
...
int athLnxAllocAllUrbs(struct _NIC_DEV * osdev)
{
    ...
    for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i)
    {
            ...
            ((endpoint->bmAttributes & 3) == 0x02))
        {
            ...
            printk(KERN_ERR "ep[%d] bulk in: wMaxPacketSize = 0x%x\n", 
            ...
            ((endpoint->bmAttributes & 3) == 0x02))
        {
            ...
            printk(KERN_ERR "ep[%d] bulk out: wMaxPacketSize = 0x%x\n", 
				i, le16_to_cpu(endpoint->wMaxPacketSize));
        }
            ...
            ((endpoint->bmAttributes & 3) == 0x03))
        {
            ...
            printk(KERN_ERR "ep[%d] interrupt in: wMaxPacketSize = 0x%x\n", 
				i, le16_to_cpu(endpoint->wMaxPacketSize));
            printk(KERN_ERR "ep[%d] interrupt in: int_interval = %d\n", 
            ...
            ((endpoint->bmAttributes & 3) == 0x03))
        {
            ...
            printk(KERN_ERR "ep[%d] interrupt out: wMaxPacketSize = 0x%x\n", 
				i, le16_to_cpu(endpoint->wMaxPacketSize));
            printk(KERN_ERR "ep[%d] interrupt out: int_interval = %d\n", 
				i, endpoint->bInterval);
...
void athLnxFreeAllUrbs(struct _NIC_DEV * osdev)
{
    int i;
...
void athLnxUnlinkAllTxUrbs(struct _NIC_DEV * osdev)
{
    int i;
...
void athLnxUnlinkAllUrbs(struct _NIC_DEV * osdev)
{
    int i;
...
uint32_t athLnxSubmitRegInUrb(struct _NIC_DEV * osdev);
...
{
    int status;
    struct _NIC_DEV * osdev = (struct _NIC_DEV *)urb->context;
...
uint32_t athLnxSubmitRegInUrb(struct _NIC_DEV * osdev)
{
    ...
    else
    {
        if (osdev->tm_rxbuf_act == 0)
        {
            ...
            //adf_os_timer_start(&osdev->tm_rxbuf_alloc, RX_URB_BUF_ALLOC_TIMER);
...
UsbUrbContext_t* athLnxAllocCmdUrbCtx(struct _NIC_DEV * osdev)
{
    int i;
...
{    	
    ...
    struct _NIC_DEV * osdev = (struct _NIC_DEV *)ctx->osdev;
    struct sk_buff *buf;
...
struct sk_buff * athLnxGetUsbRxBuffer(struct _NIC_DEV * osdev)
{
    struct sk_buff * buf;
    unsigned long irqFlag;    
...
uint32_t athLnxPutUsbRxBuffer(struct _NIC_DEV * osdev, struct sk_buff * buf)
{
    uint16_t idx;
    unsigned long irqFlag;    
...
error:
    ...
    else
    {
        ...
        //adf_os_timer_cancel(&osdev->tm_rxbuf_alloc);
        del_timer(&osdev->tm_rxbuf_alloc);
    }
}

void athLnxStartPerSecTimer(struct _NIC_DEV * osdev)
{
...
void athLnxStopPerSecTimer(struct _NIC_DEV * osdev)
{
...
void athLnxInitUsbTxQ(struct _NIC_DEV * osdev)
{
...
void athLnxInitUsbRxQ(struct _NIC_DEV * osdev)
{
    uint16_t i;
    struct sk_buff* buf;
...
#endif
{
    ...
    struct _NIC_DEV * osdev = RxUrbCtx->osdev;
    ...
    /* Check status for URB */
    if (urb->status != 0){
        printk(KERN_ERR "athLnxUsbDataIn_callback() : status=0x%x\n", urb->status);
        if ((urb->status != -ENOENT) && (urb->status != -ECONNRESET)
            && (urb->status != -ESHUTDOWN))
        {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,5,0))    
    ...
    if (remain_len != 0)
    {
        ...
        if ((remain_len + check_len) > remain_buf->len) {
            printk(KERN_ERR "%s: not enough space in the remain buffer, len: %d, remain_len: %d, check_len: %d\n",
    ...
    while(index < urb->actual_length)
    {
        ...
        if (pkt_tag == ATH_USB_STREAM_MODE_TAG)
        {
            ...
            if (index > ZM_MAX_RX_BUFFER_SIZE)
            {
                ...
                printk(KERN_INFO "%s: Frame is transmitted in the next Transfer\n", __func__);
                ...
                if (osdev->check_len > pkt_len) {
                    printk(KERN_ERR "%s: copy too many bytes, check_len: %d, pkt_len: %d\n",
    ...
    if (rxBufPoolIndex == 1)
    {
        ...
        {
            RxUrbCtx->inUse = 0;
            printk(KERN_DEBUG "%s: can't allocate rx buffer, start rx buf timer\n", __func__);
...
#endif
    {
        ...
        for(ii = packet_over_trans; ii < rxBufPoolIndex; ii++)
        {
            ...
            if (new_buf == NULL)
            {
                printk(KERN_DEBUG "%s: can't allocate rx buffer\n", __func__);
...
UsbTxUrbContext* athLnxGetFreeTxUrb(struct _NIC_DEV * osdev,  HIFUSBTxPipe *pipe)
{
    uint16_t idx;
    unsigned long irqFlag;
...
void athLnxPutTxUrb(struct _NIC_DEV * osdev, HIFUSBTxPipe *pipe, UsbTxUrbContext *ctx)
{
    unsigned long irqFlag;
...
uint16_t athLnxCheckTxBufferCnt(struct _NIC_DEV * osdev,  HIFUSBTxPipe *pipe, u_int32_t *timestamp)
{
    uint16_t TxBufCnt;
    unsigned long irqFlag;
...
UsbTxQ_t *athLnxGetUsbTxBuffer(struct _NIC_DEV *osdev, HIFUSBTxPipe *pipe, UsbTxQ_t *TxQ)
{
    uint16_t idx;
    unsigned long irqFlag;    
    u_int16_t max_tx_buf_num = ((osdev->isMagpie == 1) ? ZM_MAX_TX_BUF_NUM : ZM_MAX_TX_BUF_NUM_K2); 
    ...
    else
    {
        ...
        {
            printk(KERN_ERR "athLnxGetUsbTxBuffer UsbTxBufQ inconsistent: \
...
#endif
{
    ...
    struct _NIC_DEV * osdev = ctx->osdev;
...
uint32_t athLnxUsbSubmitTxData_NONDATA(struct _NIC_DEV * osdev, HIFUSBTxPipe *pipe)
{
    uint32_t ret;
...
uint32_t athLnxUsbSubmitTxData(struct _NIC_DEV * osdev, HIFUSBTxPipe *pipe)
{
    ...
    if (usbTxAggCnt == 1)
    {
        ...
        if (skb_headroom(TxData->buf) < 4)
        {
            ...
            printk(KERN_ERR "%s: header room is not enough\n", __func__);
        ...
        *pUsbTxHdr++ = cpu_to_le16(TxData->hdrlen + TxData->snapLen + 
                               (TxData->buf->len - TxData->offset - 4) +  TxData->tailLen);

        *pUsbTxHdr++ = cpu_to_le16(0x697e);
        ...
        {
            printk(KERN_EMERG "%s: buffer is not 2 byte alignment\n", __func__);
            BUG();
        }
...
uint16_t athLnxPutUsbTxBuffer(struct _NIC_DEV *osdev, struct sk_buff * buf, HIFUSBTxPipe *pipe)
{
    uint16_t idx;
    UsbTxQ_t *TxQ;
    unsigned long irqFlag;    
    ...
    {
#if 0
        printk(KERN_ERR "athLnxPutUsbTxBuffer UsbTxBufQ inconsistent: TxBufHead: %d, TxBufTail: %d, TxBufCnt: %d\n",
...
uint32_t athLnxUsbOut(struct _NIC_DEV *osdev, struct sk_buff * buf, HIFUSBTxPipe *pipe)
{
    uint32_t ret = 0;
    ...
    {
        /* free packet */
        printk(KERN_ERR "athLnxPutUsbTxBuffer Error!!\n");
        return 0xffff;
    }
...
uint32_t athLnxUsbSubmitControl(struct _NIC_DEV *osdev, 
                                 uint8_t req,
                                 uint8_t reqtype,
                                 uint16_t value, 
                                 uint16_t index, 
                                 ...
                                 uint32_t size)
{
    ...
    printk(KERN_ERR "reqtype = 0x%02x\n", reqtype);
    ...
    if (result < 0)
    {
        printk(KERN_ERR "athLnxUsbSubmitControl() failed, result=0x%x\n", result);
        ret = 1;
    }
...
#if ZM_USB_TX_STREAM_MODE
void
athLnx_rx_streammode_timeout_change(struct _NIC_DEV *osdev, u_int32_t val)
{
...
u_int32_t
OS_Usb_SubmitMsgInUrb(struct _NIC_DEV *osdev)
{
    osdev->RegInFailCnt = 0;
...
u_int32_t 
OS_Usb_SubmitCmdOutUrb(struct _NIC_DEV *osdev, uint8_t* buf, uint16_t len, void *context)
{
    ...
    if ( ctx == NULL ) {
        printk("!!Error can't get CmdOutUrb!!\n");
        return 1;
    }    
    ...
    while (transfer_len > 0) {
        ...
        if (remain_ctx == NULL)
        {
            printk("%s: can't allocate UrbContext\n", __func__);
    ...
    if (ret != 0) {
        printk("%s athLnxUsbSubmitBulkUrb fail, ret: 0x%08x\n", __func__, ret);
        ctx->inUse = 0;
        goto error;
    }
    ...
    for (ii = 0; ii < urb_num; ii++)
    {
        ...
        if (status != 0) {
            printk("%s athLnxUsbSubmitBulkUrb fail, status: 0x%08x\n", __func__, status);
...
u_int32_t
OS_Usb_SubmitRxUrb(struct _NIC_DEV *osdev)
{
...
u_int32_t 
OS_Usb_SubmitTxUrb(struct _NIC_DEV *osdev, uint8_t* buf, uint16_t len, void *context, HIFUSBTxPipe *pipe)
{
    return athLnxUsbOut(osdev, (struct sk_buff *)context, pipe);
}

u_int16_t
OS_Usb_GetFreeTxBufferCnt(struct _NIC_DEV *osdev, HIFUSBTxPipe *pipe)
{
...
u_int16_t
OS_Usb_GetMaxTxBufferCnt(struct _NIC_DEV *osdev)
{
    return ((osdev->isMagpie == 1) ? ZM_MAX_TX_BUF_NUM : ZM_MAX_TX_BUF_NUM_K2);
}

u_int16_t
OS_Usb_GetTxBufferCnt(struct _NIC_DEV *osdev, HIFUSBTxPipe *pipe)
{
    uint16_t TxBufCnt;
    unsigned long irqFlag;
...
u_int16_t
OS_Usb_GetTxBufferThreshold(struct _NIC_DEV *osdev)
{
    return ZM_MAX_TX_AGGREGATE_NUM;    
}

u_int16_t
OS_Usb_GetFreeCmdOutBufferCnt(struct _NIC_DEV *osdev)
{
    return 1;
}

u_int16_t
OS_Usb_InitTxRxQ(struct _NIC_DEV *osdev)
{
    /* Initialize USB TxQ */
...
u_int32_t
athUsbFirmwareDownload(struct _NIC_DEV *osdev, uint8_t* fw, uint32_t len, uint32_t offset)
{
    ...
    while (len > 0) 
    {
        ...
        if (result != 0)
        {
            printk(KERN_ERR "FIRMWARE_DOWNLOAD failed\n");
            ret = 1;
            goto exit;
        }
    ...
    {   
        ...
        if (result != 0)
        {
            printk(KERN_ERR "FIRMWARE_DOWNLOAD_COMP failed\n");
            ret = 1;
            goto exit;
        }
        else if ((fwdl_stat&0xff) == 0x0)
        {
            printk(KERN_ERR "SUCCESS [FW downlaod] -- confirmed by target. result = 0x%08x\n", 
                         result);
        }
        else
        {
            printk(KERN_ERR "FAIL [FW download] -- 0x%08x said by target. result = 0x%08x\n", 
                          fwdl_stat, result);
            ret = 1;
            goto exit;            
        }
...
int athLnxRebootCmd(struct _NIC_DEV *osdev)
{
    int ret;
...
void athLnxFlushTxQ(struct _NIC_DEV *osdev)
{
    UsbTxQ_t *TxData, TxQ;

    /* Flush buffer in the TxQ */
    while ((TxData = athLnxGetUsbTxBuffer(osdev, &osdev->TxPipe, &TxQ)) != NULL) {
        printk(KERN_INFO "%s: free buffer in TxPipe\n", __func__);
        dev_kfree_skb_any(TxData->buf);
    }

    while ((TxData = athLnxGetUsbTxBuffer(osdev, &osdev->HPTxPipe, &TxQ)) != NULL) {
        printk(KERN_INFO "%s: free buffer in HPTxPipe\n", __func__);
        dev_kfree_skb_any(TxData->buf);
    }
}

void athLnxFwRecover(struct _NIC_DEV *osdev)
{
    /* Watch dog Handler */
    if (osdev->enablFwRcv) {
        printk("\nstart FW recover \n");
        ...
        if (athUsbFirmwareDownload(osdev, osdev->Image, osdev->ImageSize,
            ...
            printk(KERN_ERR "%s: athUsbFirmwareDownload fail!\n", __func__);

            osdev->enablFwRcv = 0;
            return;
        }
        else {
            printk("%s_%d: Firmware download success !\n", __func__, __LINE__);
            msleep(100);

            /* urb */
            if (!athLnxAllocAllUrbs(osdev)) {
                printk(KERN_ERR "%s: athLnxAllocAllUrbs fail!\n", __func__);
            }
...
void athLnxScheduleWorkQeueue(struct _NIC_DEV *osdev, int flag)
{
    if (osdev == NULL) {
        printk("%s osdev is NULL\n", __func__);
        return;
    }
...
int athLnxCreateRecoverWorkQueue(struct _NIC_DEV *osdev)
{
...
void
OS_Usb_Enable_FwRcv(struct _NIC_DEV *osdev)
{
    osdev->enablFwRcv = 1;
