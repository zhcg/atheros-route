...
#ifdef AH_SUPPORT_XR
static HAL_BOOL		ar5212SetXrMode(struct ath_hal *ah, HAL_OPMODE opmode,HAL_CHANNEL *chan);
#endif
 ...
 */
HAL_BOOL ar5212Reset(struct ath_hal *ah, HAL_OPMODE opmode, HAL_CHANNEL *chan,
...
#undef IS
	...
	if ((IS_2413(ah) || IS_5413(ah)) &&
		(AH_PRIVATE(ah)->ah_macVersion != AR_SREV_2413)) {
		/*
...
//	case CHANNEL_XR_G:
	...
	if (opmode != HAL_M_STA && ahp->ah_xrEnable && !ar5212SetXrMode(ah, opmode,chan)) {
		HDPRINTF(ah, HAL_DBG_RESET, "%s: unable to setup XR mode\n", __func__);
		FAIL(HAL_EIO);
	}
#endif /* AH_SUPPORT_XR */
	...
	/* TPC for self-generated frames */
 	...
 	if (tpcPow > ichan->maxTxPower)
 		tpcPow = ichan->maxTxPower;
 	...
 	if (tpcPow > (2*ichan->maxRegTxPower - twiceAntennaReduction))
 		tpcPow = 2*ichan->maxRegTxPower - twiceAntennaReduction;
	 ...
	 */
	AH_PRIVATE(ah)->ah_txTrigLevel = MS(OS_REG_READ(ah, AR_TXCFG), AR_FTRIG);
	...
	if (bChannelChange) {
		...
		if (opmode == HAL_M_HOSTAP && (chan->privFlags & CHANNEL_DFS) &&
...
#ifdef AH_SUPPORT_DFS
	if (((ichan->privFlags & CHANNEL_DFS) != 0) &&
...
ar5212SetResetReg(struct ath_hal *ah, u_int32_t resetMask)
{
	...
	if (resetMask & (AR_RC_MAC | AR_RC_PCI)) {
		...
		if (ar5212MacStop(ah) != AH_TRUE) {
			...
			if (ah->ah_bustype == HAL_BUS_TYPE_PCI) {
...
ar5212LoadNF(struct ath_hal *ah, HAL_CHANNEL_INTERNAL *chan)
{
    ...
    val &= 0xFFFFFE00;
    val |= (((u_int32_t)(h->base.privNF[0]) << 1) & 0x1ff);
...
{
    h->nfCalBuffer[h->base.currIndex][0] = nf;

    if (++h->base.currIndex >= HAL_NF_CAL_HIST_LEN_FULL) {
        h->base.currIndex = 0;
    }
    if (h->base.invalidNFcount > 0) {
        ...
        h->base.privNF[0] = ar5212GetNfHistMid(h, 0);
    }
    return;
}
                      ...
                      u_int16_t extra_txpow, u_int16_t tpcInDb)
{
    ...
    CHECK_CCK(ah, AH_PRIVATE(ah)->ah_curchan, isBmode);
    ...
    ret =  ar5212SetTransmitPower(ah, AH_PRIVATE(ah)->ah_curchan, 
        dummyXpdGains, AH_PRIVATE(ah)->ah_powerLimit, AH_FALSE, tpcInDb);
    UNCHECK_CCK(ah, AH_PRIVATE(ah)->ah_curchan, isBmode);
    return ret;
}
...
#define	N(a)			(sizeof (a) / sizeof (a[0]))
	...
	powerLimit = AH_MIN(powerLimit, AH_PRIVATE(ah)->ah_powerLimit);
	...
	if (powerLimit >= MAX_RATE_POWER)
		tpcInDb = AH_MAX(tpcInDb, tpcScaleReductionTable[AH_PRIVATE(ah)->ah_tpScale]);
