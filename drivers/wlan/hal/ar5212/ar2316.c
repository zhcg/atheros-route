...
#define	RF_BANK_SETUP(_pb, _ix, _col) do {				    \
	...
	HDPRINTF(ah, HAL_DBG_CHANNEL, "==>%s:chan 0x%x flag 0x%x modesIndex 0x%x\n", 
...
static HAL_BOOL
ar2316GetChipPowerLimits(struct ath_hal *ah, HAL_CHANNEL *chans, u_int32_t nchans)
{
	...
	for (i=0; i<nchans; i++) {
		if (ar2316GetChannelMaxMinPower(ah, &chans[i], &maxPow, &minPow)) {
			...
			HDPRINTF(ah, HAL_DBG_RF_PARAM, "Failed setting power table for nchans=%d\n", i);
			retVal = AH_FALSE;
		}
	}
#ifdef AH_DEBUG
...
ar2316RfAttach(struct ath_hal *ah, HAL_STATUS *status)
{
	...
	HALASSERT(ahp->ah_analogBanks == AH_NULL);
	ahp->ah_analogBanks = ath_hal_malloc(ah, sizeof(AR5212_RF_BANKS_2316));
	if (ahp->ah_analogBanks == AH_NULL) {
