        ...
        HAL_BOOL lastSeg, const void *ds0)
{
	...
	if (firstSeg) {
		 ...
		 */
		ads->ds_ctl1 |= segLen[0] | (lastSeg ? 0 : AR_More);
...
HAL_BOOL ar5212FillKeyTxDesc(struct ath_hal *ah, void *ds, HAL_KEY_TYPE keyType)
{
    return AH_TRUE;
}
...
#ifdef _WIN64
void 
ar5212GetDescLinkPtr(struct ath_hal *ah, void *ds, u_int32_t __unaligned **link)
#else
void
...
ar5212ProcTxDesc(struct ath_hal *ah, void *desc)
{
	...
	if (ahp->ah_dfsDisabled & HAL_DFS_TIMEMARK) {
		...
		if ((thisTsf - ahp->ah_dfsMarkTime) >= HAL_RADAR_DISABLE_TIME) {
			...
			for (q=0;q<AH_PRIVATE(ah)->ah_caps.halTotalQueues; q++) {
					...
					u_int flags, u_int32_t smartAntenna)
{
	...
	ads->ds_ctl2 = (durUpdateEn ? AR_DurUpdateEna : 0)
	...
	if (flags & (HAL_TXDESC_RTSENA | HAL_TXDESC_CTSENA)) {
		...
		ads->ds_ctl2 |= SM(rtsctsDuration, AR_RTSCTSDuration);
		ads->ds_ctl3 |= SM(rtsctsRate, AR_RTSCTSRate);
	}
#ifdef AH_NEED_DESC_SWAP
