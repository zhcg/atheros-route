...
ar5413WriteRegs(struct ath_hal *ah, u_int modesIndex, u_int freqIndex, int regWrites)
{
	...
	/* Eagle doesn't work with 128/32/16 bytes burst on pb42 or pb44(Hydra). Use 4 bytes*/
	OS_REG_WRITE(ah, AR_RXCFG, 0x00);
#endif

        /*** NMIWar ***/

 	if(IS_5424(ah) && AH_PRIVATE(ah)->ah_config.ath_hal_pcieL1SKPEnable) {
            OS_REG_WRITE(ah, AR_PCIE_PMC, AR_PCIE_PMC_ENABLE_L1 | AR_PCIE_PMC_EN_RESET);
        }
        else{
            /* always disable on-chip compression buffer for Eagle */
            OS_REG_WRITE(ah, AR_COMPBUF_CFG, AR_COMPBUF_DISABLE);
        }
}
...
{
	struct ath_hal_5212 *ahp = AH5212(ah);
    AR5413_VPD_TABLE *pVpdTable = ahp->ah_vpdTable;
	u_int32_t ii, jj, kk;
	...
	u_int16_t *rfXpdGain)
{
	...
	if (!ar5413getGainBoundariesAndPdadcsForPowers(ah, chan->channel,
		pRawDataset, pdGainOverlap_t2,&minCalPower5413_t2,gainBoundaries,
...
ar5413RfAttach(struct ath_hal *ah, HAL_STATUS *status)
{
	...
	HALASSERT(ahp->ah_analogBanks == AH_NULL);
	ahp->ah_analogBanks = ath_hal_malloc(ah, sizeof(AR5212_RF_BANKS_5413));
	if (ahp->ah_analogBanks == AH_NULL) {
	...
	ahp->ah_vpdTable = ath_hal_malloc(ah, sizeof(AR5413_VPD_TABLE));
	if (ahp->ah_vpdTable == AH_NULL) {
