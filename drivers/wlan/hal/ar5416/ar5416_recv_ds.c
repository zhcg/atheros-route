    ...
    u_int8_t *keyix, u_int8_t *status)
{
    ...
    *status = 0;
    ...
    if (status8 & AR_KeyMiss || ((status8 & AR_RxFrameOK) && (status8 & (AR_CRCErr|AR_PHYErr|AR_DecryptCRCErr|AR_MichaelErr)))) {
        *status = HAL_RXERR_INCOMP;
    }
    return HAL_OK;
}
                           ...
                           void *buf_addr)
{
    ...
    {        
        ...
        pcommon->common_dBmAntSignal = MS(adsp->ds_rxstatus4, AR_RxRSSICombined)-96;
        ...
        if (pmacphy != NULL) {            
            ...
            pmacphy->macphy_rssiCombined = MS(adsp->ds_rxstatus4, AR_RxRSSICombined);
            pmacphy->macphy_rssiAnt0Ctl = MS(adsp->ds_rxstatus0, AR_RxRSSIAnt00);
            pmacphy->macphy_rssiAnt1Ctl = MS(adsp->ds_rxstatus0, AR_RxRSSIAnt01);
            pmacphy->macphy_rssiAnt2Ctl = MS(adsp->ds_rxstatus0, AR_RxRSSIAnt02);
            pmacphy->macphy_rssiAnt0Ext = MS(adsp->ds_rxstatus4, AR_RxRSSIAnt10);
            pmacphy->macphy_rssiAnt1Ext = MS(adsp->ds_rxstatus4, AR_RxRSSIAnt11);
            pmacphy->macphy_rssiAnt2Ext = MS(adsp->ds_rxstatus4, AR_RxRSSIAnt12);
            ...
            pmacphy->macphy_ant0Signal = MS(adsp->ds_rxstatus0, AR_RxRSSIAnt00)-96;
            pmacphy->macphy_ant1Signal = MS(adsp->ds_rxstatus0, AR_RxRSSIAnt01)-96;
            pmacphy->macphy_ant2Signal = MS(adsp->ds_rxstatus0, AR_RxRSSIAnt02)-96;
