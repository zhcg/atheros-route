        ...
        HAL_BOOL lastSeg, const void *ds0)
{
        ...
        HALASSERT((segLen[0] &~ AR_BufLen) == 0);
        ...
        if (firstSeg) {
                 ...
                 */
                ads->ds_ctl1 |= segLen[0] | (lastSeg ? 0 : AR_TxMore);
                 ...
                 */
                ads->ds_ctl0 = 0;
                ads->ds_ctl1 = segLen[0] | AR_TxMore;
                ads->ds_ctl2 = 0;
                ads->ds_ctl3 = 0;
 ...
 */
HAL_BOOL
ar5416FillKeyTxDesc(struct ath_hal *ah, void *ds, HAL_KEY_TYPE keyType)
{
    struct ar5416_desc *ads = AR5416DESC(ds);
...
#ifdef _WIN64
void
ar5416GetDescLinkPtr(struct ath_hal *ah, void *ds, u_int32_t __unaligned **link)
#else
void
                                   ...
                                   u_int flags)
{
        ...
        ctl1 = (keyIx != HAL_TXKEYIX_INVALID ? SM(keyIx, AR_DestIdx) : 0)
                             ...
                             u_int flags, u_int32_t smartAntenna)
{
        ...
        if (ap->ah_config.ath_hal_desc_tpc) {
            ...
            txpower = ar5416GetRateTxPower(ah, mode, series[0].RateIndex,
                                           series[0].ChSel);
            ...
            if (AR_SREV_MERLIN_10_OR_LATER(ah)) {
                ...
                for (count=0; count < nseries; count++) {
                    series[count].TxPowerCap -= AR5416_PWR_TABLE_OFFSET_DB * 2;
                }
            }
            ds_ctl0 |= set11nTxPower(0, AH_MIN(txpower, series[0].TxPowerCap));
        }
        ...
        if (ap->ah_config.ath_hal_desc_tpc && AR_SREV_OWL_20_OR_LATER(ah)) {
            ...
            txpower = ar5416GetRateTxPower(
                ah, mode, series[1].RateIndex, series[1].ChSel);
            ads->ds_ctl9 =
                set11nTxPower(1, AH_MIN(txpower, series[1].TxPowerCap));
            txpower = ar5416GetRateTxPower(
                ah, mode, series[2].RateIndex, series[2].ChSel);
            ads->ds_ctl10 =
                set11nTxPower(2, AH_MIN(txpower, series[2].TxPowerCap));
            txpower = ar5416GetRateTxPower(
                ah, mode, series[3].RateIndex, series[3].ChSel);
            ads->ds_ctl11 =
...
void
ar5416Set11nAggrFirst(struct ath_hal *ah, void *ds, u_int aggrLen)
{
    struct ar5416_desc *ads = AR5416DESC(ds);
...
void
ar5416Set11nAggrMiddle(struct ath_hal *ah, void *ds, u_int numDelims)
{
    struct ar5416_desc *ads = AR5416DESC(ds);
    unsigned int ctl6;
