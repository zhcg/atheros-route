       ...
       case HAL_CAP_RIFS_RX_ENABLED:
        ...
        case HAL_CAP_DS:
                return (AR_SREV_MERLIN_20_OR_LATER(ah) &&
                       ...
                       HAL_ENOTSUPP : HAL_OK;
        case HAL_CAP_TS:
                return ((pCap->halTxChainMask & 0x7) != 0x7 ||
...
int
ar5416UpdateNavreg(struct ath_hal *ah)
{
   u_int32_t regval = 0;
...
#ifdef ATH_TX99_DIAG
#ifdef ATH_SUPPORT_HTC
void ar5416_tx99_tx_stopdma(struct ath_hal *ah, u_int32_t hwq_num)
{
    ah_tx99_tx_stopdma(ah, hwq_num);
}

void ar5416_tx99_drain_alltxq(struct ath_hal *ah)
{
    ah_tx99_drain_alltxq(ah);
}
#else
