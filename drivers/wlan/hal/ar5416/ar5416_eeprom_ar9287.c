...
ar9287EepromSetBoardValues(struct ath_hal *ah, HAL_CHANNEL_INTERNAL *chan)
{
    ...
    u_int16_t antWrites[AR9287_ANT_16S];
    struct ath_hal_5416 *ahp = AH5416(ah);
    ar9287_eeprom_t *eep = &ahp->ah_eeprom.map.mapAr9287;
	...
	antWrites[0] = (u_int16_t)((pModal->antCtrlCommon >> 28) & 0xF); /* ??? kiwi check this */
	antWrites[1] = (u_int16_t)((pModal->antCtrlCommon >> 24) & 0xF);
	antWrites[2] = (u_int16_t)((pModal->antCtrlCommon >> 20) & 0xF);
	antWrites[3] = (u_int16_t)((pModal->antCtrlCommon >> 16) & 0xF);
	antWrites[4] = (u_int16_t)((pModal->antCtrlCommon >> 12) & 0xF);
	antWrites[5] = (u_int16_t)((pModal->antCtrlCommon >> 8) & 0xF);
	antWrites[6] = (u_int16_t)((pModal->antCtrlCommon >> 4)  & 0xF);
	antWrites[7] = (u_int16_t)(pModal->antCtrlCommon & 0xF);

	offset_num = 8;

    for (i = 0, j = offset_num; i < AR9287_MAX_CHAINS; i++) {
		antWrites[j++] = (u_int16_t)((pModal->antCtrlChain[i] >> 28) & 0xf);
        antWrites[j++] = (u_int16_t)((pModal->antCtrlChain[i] >> 10) & 0x3);
        antWrites[j++] = (u_int16_t)((pModal->antCtrlChain[i] >> 8) & 0x3);
        antWrites[j++] = 0; //dummy placeholder this is missing for merlin
        antWrites[j++] = (u_int16_t)((pModal->antCtrlChain[i] >> 6) & 0x3);
        antWrites[j++] = (u_int16_t)((pModal->antCtrlChain[i] >> 4) & 0x3);
        antWrites[j++] = (u_int16_t)((pModal->antCtrlChain[i] >> 2) & 0x3);
        antWrites[j++] = (u_int16_t)(pModal->antCtrlChain[i] & 0x3);
    }
    HALASSERT (j <= AR9287_ANT_16S);

    if (HAL_OK == ar5416EepromGetAntCfg(ahp, chan, ap->ah_config.ath_hal_defaultAntCfg, &ant_config)) {
...
ar9287FillEeprom(struct ath_hal *ah)
{
    ...
    {
        if (!ahp->ah_priv.ah_eepromRead(ah, addr, &val))
        {
            HDPRINTF(ah, HAL_DBG_EEPROM, "%s: Unable to read eeprom region \n", __func__);
            return AH_FALSE;
        }
...
ar9287EepromSetPowerCalTable(struct ath_hal *ah, ar9287_eeprom_t *pEepData, HAL_CHANNEL_INTERNAL *chan, int16_t *pTxPowerIndexOffset)
{
    ...
    u_int8_t  pdadcValues[AR9287_NUM_PDADC_VALUES];
    u_int16_t gainBoundaries[AR9287_PD_GAINS_IN_MASK];
...
#define INCREASE_MAXPOW_BY_THREE_CHAIN   10 /* 10*log10(3)*2 */
    ...
    */
    ...
    if (AH_PRIVATE(ah)->ah_config.ath_hal_desc_tpc) {
         ...
         */
        u_int8_t chainmasks[AR5416_MAX_CHAINS_KIWI] =
            {AR5416_1_CHAINMASK, AR5416_2LOHI_CHAINMASK};
        for (i = 0; i < AR5416_MAX_CHAINS_KIWI; i++) {
            int j;
	        ar9287EepromSetPowerPerRateTable(ah, pEepData, chan,
