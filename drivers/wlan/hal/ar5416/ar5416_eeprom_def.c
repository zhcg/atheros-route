...
ar5416EepromDefSetBoardValues(struct ath_hal *ah, HAL_CHANNEL_INTERNAL *chan)
{
    ...
    if (AR_SREV_MERLIN_10_OR_LATER(ah)) {
        ...
        if (IS_CHAN_2GHZ(chan)) {
            ...
            u_int32_t regValue;
            ath_carr_merlin_update_ob_rf2g_ch0(ah, regValue);
            OS_REG_WRITE(ah, AR_AN_RF2G1_CH0, regValue |
                SM(pModal->ob,AR_AN_RF2G1_CH0_OB) |
                SM(pModal->db,AR_AN_RF2G1_CH0_DB));
            OS_DELAY(100);

            ath_carr_merlin_update_ob_rf2g_ch1(ah, regValue);
            regValue &= (~AR_AN_RF2G1_CH1_OB) & (~AR_AN_RF2G1_CH1_DB);
            OS_REG_WRITE(ah, AR_AN_RF2G1_CH1, regValue |
                SM(pModal->ob,AR_AN_RF2G1_CH1_OB) |
...
#ifdef __CARRIER_PLATFORM__
            u_int32_t regValue;
            ath_carr_merlin_update_ob_rf5g_ch0(ah, regValue);
            OS_REG_WRITE(ah, AR_AN_RF5G1_CH0, regValue |
                SM(pModal->ob,AR_AN_RF5G1_CH0_OB5) |
                SM(pModal->db,AR_AN_RF5G1_CH0_DB5));
            OS_DELAY(100);

            ath_carr_merlin_update_ob_rf5g_ch1(ah, regValue);
            regValue &= (~AR_AN_RF5G1_CH1_OB5) & (~AR_AN_RF5G1_CH1_DB5);
            OS_REG_WRITE(ah, AR_AN_RF5G1_CH1, regValue |
                SM(pModal->ob,AR_AN_RF5G1_CH1_OB5) |
        ...
        {
            u_int32_t regValue;
            ath_carr_merline_reg_read_an_top2(ah, regValue);
            regValue &= (~AR_AN_TOP2_XPABIAS_LVL) & (~AR_AN_TOP2_LOCALBIAS);
            OS_REG_WRITE(ah, AR_AN_TOP2, regValue |
                SM(pModal->xpaBiasLvl,AR_AN_TOP2_XPABIAS_LVL) |
        ...
        if (ap->ah_opmode == HAL_M_STA 
            && ap->ah_config.ath_hal_sta_update_tx_pwr_enable)
        {
            for (i = 0; i < 9; i++) {
            ...
            for (i = 0; i < 9; i++) {
                ah->txPowerRecord[0][i]  = 0x0;
                if ((regList[i][1]&TXPOWER(0)) < 0x14 ) {
                    ah->txPowerRecord[0][i] |= (regList[i][1]&TXPOWER(0));
                } else {
                    ah->txPowerRecord[0][i] |= 0x14;
                }
                if ((regList[i][1]&TXPOWER(1)) < 0x1400 ) {
                    ah->txPowerRecord[0][i] |= (regList[i][1]&TXPOWER(1));
                } else {
                    ah->txPowerRecord[0][i] |= 0x1400;
                }
                if ((regList[i][1]&TXPOWER(2)) < 0x140000 ) {
                    ah->txPowerRecord[0][i] |= (regList[i][1]&TXPOWER(2));
                } else {
                    ah->txPowerRecord[0][i] |= 0x140000;
                }
                if ((regList[i][1]&TXPOWER(3)) < 0x14000000 ) {
                    ah->txPowerRecord[0][i] |= (regList[i][1]&TXPOWER(3));
                } else {
                    ah->txPowerRecord[0][i] |= 0x14000000;
                }
            }
                   
            for (i = 0; i < 9; i++) {
                ah->txPowerRecord[1][i]  = 0x0;
                if ((regList[i][1]&TXPOWER(0)) < 0x1E ) {
                    ah->txPowerRecord[1][i] |= (regList[i][1]&TXPOWER(0));
                } else {
                    ah->txPowerRecord[1][i] |= 0x1E;
                }
                if ((regList[i][1]&TXPOWER(1)) < 0x1E00 ) {
                    ah->txPowerRecord[1][i] |= (regList[i][1]&TXPOWER(1));
                } else {
                    ah->txPowerRecord[1][i] |= 0x1E00;
                }
                if ((regList[i][1]&TXPOWER(2)) < 0x1E0000 ) {
                    ah->txPowerRecord[1][i] |= (regList[i][1]&TXPOWER(2));
                } else {
                    ah->txPowerRecord[1][i] |= 0x1E0000;
                }
                if ((regList[i][1]&TXPOWER(3)) < 0x1E000000 ) {
                    ah->txPowerRecord[1][i] |= (regList[i][1]&TXPOWER(3));
                } else {
                    ah->txPowerRecord[1][i] |= 0x1E000000;
                }
            }
    ...
    u_int8_t * pPDADCValues, u_int16_t numXpdGains)
{
    ...
    u_int8_t   minPwrT4[AR5416_EEPDEF_NUM_PD_GAINS];
...
ar5416FillEepromDef(struct ath_hal *ah)
{
    ...
    for (addr = 0; addr < sizeof(ar5416_eeprom_t) / sizeof(u_int16_t); 
        addr++)
    {
        if (!ahp->ah_priv.priv.ah_eepromRead(ah, addr + owl_eep_start_loc, 
            eep_data))
        {
...
ar5416EepromDefSetPowerCalTable(struct ath_hal *ah, ar5416_eeprom_def_t *pEepData, HAL_CHANNEL_INTERNAL *chan, int16_t *pTxPowerIndexOffset)
{
    ...
    u_int16_t gainBoundaries[AR5416_EEPDEF_PD_GAINS_IN_MASK];
...
#ifdef ATH_TX99_DIAG
    ah->ah_pwr_offset = pwrTableOffset;
#endif
    ...
    */
    ...
    switch (owl_get_ntxchains(ahp->ah_txchainmask))
    {
        case 1:
            break;
        case 2:
            AH_PRIVATE(ah)->ah_maxPowerLevel += INCREASE_MAXPOW_BY_TWO_CHAIN;
            break;
        case 3:
            AH_PRIVATE(ah)->ah_maxPowerLevel += INCREASE_MAXPOW_BY_THREE_CHAIN;
            break;
        default:
            HALASSERT(0); /* Unsupported number of chains */
    }
