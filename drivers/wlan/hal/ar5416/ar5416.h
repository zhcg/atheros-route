...
#define AR5416_MAX_CHAINS_KIWI       2
...
#define ar5416_send_txpower_to_target(ah, mode) do {              \
    if ((chainmask) == AR5416_3_CHAINMASK) {                      \
        WMI_SEND_TXPOWER_TARGET((ah), (mode));                    \
    }                                                             \
} while (0)
#else
...
extern  HAL_BOOL ar5416PrintKeyCache(struct ath_hal *ah);
...
extern  void ar7010RegisterGpio(struct ath_hal *ah);
extern  HAL_BOOL ar7010GpioCfgOutput(struct ath_hal *, u_int32_t gpio, HAL_GPIO_OUTPUT_MUX_TYPE signalType);
extern  HAL_BOOL ar7010GpioCfgInput(struct ath_hal *, u_int32_t gpio);
extern  HAL_BOOL ar7010GpioSet(struct ath_hal *, u_int32_t gpio, u_int32_t val);
extern  u_int32_t ar7010GpioGet(struct ath_hal *ah, u_int32_t gpio);
extern  void ar7010GpioSetIntr(struct ath_hal *ah, u_int, u_int32_t ilevel);
#endif
...
extern  u_int64_t ar5416GetTsf2_64(struct ath_hal *ah);
...
#if ATH_WOW
extern  void ar5416WowApplyPattern(struct ath_hal *ah, u_int8_t *pAthPattern,
        u_int8_t *pAthMask, int32_t pattern_count, u_int32_t athPatternLen);
extern  u_int32_t ar5416WowWakeUp(struct ath_hal *ah);
extern  HAL_BOOL ar5416WowEnable(struct ath_hal *ah, u_int32_t patternEnable, u_int32_t timeoutInSeconds, int clearbssid);
#endif
...
extern  void ar5416SetNominalUserNFVal(struct ath_hal *ah, int16_t val,
        HAL_BOOL is2GHz);
extern  int16_t ar5416GetNominalUserNFVal(struct ath_hal *ah, HAL_BOOL is2GHz);
extern  void ar5416SetMinUserNFVal(struct ath_hal *ah, int16_t val,
        HAL_BOOL is2GHz);
extern  int16_t ar5416GetMinUserNFVal(struct ath_hal *ah, HAL_BOOL is2GHz);
extern  void ar5416SetMaxUserNFVal(struct ath_hal *ah, int16_t val,
        HAL_BOOL is2GHz);
extern  int16_t ar5416GetMaxUserNFVal(struct ath_hal *ah, HAL_BOOL is2GHz);
...
extern void ar5416ConfigFftPeriod(struct ath_hal *ah, u_int32_t inVal);
extern void ar5416ConfigScanPeriod(struct ath_hal *ah, u_int32_t inVal);
extern void ar5416ConfigScanCount(struct ath_hal *ah, u_int32_t inVal);
extern void ar5416ConfigShortRpt(struct ath_hal *ah, u_int32_t inVal);
...
extern u_int32_t ar5416SpectralScanCapable( struct ath_hal *ah);
...
extern HAL_STATUS ar5416CalculateADCRefPowers(struct ath_hal *ah, int freqMhz, int16_t *sample_min, int16_t *sample_max, int32_t *chain_ref_pwr, int num_chain_ref_pwr);
extern HAL_STATUS ar5416GetMinAGCGain(struct ath_hal *ah, int freqMhz, int32_t *chain_gain, int num_chain_gain);
...
extern int ar5416UpdateNavreg(struct ath_hal *);
...
#ifdef ATH_SUPPORT_HTC
extern void ar5416HTCResetInit(struct ath_hal *ah);
#endif
...
extern void ar5416GetVowStats(struct ath_hal *, HAL_VOWSTATS*);
#ifdef ATH_CCX
...
#ifdef _WIN64
extern  void ar5416GetDescLinkPtr(struct ath_hal *ah, void *ds, u_int32_t __unaligned **link);
#else
...
#endif
extern HAL_BOOL ar5416FillKeyTxDesc(struct ath_hal *ah, void *ds,
...
#ifdef ATH_TX99_DIAG
#ifdef ATH_SUPPORT_HTC
extern void ar5416_tx99_tx_stopdma(struct ath_hal *ah, u_int32_t hwq_num);
extern void ar5416_tx99_drain_alltxq(struct ath_hal *ah);
#else
