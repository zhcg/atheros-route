...
struct dfs_pulse ar5416_etsi_radars[] = {
    ...
    {20,  2,  300,  400, 2, 30,  4,  0,  2, 15, 0,   0, 31},
    /* 0.8-2us, 2-3 bursts, 400-1200 PRF, 15 pulses each */
    {30,  2,  400, 1200, 2, 30,  7,  0,  2, 15, 0,   0, 32},

    /* constant PRF based */
    /* 0.8-5us, 200  300 PRF, 10 pulses */

    {10, 5,   200,  400, 0, 24,  5,  0,  8, 18, 0,   0, 33},
    {10, 5,   400,  600, 0, 24,  5,  0,  8, 18, 0,   0, 37},
    {10, 5,   600,  800, 0, 24,  5,  0,  8, 18, 0,   0, 38},
    {10, 5,   800, 1000, 0, 24,  5,  0,  8, 18, 0,   0, 39},
    //{10, 5,   200, 1000, 0, 24,  5,  0,  8, 18, 0,   0, 33},

    /* 0.8-15us, 200-1600 PRF, 15 pulses */
    {15, 15,  200, 1600, 0, 24, 6,  0, 18, 15, 0,   0, 34},

    /* 0.8-15us, 2300-4000 PRF, 25 pulses*/
    {25, 15, 2300, 4000,  0, 24, 8, 0, 18, 15, 0,   0, 35},

    /* 20-30us, 2000-4000 PRF, 20 pulses*/
    {20, 30, 2000, 4000, 0, 24, 8, 19, 33, 15, 0,   0, 36},
};
...
//    {18,  5,  260,  260, 1, 10,  6,  1,  6, 18,  0, 3, 19}, // 3846 +/- 7 us
    ...
    // FCC TYPE 6
    {9,   1, 3003, 3003, 0,  7,  6,  0,  1, 18,  0, 0,  1}, // 333 +/- 7 us
    // {9,   1, 3003, 3003, 1,  7,  5,  0,  1, 18,  0, 0,  1},

    // FCC TYPE 2
    {23, 5, 4347, 6666, 0, 18, 11,  0,  7, 20,  0, 3,  2},

    // FCC TYPE 3
    {18, 10, 2000, 5000, 0, 23,  8,  6, 13, 20,  0, 3, 5},

    // FCC TYPE 4
    {16, 15, 2000, 5000, 0, 25,  7, 11, 23, 20,  0, 3, 11},
...
void ar5416_adjust_difs(struct ath_hal *ah, u_int32_t val)
{
    if (val == 0) {
         ...
         * make sure that they match values in INI file
         */

        ah->ah_fccaifs = 0;
        HDPRINTF(ah, HAL_DBG_DFS, "%s: set DIFS to default\n", __func__);
        OS_REG_WRITE(ah, AR_DLCL_IFS(0), AR5416_DEFAULT_DIFS);
        OS_REG_WRITE(ah, AR_DLCL_IFS(1), AR5416_DEFAULT_DIFS);
        OS_REG_WRITE(ah, AR_DLCL_IFS(2), AR5416_DEFAULT_DIFS);
        OS_REG_WRITE(ah, AR_DLCL_IFS(3), AR5416_DEFAULT_DIFS);
    } else {
        /*
