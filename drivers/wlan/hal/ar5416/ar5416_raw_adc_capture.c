...
static int16_t gainCalOffset(int freqMhz, int chain)
{
    ...
    for(i = 0; gain_cal_data_merlin[i].freq != 0; i++) {
        if (gain_cal_data_merlin[i].freq == freqMhz ||
            gain_cal_data_merlin[i+1].freq > freqMhz ||
...
HAL_STATUS
ar5416GetMinAGCGain(struct ath_hal *ah, int freqMhz, int32_t *chain_gain, int num_chain_gain)
{
#ifdef not_yet
    int ch;
...
HAL_STATUS
ar5416CalculateADCRefPowers(struct ath_hal *ah, int freqMhz, int16_t *sample_min, int16_t *sample_max, int32_t *chain_ref_pwr, int num_chain_ref_pwr)
{
    int32_t ref_power;
    int ch;
    const int32_t GI = (freqMhz < 5000) ? AR5416_GI_2GHZ : AR5416_GI_5GHZ;
    for(ch = 0; ch < num_chain_ref_pwr && ch < AR5416_MAX_CHAINS; ch++) {
        /*
         * calculation is done in 100 times units (0.01 db)
         */
        ref_power = -GI + gainCalOffset(freqMhz, ch) - AR5416_MAX_ADC_POWER;
