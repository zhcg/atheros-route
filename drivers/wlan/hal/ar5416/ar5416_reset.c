...
#define RESET_CAL_ATTEMPTS  100
    ...
    do {
        ...
        attempts++;
    } while ((res == AH_FALSE) && (attempts < RESET_CAL_ATTEMPTS) && ar5416DetectCardPresent(ah));
...
#define FAIL(_code)     do { ecode = _code; goto bad; } while (0)
    ...
    if (
        ...
        !(ap->ah_config.ath_hal_fullResetWarEnable && AR_SREV_MERLIN(ah)) &&
         ...
         ((chan->channelFlags & CHANNEL_ALL) ==
          (ap->ah_curchan->channelFlags & CHANNEL_ALL)))
        ||
        (AR_SREV_K2(ah) &&
         (ahp->ah_chipFullSleep != AH_TRUE) &&
         (ap->ah_curchan != AH_NULL) &&
    ...
    if ( AR_SREV_K2(ah) &&
        ...
        OS_REG_WRITE(ah, AR9271_RESET_POWER_DOWN_CONTROL, AR9271_RADIO_RF_RST);
        OS_DELAY(50);
    }
#endif
    ...
    if ( AR_SREV_K2(ah) &&
         (ah->ah_htc_isResetInit == AH_TRUE) ) {
        ah->ah_htc_isResetInit = AH_FALSE;

        OS_REG_WRITE(ah, AR9271_RESET_POWER_DOWN_CONTROL, AR9271_GATE_MAC_CTL);
        OS_DELAY(50);
    }
#endif
    ...
    ahp->ah_intrTxqs = 0;
    for (i = 0; i < ap->ah_caps.halTotalQueues; i++) {
    ...
    if ((rx_chainmask == 0x5) || (rx_chainmask == 0x3)) {
        OS_REG_WRITE(ah, AR_PHY_RX_CHAINMASK,
            ap->greenApPsOn ? HAL_GREEN_AP_RX_MASK : rx_chainmask);
...
#ifdef ATH_SUPPORT_HTC
        /* Configure Magpie target WLAN */
		if (AR_SREV_K2(ah)) {
            OS_REG_WRITE(ah, AR_CFG, AR_CFG_SWRB | AR_CFG_SWTB);
		} else {
                           ...
                           HAL_CHANNEL *chan, int is_scan)
{
    ...
    recentNFIndex = (h->base.currIndex) ?
        h->base.currIndex - 1 : nf_hist_len - 1;
    for (i = 0; i < NUM_NF_READINGS; i ++) {
...
ar5416ResetNfHistBuff(struct ath_hal *ah, HAL_CHANNEL_INTERNAL *ichan)
{
    ...
    for (i = 0; i < NUM_NF_READINGS; i++) {
         ...
         */
        nf = ar5416LimitNfRange(ah, h->nfCalBuffer[0][i]);
                       ...
                       int16_t *nfarray, int hist_len)
{
    ...
    for (i = 0; i < NUM_NF_READINGS; i++) {
        ...
        h->base.privNF[i] = ar5416LimitNfRange(
            ah, ar5416GetNfHistMid(h, i, hist_len));
    }
    if (++h->base.currIndex >= hist_len) h->base.currIndex = 0;
 ...
 */
void
ar5416SetNominalUserNFVal(struct ath_hal *ah, int16_t val, HAL_BOOL is2GHz)
{
    if (is2GHz) {
...
int16_t
ar5416GetNominalUserNFVal(struct ath_hal *ah, HAL_BOOL is2GHz)
{
    return (is2GHz) ?
...
void
ar5416SetMinUserNFVal(struct ath_hal *ah, int16_t val, HAL_BOOL is2GHz)
{
    if (is2GHz) {
...
int16_t
ar5416GetMinUserNFVal(struct ath_hal *ah, HAL_BOOL is2GHz)
{
    return (is2GHz) ?
...
void
ar5416SetMaxUserNFVal(struct ath_hal *ah, int16_t val, HAL_BOOL is2GHz)
{
    if (is2GHz) {
...
int16_t
ar5416GetMaxUserNFVal(struct ath_hal *ah, HAL_BOOL is2GHz)
{
    return (is2GHz) ?
...
void
ar5416SetNfDeltaVal(struct ath_hal *ah, int16_t val)
{
    AH_PRIVATE(ah)->nf_cw_int_delta = val;
}

int16_t
ar5416GetNfDeltaVal(struct ath_hal *ah)
{
...
ar5416InitChainMasks(struct ath_hal *ah)
{
    ...
    switch (rx_chainmask) {
        ...
        case 0x3:
            if ((ar5416Get11nHwPlatform(ah) == HAL_TRUE_CHIP) &&
                ((AH_PRIVATE(ah))->ah_macVersion == AR_SREV_REVISION_OWL_10)) {
                /*
    ...
    if (ah->ah_getDcsMode(ah)) {
        u_int32_t cap_chainmask = 0;
        if ((ah->ah_getCapability(ah, HAL_CAP_TX_CHAINMASK, 0, &cap_chainmask)
...
ar9285PACal(struct ath_hal *ah, HAL_BOOL isReset)
{
   ...
   OS_REG_WRITE(ah, 0x9808, regVal);
    ...
    if((AH_PRIVATE(ah)->ah_opmode == HAL_M_STA) && 
        ...
        OS_REG_WRITE(ah, AR9285_AN_RF2G3, ah->OBDB1[0]);
        OS_REG_WRITE(ah, AR9285_AN_RF2G4, ah->DB2[0]);
    }
        ...
        for (i = 0; i < ahp->ah_iniCommon_normal_cck_fir_coeff_K2.ia_rows; i++) {
            ...
            */
            if (reg >= 0x7800 && reg < 0x7900) {
                ...
                ath_carr_merlin_fill_reg_shadow(ah, reg, val);
#endif
            } else {
                OS_REG_WRITE(ah, reg, val);
            }
