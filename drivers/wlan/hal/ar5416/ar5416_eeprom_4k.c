...
ar5416Eeprom4kSetBoardValues(struct ath_hal *ah, HAL_CHANNEL_INTERNAL *chan)
{
    ...
    if (pModal->version >= 3) {
        ...
        if (pCap->halAntDivCombSupport && ahp->ah_diversityControl != HAL_ANT_FIXED_A 
    ...
    /* Green Tx */
    if (ahpriv->ah_opmode == HAL_M_STA 
            && ahpriv->ah_config.ath_hal_sta_update_tx_pwr_enable)
    {
        for (i = 0; i < 9; i++) {
        ...
        ah->txPowerRecord[0][0]=0x1a1a1a1a;    //18/12/9/6 = 8/8/8/8
        ah->txPowerRecord[0][1]=0x0c121a1a;    //54/48/36/24 = 1/4/8/8
        ah->txPowerRecord[0][2]=0x1a1a1a1a;    //CCK = 8
        ah->txPowerRecord[0][3]=0x1a1a1a1a;    //CCK = 8
        ah->txPowerRecord[0][4]=0x1a1a1a1a;    //HT20 MCS3/MCS2/MCS1/MCS0 = 8/8/8/8
        ah->txPowerRecord[0][5]=0x0c12161a;    //HT20 MCS7/MCS6/MCS5/MCS4 = 1/4/6/8
        ah->txPowerRecord[0][6]=0x14141414;    //HT40 MCS3/MCS2/MCS1/MCS0 = 5/5/5/5
        ah->txPowerRecord[0][7]=0x0a101414;    //HT40 MCS7/MCS6/MCS5/MCS4 = 0/3/5/5
        ah->txPowerRecord[0][8]=0x0a0a0a0a;    //duplicate = 0
            
        /* for PAL on, OB/DB1/DB2 = 2/2/2 */
        ah->txPowerRecord[1][0]=0x1e1e1e1e;    //18/12/9/6   = 10/10/10/10
        ah->txPowerRecord[1][1]=0x1e1e1e1e;    //54/48/36/24 = 10/10/10/10
        ah->txPowerRecord[1][2]=0x1e1e1e1e;    //CCK = 10
        ah->txPowerRecord[1][3]=0x1e1e1e1e;    //CCK = 10
        ah->txPowerRecord[1][4]=0x1e1e1e1e;    //HT20 MCS3/MCS2/MCS1/MCS0 = 10/10/10/10
        ah->txPowerRecord[1][5]=0x1e1e1e1e;    //HT20 MCS7/MCS6/MCS5/MCS4 = 10/10/10/10
        ah->txPowerRecord[1][6]=0x18181818;    //HT40 MCS3/MCS2/MCS1/MCS0 =  7/7/7/7
        ah->txPowerRecord[1][7]=0x18181818;    //HT40 MCS7/MCS6/MCS5/MCS4 =  7/7/7/7
        ah->txPowerRecord[1][8]=0x18181818;    //duplicate = 7         
            
        /* original target power for status 3 */
        ah->txPowerRecord[2][0] = regList[0][1];
        ah->txPowerRecord[2][1] = regList[1][1];
        ah->txPowerRecord[2][2] = regList[2][1];
        ah->txPowerRecord[2][3] = regList[3][1];
        ah->txPowerRecord[2][4] = regList[4][1];
        ah->txPowerRecord[2][5] = regList[5][1];
        ah->txPowerRecord[2][6] = regList[6][1];
        ah->txPowerRecord[2][7] = regList[7][1];
        ah->txPowerRecord[2][8] = regList[8][1];

        /* Record PAL */
        ah->PALRecord = OS_REG_READ(ah, AR9285_AN_RF2G7);

        /* Record OB/DB1/DB2 */
        ah->OBDB1[0] = OS_REG_READ(ah, AR9285_AN_RF2G3);
        ah->DB2[0]   = OS_REG_READ(ah, AR9285_AN_RF2G4);

        /* force OB/DB1/DB2 to 1/1/1,2/2/2 */
    	if (AR_SREV_K2(ah)) {
            for (i=1; i<ARRAY_LENGTH(ah->OBDB1); i++) {
                ah->OBDB1[i] = ((ah->OBDB1[0])&0xff000fff)|(i<<21)|(i<<18)|(i<<15)|(i<<12); 
                ah->DB2[i] = ((ah->DB2[0])&0x1fffffff)|(i<<29);
            }
        } else {
            for (i=1; i<ARRAY_LENGTH(ah->OBDB1); i++) {
                ah->OBDB1[i] = ((ah->OBDB1[0])&0xff000000)|(i<<21)|(i<<18)|(i<<15)|(i<<12)|(i<<9)|(i<<6)|(i<<3)|(i<<0); 
                ah->DB2[i] = ((ah->DB2[0])&0x00000fff)|(i<<29)|(i<<26)|(i<<23)|(i<<20)|(i<<17)|(i<<14)|(i<<11);
            }
        }
    }
...
ar5416Eep4kSetPowerCalTable(struct ath_hal *ah, ar5416_eeprom_4k_t *pEepData, HAL_CHANNEL_INTERNAL *chan, int16_t *pTxPowerIndexOffset)
{
    ...
    ahp = AH5416(ah);
#ifdef ATH_TX99_DIAG
    ah->ah_pwr_offset = (int8_t)ar5416EepromGet(ahp, EEP_PWR_TABLE_OFFSET);
#endif
       ...
       if (!ar5416Eep4kGetGainBoundariesAndPdadcs(ah, chan, pRawDataset,
        ...
        for (j = 0; j < 32; j++) {
                ...
                HDPRINTF(ah, HAL_DBG_PHY_IO, "PDADC (%d,%4x): %4.4x %8.8x\n",
                    0, regChainOffset, regOffset, reg32);
    ...
    */
    ...
    if (AH_PRIVATE(ah)->ah_config.ath_hal_desc_tpc) {
        ...
        /* Software Power Per Rate values in ath_hal_5416.txPowers[][]
           ...
           which for single-chain models that use the eeprom_4k model is
           only 1.
         ...
         */
        ar5416Eep4kSetPowerPerRateTable(ah, pEepData, chan,
            &ratesArray[0], cfgCtl, twiceAntennaReduction,
            twiceMaxRegulatoryPower, powerLimit, AR5416_1_CHAINMASK);
        /* adjust to -5 dBm ref */
...
ar5416CheckEeprom4k(struct ath_hal *ah)
{
    ...
    if (!ar5416EepDataInFlash(ah))
    {
        ...
        if (!ahp->ah_priv.priv.ah_eepromRead(
...
ar5416FillEeprom4k(struct ath_hal *ah)
{
    ...
    for (addr = 0; addr < sizeof(ar5416_eeprom_4k_t) / sizeof(u_int16_t); 
        addr++)
    {
        if (!ahp->ah_priv.priv.ah_eepromRead(ah, addr + eep_start_loc, 
            eep_data))
        {
