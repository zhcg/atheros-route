...
int ar9300UpdateNavReg(struct ath_hal *ah)
{
        u_int32_t regval = 0;
        regval = OS_REG_READ(ah, 0x8084);
        if(regval != 0xdeadbeef && regval > 0x7fff) {
            ath_hal_printf(ah,"%s: abnormal nav value found 0x%x\n", __func__, regval);
            OS_REG_WRITE(ah, 0x8084, 0);
        }
  return 0;
...
void
ar9300_cac_tx_quiet(struct ath_hal *ah, bool enable)
{
    u32 reg1, reg2;
        ...
        u_int32_t capability, u_int32_t *result)
{
    ...
    case HAL_CAP_DS: /* supports both two and three streams */
        return (AR_SREV_HORNET(ah) || AR_SREV_POSEIDON(ah) ||
                (pCap->halTxChainMask & 0x3) != 0x3 ||
                (pCap->halRxChainMask & 0x3) != 0x3) ? 
            HAL_ENOTSUPP : HAL_OK;
    case HAL_CAP_TS:
        return (AR_SREV_HORNET(ah) || AR_SREV_POSEIDON(ah) || 
                AR_SREV_WASP(ah) ||
                (pCap->halTxChainMask & 0x7) != 0x7 ||
                (pCap->halRxChainMask & 0x7) != 0x7) ? 
            HAL_ENOTSUPP : HAL_OK;
...
#define HAL_BB_PANIC_IDLE_TIME_OUT 0x0a8c0000
    ...
    if (idle_tmo_ms != 0) {
        ...
        OS_REG_WRITE(ah, AR_PHY_PANIC_WD_CTL_1, 
                     ...
                     (AR_PHY_BB_PANIC_IDLE_MASK & HAL_BB_PANIC_IDLE_TIME_OUT) | (AR_PHY_BB_PANIC_NON_IDLE_MASK & (idle_count << 2)));
    } else {
...
int
ar9300GetBbPanicInfo(struct ath_hal *ah, struct hal_bb_panic_info *bb_panic)
{
    bb_panic->status = AH_PRIVATE(ah)->ah_bbPanicLastStatus;
    ...
    bb_panic->tsf = ar9300GetTsf32(ah);
    ...
    bb_panic->rODFM = MS(bb_panic->status, AR_PHY_BB_WD_RX_OFDM_SM);
    bb_panic->rCCK = MS(bb_panic->status, AR_PHY_BB_WD_RX_CCK_SM);
    bb_panic->tODFM = MS(bb_panic->status, AR_PHY_BB_WD_TX_OFDM_SM);
    bb_panic->tCCK = MS(bb_panic->status, AR_PHY_BB_WD_TX_CCK_SM);
    ...
    bb_panic->cycles = ar9300GetMibCycleCountsPct(ah, &bb_panic->rxc_pcnt, &bb_panic->rxf_pcnt, &bb_panic->txf_pcnt);
    ...
    if (AH_PRIVATE(ah)->ah_config.ath_hal_show_bb_panic) {
        ...
        ath_hal_printf(ah, "** BB state: wd=%u det=%u rdar=%u rOFDM=%d rCCK=%u tOFDM=%u tCCK=%u agc=%u src=%u **\n",
            ...
            bb_panic->rODFM, bb_panic->rCCK, bb_panic->tODFM,
            bb_panic->tCCK, bb_panic->agc, bb_panic->src);
        ...
        if (bb_panic->cycles) {
            ath_hal_printf(ah, "** BB busy times: rx_clear=%d%%, rx_frame=%d%%, tx_frame=%d%% **\n",
                bb_panic->rxc_pcnt, bb_panic->rxf_pcnt, bb_panic->txf_pcnt);
        }
...
ar9300DetectMacHang(struct ath_hal *ah)
{
    ...
    HDPRINTF(ah, HAL_DBG_DFS, " dma regs: %X %X %X \n",
            mac_dbg.dma_dbg_4,mac_dbg.dma_dbg_5,
            mac_dbg.dma_dbg_6);

    if (hang_sig1 != 
            ar9300CompareDbgHang(ah, mac_dbg, hang_sig1_val, hang_sig1,&dcu_chain))
    {
        HDPRINTF(ah, HAL_DBG_DFS, " hang sig1 not found \n");
        return AH_FALSE;
    }
    ...
    for (i = 1; i <= NUM_STATUS_READS; i++) {
        if(dcu_chain < 6) {
            mac_dbg.dma_dbg_4 = OS_REG_READ(ah, AR_DMADBG_4);
            current_dcu_chain_state = ((mac_dbg.dma_dbg_4 >> shift_val) & 0x1f); 
        } else {
            mac_dbg.dma_dbg_5 = OS_REG_READ(ah, AR_DMADBG_5);
...
HAL_BOOL
ar9300SetSmartAntenna(struct ath_hal *ah, HAL_BOOL enable)
{
    struct ath_hal_9300 *ahp = AH9300(ah); 
    if (enable) {
