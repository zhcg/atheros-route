 ...
 * driver calls ar9300SetStaBeaconTimers instead.
 */
void
ar9300BeaconInit(struct ath_hal *ah,
    u_int32_t next_beacon, u_int32_t beacon_period, HAL_OPMODE opmode)
{
    ...
    case HAL_M_RAW_ADC_CAPTURE:
        OS_REG_WRITE(ah, AR_NEXT_TBTT_TIMER,
 ...
 */
HAL_BOOL
ar9300WaitForBeaconDone(struct ath_hal *ah, HAL_BUS_ADDR baddr)
{
    int i;

    for (i = 0; i < 1000; i++) {
        /* XXX no check of CAB */
        if (ar9300NumTxPending(ah, HAL_TX_QUEUE_BEACON) == 0) {
            break;
        }
        OS_DELAY(10);
    }
...
void
ar9300ResetStaBeaconTimers(struct ath_hal *ah)
{
    u_int32_t val;
 ...
 */
void
ar9300SetStaBeaconTimers(struct ath_hal *ah, const HAL_BEACON_STATE *bs)
{
