 ...
 */
HAL_BOOL
ar9300IsInterruptPending(struct ath_hal *ah)
{
     ...
     */
    u_int32_t host_isr = OS_REG_READ(ah, AR_HOSTIF_REG(ah, AR_INTR_ASYNC_CAUSE));
    ...
    HAL_BOOL nortc)
{
    ...
    *masked = 0;
    ...
    if (async_cause) {
        if (nortc) {
            OS_REG_WRITE(ah, AR_HOSTIF_REG(ah, AR_INTR_ASYNC_CAUSE_CLR), async_cause);
            // Flush prior write
...
#endif
            *masked |= HAL_INT_FATAL; /* Set FATAL flag here so that driver gets it*/
        }
    ...
    if (HAL_INT_MSI == type) {
         ...
         * In normal case, SW read HOST_INTF_PCIE_MSI (0x40A4) and write into ah_msiReg. 
         * Then use value of ah_msiReg to set bit#25 when want to enable HW write the cfg_msi_pending.
         * Sometimes, driver get MSI interrupt before read 0x40a4 and ah_msiReg is initialization value (0x0).
        ...
        OS_REG_WRITE(ah, AR_HOSTIF_REG(ah, AR_PCIE_MSI), \
                ((ahp->ah_msiReg | AR_PCIE_MSI_ENABLE) & msi_pend_addr_mask));
...
HAL_INT
ar9300GetInterrupts(struct ath_hal *ah)
{
    return AH9300(ah)->ah_maskReg;
}
 ...
 */
HAL_INT
ar9300SetInterrupts(struct ath_hal *ah, HAL_INT ints, HAL_BOOL nortc)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
    u_int32_t omask = ahp->ah_maskReg;
    u_int32_t mask, mask2, msiMask = 0;
    u_int32_t msi_pend_addr_mask = 0;
    u_int32_t sync_en_def = AR_INTR_SYNC_DEFAULT;
    ...
    if (omask & HAL_INT_GLOBAL) {
        ...
        (void) OS_REG_READ(ah, AR_HOSTIF_REG(ah, AR_INTR_SYNC_ENABLE)); /* flush write to HW */
        OS_REG_WRITE(ah, AR_HOSTIF_REG(ah, AR_INTR_ASYNC_ENABLE), 0);
        (void) OS_REG_READ(ah, AR_HOSTIF_REG(ah, AR_INTR_ASYNC_ENABLE)); /* flush write to HW */
    }
    ...
    if (!nortc) {
        ...
        if (ints & HAL_INT_GLOBAL) {
            ...
            if (ahp->ah_ier_refcount == 0) {
                HDPRINTF(ah, HAL_DBG_UNMASKABLE,
                    "%s: WARNING: ah_ier_refcount is 0 "
                    "and attempting to enable IER\n",
                    __func__);
            }
#endif
            if (ahp->ah_ier_refcount > 0) {
                OS_ATOMIC_DEC(&ahp->ah_ier_refcount);
            } 
        } else {
            HDPRINTF(ah, HAL_DBG_INTERRUPT,
                "%s: Request HAL_INT_GLOBAL DISABLED\n", __func__);
            OS_ATOMIC_INC(&ahp->ah_ier_refcount);
        }
        HDPRINTF(ah, HAL_DBG_INTERRUPT,
            "%s: ah_ier_refcount = %d\n", __func__, ahp->ah_ier_refcount);
        ...
        if (ints & HAL_INT_TX) {
            ...
            } else if (ahp->ah_txOkInterruptMask) {
                mask |= AR_IMR_TXOK;
            }
            msiMask |= AR_INTR_PRIO_TX;
        ...
        if (ints & HAL_INT_RX) {
            ...
            msiMask |= AR_INTR_PRIO_RXLP | AR_INTR_PRIO_RXHP;
            if (! pCap->halAutoSleepSupport) {
                mask |= AR_IMR_GENTMR;
            }
        }
    ...
    if ((ints & HAL_INT_GLOBAL)) {
#else
    if ((ints & HAL_INT_GLOBAL) && (ahp->ah_ier_refcount == 0)) {
#endif
...
#if 0 
// Why GPIO should be enabled for both SYNC and ASYNC ????
        ...
        if (AH_PRIVATE(ah)->ah_config.ath_hal_enableMSI) {
            OS_REG_WRITE(ah, AR_HOSTIF_REG(ah, AR_INTR_PRIO_ASYNC_ENABLE), msiMask);
            OS_REG_WRITE(ah, AR_HOSTIF_REG(ah, AR_INTR_PRIO_ASYNC_MASK), msiMask);
        ...
        OS_REG_WRITE(ah, 
            AR_HOSTIF_REG(ah, AR_INTR_SYNC_ENABLE), (sync_en_def | mask));
        OS_REG_WRITE(ah, 
            AR_HOSTIF_REG(ah, AR_INTR_SYNC_MASK), (sync_en_def | mask));
...
u_int32_t
ar9300GetIntrMitigationTimer(struct ath_hal* ah, HAL_INT_MITIGATION reg)
{
    u_int32_t val = 0;
#ifdef AR5416_INT_MITIGATION
    switch (reg) {
