...
{
	u_int16_t numChannels = pCalDataset->numChannels;
	const u_int16_t *pChanList = pCalDataset->pChannels;
	void *data;
	int i, j;

	/* Allocate the channel and Power Data arrays together */
	data = ath_hal_malloc(ah, 
		roundup(sizeof(u_int16_t) * numChannels, sizeof(u_int32_t)) +
