...
{
    //a_uint32_t count = 0, loop = 0;
    //HTC_ENDPOINT_ID i;
    //HTC_ENDPOINT *pEndpoint;
    //HTC_AGGRNUM_REC *pAggrNumRec = NULL;
    a_uint32_t i;
    HTC_ENDPOINT_ID ep;
    HTC_ENDPOINT *pEndpoint;
    //adf_os_handle_t os_hdl = target->os_handle;
    ...
    while (loop < 1000) {
        /* free memory for recording number of aggregated buffers */
        for (i = 0; i <ENDPOINT_MAX; i++) {
            ...
            if ((pAggrNumRec = HTC_AGGRNUMREC_DEQUEUE(&pEndpoint->AggrNumRecQueue)) != NULL) {
                adf_os_mem_free(pAggrNumRec);
                count++;
            }
        }
        ...
        while ((pAggrNumRec = HTC_AGGRNUMREC_DEQUEUE(&target->FreeAggrNumRecQueue)) != NULL) {
            adf_os_mem_free(pAggrNumRec);
            count++;
        }

        if (count == NUM_AGGRNUM_REC_BUFFERS) {
            break;
        }
...
#if 0
    //wait_status = adf_os_sleep_waitq(&target->wq, 100000);
    wait_status = adf_os_sleep_waitq(&target->wq, 10000);
    //wait_status = wait_event_timeout(target->wq, target->TargetCredits != 0, 100000);

    adf_os_print("[%s %d] Finish waiting...\n", __FUNCTION__, __LINE__);

    if (wait_status == A_STATUS_FAILED) {
        adf_os_print("[%s %d] timeout!\n", __FUNCTION__, __LINE__);
        /* timeout */
        status = A_ERROR;
    }
#else
