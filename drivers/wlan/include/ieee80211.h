...
/* The following struct has been changed from its original definition. In past
 * it contained a 'union ieee80211_capability', which held a bitfield of
 * capabilities. However, there's a problem with 'offsetof' calculation when a
 * struct contains bitfield. Therefore it's been changed to u_int16_t with
 * masks IEEE80211_CAPINFO_*.
 */ 
...
struct ieee80211_ie_qbssload {
    u_int8_t     qbssload_id;     /* IEEE80211_ELEMID_VENDOR */
    u_int8_t     qbssload_len;    /* length in bytes */
    u_int16_t    channel_load;    /* channel load */
};
