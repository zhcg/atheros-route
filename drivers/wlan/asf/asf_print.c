...
#include "asf_amem.h"     /* amalloc */
#include "adf_os_mem.h"   /* adf_os_str_cmp */
 ...
 */
static struct asf_print_ctrl AsfPrintCtrlShared = {
    {0}, /* all categories disabled */
    0,   /* lowest verbosity */
    "shared",
    0,
    NULL,
...
{
    /*
     * If the printout function pointer has already been initialized,
     * don't reinitialize it.  However, allow the function pointer
     * to be reset to NULL, so it can be reinitialized via a two-step
     * process of first resetting it to NULL, then setting the function
    ...
    const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    asf_vprint(handle, category, verb, fmt, args);
    va_end(args);
}
    ...
    va_list args)
{
    ...
    word = category / ASF_BITS_PER_WORD;
    bit = category - word * ASF_BITS_PER_WORD;

    if ( ! handle ) handle = &AsfPrintCtrlShared;
    if ( handle->category_mask[word] & (1 << bit) &&
         verb <= handle->verb_threshold ) {
        if (vprint) vprint(fmt, args);
    }
}
...
void asf_print_mask_set(struct asf_print_ctrl *handle, int category, int enable)
{
    ...
    if (enable) {
        handle->category_mask[word] |= (1 << bit);
    } else {
        handle->category_mask[word] &= ~(1 << bit);
    }
}
    ...
    int enable)
{
    ...
    do {
        if (! name || (p->name && ! adf_os_str_cmp(name, p->name))) {
...
{
    struct asf_print_ctrl *p;
    struct asf_print_bit_spec *bit_specs = NULL;
    ...
    p = handle->print_ctrl_list;
    do {
        if (p->name && ! adf_os_str_cmp(name, p->name)) {
