...
{
    /*
     * If the alloc_func and free_func have already been initialized,
     * don't reinitialize them.  However, allow the function pointers
     * to be reset to NULL, so they can be reinitialized via a two-step
     * process of first resetting them to NULL, then setting the function
     * pointers to a new value.
     */
    if (asf_amem.alloc_func && asf_amem.free_func &&
        (alloc_func != NULL || free_func != NULL)) {
        return;
    }
...
#if ASF_AMEM_SUPPORT >= ASF_AMEM_SUPPORT_SIMPLE_DIAG
    {
        ...
        for (i = 0; i < ASF_PRINT_MASK_WORDS; i++) {   
...
static enum asf_amem_status asf_amem_cache_add(asf_amem_cache_handle handle, int add_elem)
{
    ...
    struct asf_amem_elem_free_node *ptr;
     ...
     /* legal (though perhaps non-sensical) to request adding no elems */
    if ( add_elem <= 0 ) {
        return asf_amem_status_success;
    }
