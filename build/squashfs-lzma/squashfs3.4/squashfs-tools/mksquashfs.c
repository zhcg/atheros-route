...
/* All code marked with DEVFILE_HACK added by Ted Merrill, Atheros
*       2009-1-26
*/
#define DEVFILE_HACK 1
	...
	while(_readdir(filename, dir_name, dir) != FALSE) {
		...
		if(strcmp(dir_name, ".") == 0 || strcmp(dir_name, "..") == 0)
			continue;
                ...
                #ifdef DEVFILE_HACK
                if (devfile_hack && *dir_name == '+' &&
                            sscanf(dir_name, "+%cdev+%u_%u_%s",
                                &DevType, &Major, &Minor, NewName) == 4 &&
                            (DevType == 'c' || DevType == 'b')) {
                        /* do lstat so we get unique st_dev and st_ino */
		        if(lstat(filename, &buf) == -1) {
			        ERROR("Cannot stat dir/file %s because %s, ignoring", filename, strerror(errno));
			        continue;
		        }
                        INFO("Hack %s (%s) type %c dev %d.%d\n", 
                            filename, dir_name, DevType, Major, Minor);
                        strcpy(dir_name, NewName);
                        strcpy(strrchr(filename,'/')+1, NewName);
                        INFO("... to %s (%s)\n", filename, dir_name);
                        /* override some information */
                        if (DevType == 'c')
                            buf.st_mode = S_IFCHR | 0666;
                        else
                            buf.st_mode = S_IFBLK | 0666;
...
int main(int argc, char *argv[])
{
	...
	for(; i < argc; i++) {
                #ifdef DEVFILE_HACK
		if(strcmp(argv[i], "-devfile-hack") == 0) {
                    devfile_hack = 1;
                } else
                #endif /* DEVFILE_HACK */
...
printOptions:
			...
			ERROR("\nOptions are\n");
                        #ifdef DEVFILE_HACK
                        ERROR("-devfile-hack\t\tSpecial filenames make dev files: +{b|c}dev+<major>_<minor>_<name>\n");
                        #endif
